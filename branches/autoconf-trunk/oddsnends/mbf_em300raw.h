 ...
 *	$Id: mbf_em300raw.h,v 5.2 2001-05-24 23:18:07 caress Exp $
 *
 ...
 *--------------------------------------------------------------------*/
/*
 * mbf_em300raw.h defines the data structures used by MBIO functions
 * to store multibeam data read from the MBF_EM300RAW format 
 * (MBIO id 56).  
 *
 * Author:	D. W. Caress
 ...
 */
/*
 * Notes on the MBF_EM300RAW data structure:
...
#define	MBF_EM300RAW_MAXSSV		100
#define	MBF_EM300RAW_COMMENT_LENGTH	256
#define	MBF_EM300RAW_BUFFER_SIZE	1024
	
/* internal data structure */
struct mbf_em300raw_struct
	{
	/* type of data record */
	...
	char	par_rop[MBF_EM300RAW_COMMENT_LENGTH];	
				/* responsible operator */
	char	par_sid[MBF_EM300RAW_COMMENT_LENGTH];	
				/* survey identifier */
	char	par_pll[MBF_EM300RAW_COMMENT_LENGTH];	
				/* survey line identifier (planned line number) */
	char	par_com[MBF_EM300RAW_COMMENT_LENGTH];	
				/* comment */
	...
	int	svp_depth[MBF_EM300RAW_MAXSVP]; 
				/* depth of svp entries (according to svp_depth_res) */
	int	svp_vel[MBF_EM300RAW_MAXSVP];	
	...
	int	png_depth[MBF_EM300RAW_MAXBEAMS];	
				/* depths in depth resolution units */
	int	png_acrosstrack[MBF_EM300RAW_MAXBEAMS];
				/* acrosstrack distances in distance resolution units */
	int	png_alongtrack[MBF_EM300RAW_MAXBEAMS];
				/* alongtrack distances in distance resolution units */
	int	png_depression[MBF_EM300RAW_MAXBEAMS];
				/* Primary beam angles in one of two formats (see note 10 above)
				   ...
				   2) Uncorrected format - gives beam pointing angles
					...
					for vessel motion. */
	int	png_azimuth[MBF_EM300RAW_MAXBEAMS];
				/* Secondary beam angles in one of two formats (see note 10 above)
				   ...
				   2) Uncorrected format - combines a flag indicating that
					...
					motion. */
	int	png_range[MBF_EM300RAW_MAXBEAMS];
	...
	int	png_quality[MBF_EM300RAW_MAXBEAMS];	
				/* 0-254 */
	int	png_window[MBF_EM300RAW_MAXBEAMS];		
				/* samples/4 */
	int	png_amp[MBF_EM300RAW_MAXBEAMS];		
				/* 0.5 dB */
	int	png_beam_num[MBF_EM300RAW_MAXBEAMS];	
	...
	int	png_beam_index[MBF_EM300RAW_MAXBEAMS];	
				/* beam index number */
	int	png_sort_direction[MBF_EM300RAW_MAXBEAMS];	
				/* sorting direction - first sample in beam has lowest
				    range if 1, highest if -1. */
	int	png_beam_samples[MBF_EM300RAW_MAXBEAMS];	
				/* number of sidescan samples derived from
					each beam */
	int	png_start_sample[MBF_EM300RAW_MAXBEAMS];	
				/* start sample number */
	int	png_center_sample[MBF_EM300RAW_MAXBEAMS];	
				/* center sample number */
	mb_s_char png_ssraw[MBF_EM300RAW_MAXRAWPIXELS];
	...
	int	att_time[MBF_EM300RAW_MAXATTITUDE];
				/* time since record start (msec) */
	int	att_sensor_status[MBF_EM300RAW_MAXATTITUDE];
				/* see note 12 above */
	int	att_roll[MBF_EM300RAW_MAXATTITUDE];
				/* roll (0.01 degree) */
	int	att_pitch[MBF_EM300RAW_MAXATTITUDE];
				/* pitch (0.01 degree) */
	int	att_heave[MBF_EM300RAW_MAXATTITUDE];
				/* heave (cm) */
	int	att_heading[MBF_EM300RAW_MAXATTITUDE];
				/* heading (0.01 degree) */
	...
	int	hed_time[MBF_EM300RAW_MAXHEADING];
				/* time since record start (msec) */
	int	hed_heading[MBF_EM300RAW_MAXHEADING];
				/* heading (0.01 degree) */
