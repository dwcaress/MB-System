 ...
 *    $Id: mbeditviz_prog.c 1836 2010-02-28 00:48:05Z caress $
 *
...
/* id variables */
static char rcs_id[] = "$Id: mbeditviz_prog.c 1836 2010-02-28 00:48:05Z caress $";
	...
	for (ifile=0;ifile<mbev_num_files;ifile++)
		...
		if (file->load_status == MB_YES)
			...
			for (iping=0;iping<file->num_pings;iping++)
				...
				for (ibeam=0;ibeam<ping->beams_bath;ibeam++)
					{
fprintf(stderr,"%d:%d:%d flag:%d",ifile,iping,ibeam,ping->beamflag[ibeam]);
					if (mb_beam_ok(ping->beamflag[ibeam]))
						{
						mbeditviz_grid_beam(file, ping, ibeam, MB_YES, MB_NO);
fprintf(stderr," gridded");
						}
fprintf(stderr,"\n");
					}
				}
