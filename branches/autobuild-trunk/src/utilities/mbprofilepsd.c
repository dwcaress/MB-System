 ...
 *--------------------------------------------------------------------*/
/*
 * mbsegypsd calculates the power spectral density (PSD) function of a bathymetry
 * profile extracted using MBgrdviz. The user specifies the characteristic
 * length, sample spacing, and offset to use. The program calculates a number
 * of PSDs, each using a profile segment of the specified length which starts
 * offset distance from the previous segment. All data are resampled using linear
 * interpolation to the specified sample spacing. Each segment has a Hanning (cosine)
 * taper applied. The PSDs are output as a GMT grid file with segment start 
 * distance along the x axis and frequency along the y axis.
 *
 * Author:	D. W. Caress
 * Date:	February 19, 2010
 *              R/V Zephyr
 *              MBARI Mapping AUV Surveys on Davidson Seamount
 *
 * $Log: mbsegypsd.c,v $
 *
 */
...
char program_name[] = "mbprofilepsd";
char help_message[] =  "mbprofilepsd calculates the power spectral density (PSD) function of a bathymetry \nprofile extracted using MBgrdviz as a function of distance along the profile. The 2D PSD is output as \na GMT grid file with segment start distance along the x axis and frequency along the y axis.";
char usage_message[] = "mbprofilepsd -Dsampleinterval -Ifile -Lsegmentlength  \n\
          -Msegmentoffset -Oroot [-H -V]";
...
int main (int argc, char **argv)
{
	...
	/* profile data */
	char	profilefile[MB_PATH_MAXLINE];
	FILE	*fp;
	int	nprofile = 0;
	float	*xprofile = NULL;
	float	*zprofile = NULL;
	int	nprofileresampled = 0;
	double	*spsd = NULL;
	double	*wpsd = NULL;
	double	*spsdtot = NULL;
	double	*wpsdtot = NULL;
...
char usage_message[] = "mbprofilepsd -Dsampleinterval -Ifile -Lsegmentlength  \n\
	...
	while ((c = getopt(argc, argv, "D:d:I:i:L:l:M:m:O:o:VvHh")) != -1)
	  switch (c) 
		{
		case 'H':
		case 'h':
		...
		case 'D':
		case 'd':
			n = sscanf (optarg,"%lf", &sampleinterval);
			flag++;
			break;
		case 'I':
		case 'i':
		...
		case 'L':
		case 'l':
			n = sscanf (optarg,"%lf", &segmentlength);
			flag++;
			break;
		case 'M':
		case 'm':
			n = sscanf (optarg,"%lf", &segmentoffset);
			flag++;
			break;
		case 'O':
		case 'o':
	...
	if (verbose >= 2)
		...
		fprintf(outfp,"dbg2       sampleinterval: %f\n",sampleinterval);
		fprintf(outfp,"dbg2       segmentoffset:  %f\n",offset);
