...
int mbr_rt_nvnetcdf(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (*dataread == MB_NO) {
		*dataread = MB_YES;
		...
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_inq_dimid mbPositionNbr error: %s\n", nc_strerror(nc_status));
		nc_status = nc_inq_dimlen(mb_io_ptr->ncid, dim_id, &store->mbPositionNbr);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_inq_dimlen mbPositionNbr error: %s\n", nc_strerror(nc_status));
		...
		if (status == MB_SUCCESS) {
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_att mbPointCounter error: %s\n", nc_strerror(nc_status));
			if (nc_status != NC_NOERR) {
				status = MB_FAILURE;
				*error = MB_ERROR_EOF;
			}
		...
		if (status == MB_SUCCESS) {
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_inq_varid mbAltitude_id error: %s\n", nc_strerror(nc_status));
			nc_status = nc_inq_varid(mb_io_ptr->ncid, "mbImmersion", &store->mbImmersion_id);
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_inq_varid mbImmersion_id error: %s\n", nc_strerror(nc_status));
		...
		if (status == MB_SUCCESS) {
			...
			if (store->mbAltitude_id >= 0) {
				...
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_type error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "long_name", store->mbAltitude_long_name);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_long_name error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "name_code", store->mbAltitude_name_code);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_name_code error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "units", store->mbAltitude_units);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_units error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "unit_code", store->mbAltitude_unit_code);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_unit_code error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_double(mb_io_ptr->ncid, store->mbAltitude_id, "add_offset", &store->mbAltitude_add_offset);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_add_offset error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_double(mb_io_ptr->ncid, store->mbAltitude_id, "scale_factor", &store->mbAltitude_scale_factor);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_scale_factor error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_int(mb_io_ptr->ncid, store->mbAltitude_id, "minimum", &store->mbAltitude_minimum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_minimum error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_int(mb_io_ptr->ncid, store->mbAltitude_id, "maximum", &store->mbAltitude_maximum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_maximum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbAltitude_id, "valid_minimum", &store->mbAltitude_valid_minimum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_valid_minimum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbAltitude_id, "valid_maximum", &store->mbAltitude_valid_maximum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_valid_maximum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbAltitude_id, "missing_value", &store->mbAltitude_missing_value);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_missing_value error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "format_C", store->mbAltitude_format_C);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_format_C error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbAltitude_id, "orientation", store->mbAltitude_orientation);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbAltitude_orientation error: %s\n", nc_strerror(nc_status));
			}
			if (store->mbImmersion_id >= 0) {
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "type", store->mbImmersion_type);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_type error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "long_name", store->mbImmersion_long_name);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_long_name error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "name_code", store->mbImmersion_name_code);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_name_code error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "units", store->mbImmersion_units);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_units error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "unit_code", store->mbImmersion_unit_code);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_unit_code error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_double(mb_io_ptr->ncid, store->mbImmersion_id, "add_offset", &store->mbImmersion_add_offset);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_add_offset error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_double(mb_io_ptr->ncid, store->mbImmersion_id, "scale_factor", &store->mbImmersion_scale_factor);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_scale_factor error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_int(mb_io_ptr->ncid, store->mbImmersion_id, "minimum", &store->mbImmersion_minimum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_minimum error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_int(mb_io_ptr->ncid, store->mbImmersion_id, "maximum", &store->mbImmersion_maximum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_maximum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbImmersion_id, "valid_minimum", &store->mbImmersion_valid_minimum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_valid_minimum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbImmersion_id, "valid_maximum", &store->mbImmersion_valid_maximum);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_valid_maximum error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_int(mb_io_ptr->ncid, store->mbImmersion_id, "missing_value", &store->mbImmersion_missing_value);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_missing_value error: %s\n", nc_strerror(nc_status));
				nc_status = nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "format_C", store->mbImmersion_format_C);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_format_C error: %s\n", nc_strerror(nc_status));
				nc_status =
				    nc_get_att_text(mb_io_ptr->ncid, store->mbImmersion_id, "orientation", store->mbImmersion_orientation);
				if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
					fprintf(stderr, "nc_get_att mbImmersion_orientation error: %s\n", nc_strerror(nc_status));
			}
			...
			if (verbose >= 2) {
				...
				for (i = 0; i < store->mbNbrHistoryRec; i++) {
					...
					fprintf(stderr, "dbg2       mbHistAutor[%2d]:         %s\n", i,
					        &(store->mbHistAutor[i * store->mbNameLength]));
					fprintf(stderr, "dbg2       mbHistModule[%2d]:        %s\n", i,
					        &(store->mbHistModule[i * store->mbNameLength]));
	...
	if (status == MB_SUCCESS && store->mbNbrHistoryRec > *commentread) {
		...
		/* get next comment */
		strncpy(store->comment, &(store->mbHistComment[(*commentread) * store->mbCommentLength]), MBSYS_NAVNETCDF_COMMENTLEN);
	...
	/* next run through all survey records */
		...
		if (store->mbAltitude_id >= 0) {
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbAltitude error: %s\n", nc_strerror(nc_status));
		}
		if (store->mbImmersion_id >= 0) {
			index[0] = *recread;
			count[0] = 1;
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbImmersion error: %s\n", nc_strerror(nc_status));
		}
		if (store->mbHeading_id >= 0) {
			index[0] = *recread;
			count[0] = 1;
		...
		if (store->mbSpeed_id >= 0) {
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbSpeed error: %s\n", nc_strerror(nc_status));
		}
		if (store->mbPType_id >= 0) {
			index[0] = *recread;
			count[0] = 1;
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbPType error: %s\n", nc_strerror(nc_status));
		}
		if (store->mbPQuality_id >= 0) {
			index[0] = *recread;
			count[0] = 1;
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbPQuality error: %s\n", nc_strerror(nc_status));
		}
		if (store->mbPFlag_id >= 0) {
			index[0] = *recread;
			count[0] = 1;
			...
			if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
				fprintf(stderr, "nc_get_vara mbPFlag error: %s\n", nc_strerror(nc_status));
		}
		/* check status */
		if (nc_status != NC_NOERR) {
			status = MB_FAILURE;
...
int mbr_wt_nvnetcdf(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (store->kind == MB_DATA_DATA && *recwrite == 0 && status == MB_SUCCESS) {
		...
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_def_var mbAltitude_id error: %s\n", nc_strerror(nc_status));
		dims[0] = mbPositionNbr_id;
		nc_status = nc_def_var(mb_io_ptr->ncid, "mbImmersion", NC_SHORT, 1, dims, &storelocal->mbImmersion_id);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_def_var mbImmersion_id error: %s\n", nc_strerror(nc_status));
		dims[0] = mbPositionNbr_id;
		...
		nc_status =
		    nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbTimeReference", MBSYS_NAVNETCDF_ATTRIBUTELEN, store->mbTimeReference);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status =
		    nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbMeridian180", MBSYS_NAVNETCDF_ATTRIBUTELEN, store->mbMeridian180);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbMeridian180 error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbGeoDictionnary", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            store->mbGeoDictionnary);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbGeoDictionnary error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbGeoRepresentation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            store->mbGeoRepresentation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbGeoRepresentation error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbGeodesicSystem", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            store->mbGeodesicSystem);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbGeodesicSystem error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbEllipsoidName", MBSYS_NAVNETCDF_COMMENTLEN, store->mbEllipsoidName);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, NC_GLOBAL, "mbProjParameterCode", MBSYS_NAVNETCDF_COMMENTLEN,
		                            store->mbProjParameterCode);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistDate_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistDate_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistTime_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistTime_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistCode_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistCode_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistAutor_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistAutor_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistAutor_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistAutor_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistAutor_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistAutor_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistModule_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistModule_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistModule_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistModule_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistModule_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistModule_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistComment_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistComment_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistComment_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistComment_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHistComment_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHistComment_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbDate_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbDate_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbTime_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbTime_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "minimum", NC_INT, 1, &storelocal->mbOrdinate_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbOrdinate_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "maximum", NC_INT, 1, &storelocal->mbOrdinate_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbOrdinate_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbOrdinate_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "minimum", NC_INT, 1, &storelocal->mbAbscissa_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAbscissa_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "maximum", NC_INT, 1, &storelocal->mbAbscissa_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAbscissa_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAbscissa_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbAltitude_id, "add_offset", NC_DOUBLE, 1,
		                              &storelocal->mbAltitude_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbAltitude_id, "scale_factor", NC_DOUBLE, 1,
		                              &storelocal->mbAltitude_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAltitude_id, "minimum", NC_INT, 1, &storelocal->mbAltitude_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAltitude_id, "maximum", NC_INT, 1, &storelocal->mbAltitude_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAltitude_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbAltitude_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAltitude_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbAltitude_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbAltitude_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbAltitude_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbAltitude_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbAltitude_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbAltitude_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbImmersion_id, "add_offset", NC_DOUBLE, 1,
		                              &storelocal->mbImmersion_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbImmersion_id, "scale_factor", NC_DOUBLE, 1,
		                              &storelocal->mbImmersion_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbImmersion_id, "minimum", NC_INT, 1, &storelocal->mbImmersion_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbImmersion_id, "maximum", NC_INT, 1, &storelocal->mbImmersion_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbImmersion_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbImmersion_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbImmersion_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbImmersion_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbImmersion_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbImmersion_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbImmersion_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbImmersion_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbImmersion_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		...
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbHeading_id, "minimum", NC_INT, 1, &storelocal->mbHeading_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbHeading_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbHeading_id, "maximum", NC_INT, 1, &storelocal->mbHeading_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
		...
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbHeading_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbHeading_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbSpeed_id, "add_offset", NC_DOUBLE, 1,
		                              &storelocal->mbSpeed_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_double(mb_io_ptr->ncid, storelocal->mbSpeed_id, "scale_factor", NC_DOUBLE, 1,
		                              &storelocal->mbSpeed_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbSpeed_id, "minimum", NC_INT, 1, &storelocal->mbSpeed_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbSpeed_id, "maximum", NC_INT, 1, &storelocal->mbSpeed_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbSpeed_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbSpeed_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbSpeed_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbSpeed_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbSpeed_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbSpeed_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbSpeed_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbSpeed_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbSpeed_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "add_offset", NC_INT, 1, &storelocal->mbPType_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "scale_factor", NC_INT, 1, &storelocal->mbPType_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "minimum", NC_INT, 1, &storelocal->mbPType_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "maximum", NC_INT, 1, &storelocal->mbPType_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbPType_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbPType_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPType_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbPType_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPType_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPType_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPType_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "add_offset", NC_INT, 1,
		                           &storelocal->mbPQuality_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "scale_factor", NC_INT, 1,
		                           &storelocal->mbPQuality_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "minimum", NC_INT, 1, &storelocal->mbPQuality_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_minimum error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "maximum", NC_INT, 1, &storelocal->mbPQuality_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbPQuality_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbPQuality_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPQuality_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbPQuality_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPQuality_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPQuality_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPQuality_orientation);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "type", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_type);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "long_name", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_long_name);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "name_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_name_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "units", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_units);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "unit_code", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_unit_code);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "add_offset", NC_INT, 1, &storelocal->mbPFlag_add_offset);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_add_offset error: %s\n", nc_strerror(nc_status));
		nc_status =
		    nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "scale_factor", NC_INT, 1, &storelocal->mbPFlag_scale_factor);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_scale_factor error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "minimum", NC_INT, 1, &storelocal->mbPFlag_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "maximum", NC_INT, 1, &storelocal->mbPFlag_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "valid_minimum", NC_INT, 1,
		                           &storelocal->mbPFlag_valid_minimum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_valid_minimum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "valid_maximum", NC_INT, 1,
		                           &storelocal->mbPFlag_valid_maximum);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_valid_maximum error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_int(mb_io_ptr->ncid, storelocal->mbPFlag_id, "missing_value", NC_INT, 1,
		                           &storelocal->mbPFlag_missing_value);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att mbPFlag_missing_value error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "format_C", MBSYS_NAVNETCDF_ATTRIBUTELEN,
		                            storelocal->mbPFlag_format_C);
		if ((verbose >= 2 || nc_verbose >= 1) && nc_status != NC_NOERR)
			fprintf(stderr, "nc_put_att JJJJ error: %s\n", nc_strerror(nc_status));
		nc_status = nc_put_att_text(mb_io_ptr->ncid, storelocal->mbPFlag_id, "orientation", MBSYS_NAVNETCDF_ATTRIBUTELEN,
	...
	/* now write the data */
	if (store->kind == MB_DATA_DATA && status == MB_SUCCESS) {
