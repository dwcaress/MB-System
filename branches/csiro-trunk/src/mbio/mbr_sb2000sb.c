...
int mbr_info_sb2000sb(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = 0;
	...
	strncpy(format_description,
	        ...
	        " SeaBeam 2000, bathymetry, 121 beams, \n                      binary,  SIO.\n",
...
int mbr_rt_sb2000sb(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		if ((strncmp(store->sensor_type, "S2", 2) != 0 || strncmp(store->data_type, "SR", 2) != 0) &&
			...
			*error = MB_ERROR_UNINTELLIGIBLE;
			...
			/* read rest of record */
			if ((read_status = fread(buffer, 1, store->sensor_size, mb_io_ptr->mbfp)) != store->sensor_size) {
				status = MB_FAILURE;
				*error = MB_ERROR_EOF;
			}
			mb_io_ptr->file_bytes += read_status;
			if ((read_status = fread(buffer, 1, store->data_size, mb_io_ptr->mbfp)) != store->data_size) {
				status = MB_FAILURE;
				*error = MB_ERROR_EOF;
			}
			mb_io_ptr->file_bytes += read_status;
