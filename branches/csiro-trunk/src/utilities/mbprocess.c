...
int get_corrtable(int verbose, double time_d, int ncorrtable, int ncorrangle, struct mbprocess_sscorr_struct *corrtable,
				  struct mbprocess_sscorr_struct *refcorrtable, struct mbprocess_sscorr_struct *corrtableuse, int *error);
	...
	while (read_data == MB_YES) {
		...
		if (process.mbp_sap_use == MBP_SAP_USE_CONST)
			...
			if (process.mbp_sap_use == MBP_SAP_USE_PROFILE) {
				...
				for (i=1;i<nsap;i++) {
					absorption_sum[i] = absorption_sum[i-1] + 0.0005*(absorption[i-1] + absorption[i]) *(abs_depth[i] - abs_depth[i-1]);
				}
			}
			...
			if (process.mbp_sscorr_mode == MBP_SSCORR_ON || process.mbp_ampcorr_mode == MBP_AMPCORR_ON ||
				process.mbp_static_mode == MBP_STATIC_ANGLE_ON || process.mbp_ampcorr_area == MB_YES || process.mbp_sscorr_area == MB_YES) {
				if (error == MB_ERROR_NO_ERROR)
					status =
			...
			while (error <= MB_ERROR_NO_ERROR) {
				...
				if (process.mbp_sap_mode == MBP_SAP_ON && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA) {
					...
					if (process.mbp_sap_use == MBP_SAP_USE_PROFILE) {
						...
						abs_at_bathy = absorption[i-1] + (absorption[i] - absorption[i-1]) * (bathy - abs_depth[i-1]) / (abs_depth[i] - abs_depth[i-1]);
						
						abs = absorption_sum[i-1] + 0.0005*(absorption[i-1] + abs_at_bathy) * (bathy - abs_depth[i-1]);
				...
				if (((process.mbp_ampcorr_mode == MBP_AMPCORR_ON || process.mbp_ampcorr_area == MB_YES) && (process.mbp_ampcorr_slope == MBP_AMPCORR_IGNORESLOPE || process.mbp_ampcorr_slope == MBP_AMPCORR_USESLOPE)) || ((process.mbp_sscorr_mode == MBP_SSCORR_ON|| process.mbp_sscorr_area == MB_YES) &&
					...
					if (error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA && (process.mbp_ampcorr_area == MB_YES || process.mbp_sscorr_area == MB_YES)) {
						status = get_footprint_characteristics(verbose, imbio_ptr, &rx_beamwidth, &tx_beamwidth, &soundspeed, &pulselength, &nareas, grazing, ranges, &error);
					}

					/* correct the amplitude if desired */
					if ((process.mbp_ampcorr_mode == MBP_AMPCORR_ON || process.mbp_ampcorr_area == MB_YES) && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA) {
						
						if (process.mbp_ampcorr_mode == MBP_AMPCORR_ON && nampcorrtable > 0 && nampcorrangle > 0) {
							/* calculate the correction table */
							status = get_corrtable(verbose, time_d, nampcorrtable, nampcorrangle, ampcorrtable, &ampcorrreftable, &ampcorrtableuse, &error);
							...
							if (process.mbp_ampcorr_stddev == MBP_AMPCORR_NORMALISESTD) {
								status = get_anglecorr(verbose, ampcorrtableuse.nangle, ampcorrtableuse.angle, ampcorrtableuse.sigma,
													(-process.mbp_ampcorr_angle), &reference_sigma_port, &error);
								status = get_anglecorr(verbose, ampcorrtableuse.nangle, ampcorrtableuse.angle, ampcorrtableuse.sigma,
													process.mbp_ampcorr_angle, &reference_sigma_stbd, &error);
							...
							if (mb_beam_ok(beamflag[i])) {
								...
								if (bathy > 0.0) {
									...
									if (process.mbp_ampcorr_area == MB_YES) {
										status = get_area(verbose, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, grazing[i], 0, ranges[i], &acq_area, &error);
										status = get_area(verbose, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, angle, 0, ranges[i], &cor_area, &error);
										
										if (acq_area > 0 && cor_area > 0)
										{
											if (verbose >= 1) {
												fprintf(stderr,"area: %d %d  %.1f  %.1f  %.0f  %f  %.2f  %.2f  %.2f  %.0f  %f  %f  %f  %f\n",
														idata, i, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, angle - RTD * atan(slope), grazing[i], angle, ranges[i], acq_area, cor_area, 10 * log10(acq_area / cor_area), depths[i]);
											}
											...
											if (verbose >= 1) {
												fprintf(stderr,"badarea: %d  %d  %.1f  %.1f  %.0f  %f  %.2f  %.2f  %.2f  %.0f  %f  %f\n",
														idata, i, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, angle - RTD * atan(slope), grazing[i], angle, ranges[i], acq_area, cor_area);
											}
									...
									if (process.mbp_ampcorr_stddev == MBP_AMPCORR_IGNORESTD)
										...
										status = get_anglecorr(verbose, ampcorrtableuse.nangle, ampcorrtableuse.angle, ampcorrtableuse.sigma, angle, &sigma, &error);
										if (sigma == 0)
											sigma = 1;
									}
					...
					if (process.mbp_sscorr_mode == MBP_SSCORR_ON && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA &&
						...
						/* calculate the correction table */
						status = get_corrtable(verbose, time_d, nsscorrtable, nsscorrangle, sscorrtable, &ampcorrreftable, &sscorrtableuse, &error);
				...
				else if (((process.mbp_ampcorr_mode == MBP_AMPCORR_ON || process.mbp_ampcorr_area == MB_YES) &&
				          (process.mbp_ampcorr_slope == MBP_AMPCORR_USETOPO ||
				           process.mbp_ampcorr_slope == MBP_AMPCORR_USETOPOSLOPE)) ||
				           ((process.mbp_sscorr_mode == MBP_SSCORR_ON || process.mbp_sscorr_area == MB_YES) && (process.mbp_sscorr_slope == MBP_SSCORR_USETOPO ||
						...
						for (i = 0; i < namp; i++) {
							if (mb_beam_ok(beamflag[i])) {
								...
								if (ix > 0 && ix < grid.n_columns - 1 && jy > 0 && jy < grid.n_rows - 1 &&
									...
									r[0] =  headingy * (bathacrosstrack[i] - sonar_acrosstrack) + headingx * (bathalongtrack[i] - sonar_alongtrack);
									r[1] =  -headingx * (bathacrosstrack[i] - sonar_acrosstrack) + headingy * (bathalongtrack[i] - sonar_alongtrack);
									...
									if (process.mbp_ampcorr_area == MB_YES) {
										...
										/* fprintf(stderr, "grid (sw,nw,se,ne): %f %f %f %f  ne: %f %f %f  xg: %f %f %f\n", 
												grid.data[kgrid00], grid.data[kgrid01], grid.data[kgrid10], grid.data[kgrid11], v1[0], v1[1], v[2], v[0], v[1], v[2]); */
								...
								if (process.mbp_ampcorr_area == MB_YES) {
									...
									status = get_area(verbose, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, grazing[i], 0, ranges[i], &acq_area, &error);
									status = get_area(verbose, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, across_angle, along_angle, ranges[i], &cor_area, &error);
									
									if (acq_area > 0 && cor_area > 0) {
										if (verbose >= 1) {
											fprintf(stderr,"area: %d  %d  %.1f  %.1f  %.0f  %f  %.2f  %.2f  %.2f  %.2f  %.0f  %f  %f  %f  %f\n",
													idata, i, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, angle, grazing[i], across_angle, along_angle, ranges[i], acq_area, cor_area, 10 * log10(acq_area / cor_area), depths[i]);
										}
										amp[i] = amp[i] + 10 * log10(acq_area / cor_area);
									}
									else {
										if (verbose >= 1) {
											fprintf(stderr,"badarea: %d  %d  %.1f  %.1f  %.0f  %f  %.2f  %.2f  %.2f  %.2f  %.0f  %f  %f \n",
													idata, i, rx_beamwidth, tx_beamwidth, soundspeed, pulselength, angle, grazing[i], across_angle, along_angle, ranges[i], acq_area, cor_area);
										}
									}
								}
						...
						for (i = 0; i < nss; i++) {
							if (ss[i] > MB_SIDESCAN_NULL) {
								...
								if (ix > 0 && ix < grid.n_columns - 1 && jy > 0 && jy < grid.n_rows - 1 &&
									...
									/* get look vector for data */
									bathy = -grid.data[kgrid];
									r[0] =  headingy * (ssacrosstrack[i] - sonar_acrosstrack) + headingx * (ssalongtrack[i] - sonar_alongtrack);
									r[1] =  -headingx * (ssacrosstrack[i] - sonar_acrosstrack) + headingy * (ssalongtrack[i] - sonar_alongtrack);
									r[2] = grid.data[kgrid] + sonardepth;
									rr = -sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);
									r[0] /= rr;
									r[1] /= rr;
...
int get_corrtable(int verbose, double time_d, int ncorrtable, int ncorrangle, struct mbprocess_sscorr_struct *corrtable,
	...
	if (ncorrtable == 1 || time_d <= corrtable[0].time_d) {
		...
		corrtableuse->nangle = MIN(corrtable[itable].nangle, corrtable[itable + 1].nangle);
...
int set_absorption_simrad2(int verbose, void *mbio_ptr, double absorption, int* error) {
	...
	if (store_ptr->sonar == MBSYS_SIMRAD2_EM12S || store_ptr->sonar == MBSYS_SIMRAD2_EM12D || store_ptr->sonar == MBSYS_SIMRAD2_EM121 || store_ptr->sonar == MBSYS_SIMRAD2_EM1000) {
 ...
 */
int get_footprint_characteristics(int verbose, void *mbio_ptr, double* xbeamwidth, double* ybeamwidth, double* soundspeed, double* pulselength, 
								  int* nangles, double* incident, double* range, int* error) {
    ...
    case MBF_EM300MBA:
    case MBF_EM300RAW:
        status = get_footprint_characteristics_simrad2(verbose, mbio_ptr, xbeamwidth, ybeamwidth, soundspeed, pulselength,
...
/*--------------------------------------------------------------------*/
int get_footprint_characteristics_simrad2(int verbose, void *mbio_ptr, double* xbeamwidth, double* ybeamwidth, double* soundspeed, double* pulselength, 
										  int* nangles, double* incident, double* range, int* error) {
	char *function_name = "get_absorption_simrad2";
	int status = MB_SUCCESS;
	int i, j;
