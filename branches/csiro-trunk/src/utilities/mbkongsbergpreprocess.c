...
int main (int argc, char **argv)
{
	...
	int	nrectot_0x30_parameter_stop = 0;
	int	nrectot_0x31_parameter_off = 0;
	int	nrectot_0x32_parameter_on = 0;
	int	nrectot_0x33_parameter_extra = 0;
	int	nrectot_0x41_attitude = 0;
	int	nrectot_0x43_clock = 0;
	int	nrectot_0x44_bathymetry = 0;
	int	nrectot_0x45_singlebeam = 0;
	int	nrectot_0x46_rawbeamF = 0;
	int	nrectot_0x47_surfacesoundspeed2 = 0;
	int	nrectot_0x48_heading = 0;
	int	nrectot_0x49_parameter_start = 0;
	int	nrectot_0x4A_tilt = 0;
	int	nrectot_0x4B_echogram = 0;
	int	nrectot_0x4E_rawbeamN = 0;
	int	nrectot_0x50_pos = 0;
	int	nrectot_0x52_runtime = 0;
	int	nrectot_0x53_sidescan = 0;
	int	nrectot_0x54_tide = 0;
	int	nrectot_0x55_svp2 = 0;
	int	nrectot_0x56_svp = 0;
	int	nrectot_0x57_surfacesoundspeed = 0;
	int	nrectot_0x58_bathymetry2 = 0;
	int	nrectot_0x59_sidescan2 = 0;
	int	nrectot_0x66_rawbeamf = 0;
	int	nrectot_0x68_height = 0;
	int	nrectot_0x69_parameter_stop = 0;
	int	nrectot_0x6B_water_column = 0;
	int	nrectot_0x6E_network_attitude = 0;
	int	nrectot_0x70_parameter = 0;
	int	nrectot_0x73_surface_sound_speed = 0;
	int	nrectot_0xE1_bathymetry_mbari57 = 0;
	int	nrectot_0xE2_sidescan_mbari57 = 0;
	int	nrectot_0xE3_bathymetry_mbari59 = 0;
	int	nrectot_0xE4_sidescan_mbari59 = 0;
	int	nrectot_0xE5_bathymetry_mbari59 = 0;
		...
		if (status == MB_SUCCESS
			...
			if (istore->kind == nav_source
				...
				if (ndat_nav == 0 || dat_nav_time_d[ndat_nav-1] < time_d)
					...
					dat_nav_lon[ndat_nav] = (double)(0.0000001 * store->pos_longitude);
					dat_nav_lat[ndat_nav] = (double)(0.00000005 * store->pos_latitude);
					ndat_nav++;
					}
				}
			...
			if (istore->kind == heading_source
				...
				if (ndat_heading == 0 || dat_heading_time_d[ndat_heading-1] < time_d)
					...
					dat_heading_heading[ndat_heading] = (double)(0.01 * store->pos_heading);
					ndat_heading++;
					}
				}
		...
		if (status == MB_SUCCESS && kind == MB_DATA_ALTITUDE)
			...
			if (device->DV2_enabled == MB_YES)
				...
				if (ndat_altitude + 1 >= ndat_altitude_alloc)
					{
					ndat_altitude_alloc +=  MBKONSBERGPREPROCESS_ALLOC_CHUNK;
...
/*fprintf(stderr,"READ FAILURE: status:%d error:%d kind:%d\n",status,error,kind);*/
	...
	/* apply time lag to all relevant data
		timelag value calculated either from model imported from file (timelagmode == MBKONSBERGPREPROCESS_TIMELAG_MODEL)
			or by a constant offset (timelagmode == MBKONSBERGPREPROCESS_TIMELAG_CONSTANT) */
	if (timelagmode != MBKONSBERGPREPROCESS_TIMELAG_OFF)
		{
...
fprintf(stderr,"Applying timelag to %d sonardepth nav data\n", nsonardepth);
	...
	if (nnav > 0 && (verbose > 0 || mode == MBKONSBERGPREPROCESS_TIMESTAMPLIST))
		fprintf(stdout, "\nTotal navigation data read: %d\n", nnav);
	if (nnav > 0 && (mode == MBKONSBERGPREPROCESS_TIMESTAMPLIST))
		{
		fprintf(stdout, "\nTotal navigation data read: %d\n", nnav);
		for (i=0;i<nnav;i++)
			{
	...
	if (nsonardepth > 0 && (mode == MBKONSBERGPREPROCESS_TIMESTAMPLIST))
		{
		fprintf(stdout, "\nTotal sonardepth data read: %d\n", nsonardepth);
		for (i=0;i<nnav;i++)
			{
