...
# (5.5.2283) on new installations of Ubuntu 14.04 and 16.04 in early October
# 2016. The methodology is essentially the same for both versions of
...
# MB-System:
# ----------
# These steps assume you have downloaded a tar.gz file from the website or
# ftp site - the current distribution file is mbsystem-5.5.2283.tar.gz 
...
# root privileges using sudo is necessary for all steps:
        sudo tar xvzf mbsystem-5.5.2283.tar.gz
# and then cd into the resulting directory:
        cd mbsystem-5.5.2283
#
...
# required:
       sudo ./configure
# 
 # If any prerequisites packages are missing, configure will fail with a
# error message indicating what is missing.
# 
# If you want to install into another location, then specify that with
# the --prefix option, for instance:
        sudo ./configure --prefix=/home/caress/sandbox/mbsystem
# 
# Once the makefiles have been generated by configure, build and install using:
        sudo make
        sudo make install
...
#------------------------------------------------------------------------------
#
# Finally there are some user enviroment setting that are required to use some
