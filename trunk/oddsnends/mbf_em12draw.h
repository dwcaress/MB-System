 ...
 *--------------------------------------------------------------------*/
/*
 * mbf_em12draw.h defines the data structures used by MBIO functions
 * to store multibeam data read from the MBF_EM12DRAW format (MBIO id 51).  
 *
 * Author:	D. W. Caress
 * Date:	August 8, 1994
...
#define	MBF_EM12DRAW_MAXBEAMS	81
#define	MBF_EM12DRAW_MAXPIXELS	50*MBF_EM12DRAW_MAXBEAMS

struct mbf_em12draw_struct
	{
	/* type of data record */
	...
	short int bath[MBF_EM12DRAW_MAXBEAMS];	
				/* depths:
					EM-1000:        0.02 meters 
					EM-12 high res: 0.10 meters 
					EM-12 low res:  0.20 meters */
	short int bath_acrosstrack[MBF_EM12DRAW_MAXBEAMS];
				/* acrosstrack distances:
					EM-1000:         0.1 meters 
					EM-12 high res:  0.2 meters 
					EM-12 low res:   0.5 meters */
	short int bath_alongtrack[MBF_EM12DRAW_MAXBEAMS];
				/* alongtrack distances:
					EM-1000:         0.1 meters 
					EM-12 high res:  0.2 meters 
					EM-12 low res:   0.5 meters */
	short int tt[MBF_EM12DRAW_MAXBEAMS];	/* meters */
				/* travel times:
					EM-1000:         0.05 msec 
					EM-12 high res:  0.20 msec 
					EM-12 low res:   0.80 msec */
	signed char	amp[MBF_EM12DRAW_MAXBEAMS];	    /* 0.5 dB */
	unsigned char	quality[MBF_EM12DRAW_MAXBEAMS];	    /* meters */
	signed char	heave[MBF_EM12DRAW_MAXBEAMS];	    /* 0.1 meters */
	...
	short int beam_frequency[MBF_EM12DRAW_MAXBEAMS]; 
				/*	0 = 12.67 kHz
					1 = 13.00 kHz
					2 = 13.33 kHz
					3 = 95.00 kHz */
	short int beam_samples[MBF_EM12DRAW_MAXBEAMS];	
				/* number of sidescan samples derived from
					each beam */
	short int beam_center_sample[MBF_EM12DRAW_MAXBEAMS];
