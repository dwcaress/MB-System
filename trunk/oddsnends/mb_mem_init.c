 ...
 *    $Id: mb_mem_init.c,v 4.22 2000-10-11 01:02:30 caress Exp $
 *
...
/*--------------------------------------------------------------------*/
int mb_mem_init(int verbose, char *mbio_ptr, int *error)
{
  static char rcs_id[]="$Id: mb_mem_init.c,v 4.22 2000-10-11 01:02:30 caress Exp $";
	...
	if (mb_io_ptr->format == MBF_SBSIOMRG)
		...
		{
		status = mbr_alm_mbpronav(verbose,mbio_ptr,error);
		}
	else if (mb_io_ptr->format == MBF_ELMK2XSE)
		{
		status = mbr_alm_elmk2xse(verbose,mbio_ptr,error);
		}
	else
		{
		status = MB_FAILURE;
