 ...
 *	$Id: mbf_em1000rw.h,v 4.2 1996-08-05 15:25:43 caress Exp $
 *
 *    Copyright (c) 1994 by 
 ...
 *--------------------------------------------------------------------*/
/*
 * mbf_em1000rw.h defines the data structures used by MBIO functions
 * to store multibeam data read from the MBF_EM1000RW format (MBIO id 51).  
 *
 * Author:	D. W. Caress
 * Date:	August 8, 1994
 * $Log: not supported by cvs2svn $
 * Revision 4.1  1996/07/26  21:09:33  caress
...
struct mbf_em1000rw_struct
	...
	short int bath[MBF_EM1000RW_MAXBEAMS];	
				/* depths:
					EM-1000:        0.02 meters 
					EM-12 high res: 0.10 meters 
					EM-12 low res:  0.20 meters */
	short int bath_acrosstrack[MBF_EM1000RW_MAXBEAMS];
				/* acrosstrack distances:
					EM-1000:         0.1 meters 
					EM-12 high res:  0.2 meters 
					EM-12 low res:   0.5 meters */
	short int bath_alongtrack[MBF_EM1000RW_MAXBEAMS];
				/* alongtrack distances:
					EM-1000:         0.1 meters 
					EM-12 high res:  0.2 meters 
					EM-12 low res:   0.5 meters */
	short int tt[MBF_EM1000RW_MAXBEAMS];	/* meters */
				/* travel times:
					EM-1000:         0.05 msec 
	...
	short int beam_frequency[MBF_EM1000RW_MAXBEAMS]; 
				/*	0 = 12.67 kHz
					1 = 13.00 kHz
					2 = 13.33 kHz
					3 = 95.00 kHz */
	short int beam_samples[MBF_EM1000RW_MAXBEAMS];	
				/* number of sidescan samples derived from
					each beam */
	short int beam_center_sample[MBF_EM1000RW_MAXBEAMS];
