 ...
 *	$Id: mbr_dsl120pf.c,v 1.1 1996-08-26 17:24:56 caress Exp $
 *
 *    Copyright (c) 1996 by 
...
int	*error;
{
	static char res_id[]="$Id: mbr_dsl120pf.c,v 1.1 1996-08-26 17:24:56 caress Exp $";
...
int	*error;
{
	...
	if (verbose >= 5 && status == MB_SUCCESS)
		...
		for (i=0;i<data->amp_num_samp;i++)
			fprintf(stderr,"dbg5       amp[%d]:          %f\t%f\n", 
				data->amp_port[i], data->amp_stbd[i]);
		}
