 ...
 *--------------------------------------------------------------------*/
/*
 * mbsys_sb2120.h contains the functions for handling the data structure
 * used by MBIO functions to store swath sonar data in the SB2120 
 * XSE Data Exchange Format developed by L-3 Communications ELAC Nautik.
 * This format is used for data from SeaBeam 2120 multibeam sonars
...
/*--------------------------------------------------------------------*/
int mbsys_sb2120_alloc(verbose,mbio_ptr,store_ptr,error)
int	verbose;
char	*mbio_ptr;
char	**store_ptr;
int	*error;
{
 static char res_id[]="$Id: mbsys_sb2120.c,v 5.4 2009-03-02 18:51:52 caress Exp $";
	...
	status = mb_malloc(verbose,sizeof(struct mbsys_sb2120_struct),
				store_ptr,error);
	...
	for (i=0;i<MBSYS_SB2120_MAXSVP;i++)
	    {
	...
	for (i=0;i<MBSYS_SB2120_DESCRIPTION_LENGTH;i++)
	...
	for (i=0;i<MBSYS_SB2120_MAXBEAMS;i++)
	    {
	...
	for (i=0;i<MBSYS_SB2120_MAXPIXELS;i++)
	    store->ss[i] = 0; /* sidescan amplitude in dB */

	/* comment */
	for (i=0;i<MBSYS_SB2120_COMMENT_LENGTH;i++)
	    store->comment[i] = 0;
...
/*--------------------------------------------------------------------*/
int mbsys_sb2120_deall(verbose,mbio_ptr,store_ptr,error)
int	verbose;
char	*mbio_ptr;
char	**store_ptr;
int	*error;
{
	char	*function_name = "mbsys_sb2120_deall";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
...
/*--------------------------------------------------------------------*/
int mbsys_sb2120_extract(verbose,mbio_ptr,store_ptr,kind,
...
int	*error;
{
	char	*function_name = "mbsys_sb2120_extract";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	int	i, j;
...
{
	char	*function_name = "mbsys_sb2120_insert";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	double	maxoffset;
	...
	if (store->kind == MB_DATA_DATA)
		{
		/* get time */
		store->mul_sec = ((unsigned int) time_d) + MBSYS_SB2120_TIME_OFFSET;
		...
		{
		/* get time */
		store->nav_sec = ((unsigned int) time_d) + MBSYS_SB2120_TIME_OFFSET;
...
/*--------------------------------------------------------------------*/
int mbsys_sb2120_ttimes(verbose,mbio_ptr,store_ptr,kind,nbeams,
...
int	*error;
{
	char	*function_name = "mbsys_sb2120_ttimes";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	double	alpha, beta;
...
int mbsys_sb2120_extract_nav(int verbose, char *mbio_ptr, char *store_ptr,
		...
		int *error)
{
	char	*function_name = "mbsys_sb2120_extract_nav";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	int	i, j;
...
int mbsys_sb2120_insert_nav(int verbose, char *mbio_ptr, char *store_ptr,
		...
		int *error)
{
	char	*function_name = "mbsys_sb2120_insert_nav";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	int	kind;
...
/*--------------------------------------------------------------------*/
int mbsys_sb2120_copy(verbose,mbio_ptr,store_ptr,copy_ptr,error)
int	verbose;
char	*mbio_ptr;
char	*store_ptr;
char	*copy_ptr;
int	*error;
{
	char	*function_name = "mbsys_sb2120_copy";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_sb2120_struct *store;
	struct mbsys_sb2120_struct *copy;
	...
	store = (struct mbsys_sb2120_struct *) store_ptr;
	copy = (struct mbsys_sb2120_struct *) copy_ptr;
