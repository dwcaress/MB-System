 ...
 *	$Id: mbr_sb2100b1.c,v 5.7 2003-04-17 21:05:23 caress Exp $
 *
...
int mbr_dem_sb2100b1(int verbose, void *mbio_ptr, int *error);
int mbr_zero_sb2100b1(int verbose, char *data_ptr, int *error);
int mbr_rt_sb2100b1(int verbose, void *mbio_ptr, void *store_ptr, int *error);
int mbr_wt_sb2100b1(int verbose, void *mbio_ptr, void *store_ptr, int *error);
int mbr_sb2100b1_rd_data(int verbose, void *mbio_ptr, int *error);
...
int mbr_sb2100b1_rd_sr(int verbose, FILE *mbfp, 
		struct mbf_sb2100b1_struct *data, short record_length,
		int *error);
int mbr_sb2100b1_wr_data(int verbose, void *mbio_ptr, char *data_ptr, int *error);
...
{
	static char res_id[]="$Id: mbr_sb2100b1.c,v 5.7 2003-04-17 21:05:23 caress Exp $";
	...
	mb_io_ptr->mb_io_format_alloc = &mbr_alm_sb2100b1;
	mb_io_ptr->mb_io_format_free = &mbr_dem_sb2100b1; 
	mb_io_ptr->mb_io_store_alloc = &mbsys_sb2100_alloc; 
	mb_io_ptr->mb_io_store_free = &mbsys_sb2100_deall; 
	mb_io_ptr->mb_io_read_ping = &mbr_rt_sb2100b1; 
	mb_io_ptr->mb_io_write_ping = &mbr_wt_sb2100b1; 
...
int mbr_sb2100b1_wr_fh(int verbose, FILE *mbfp, int *error)
{
	...
	if (verbose >= 5)
		...
		fprintf(stderr,"dbg5       file_header_text: \n%s%s\n", 
			mbf_sb2100b1_file_header_text_1, 
			mbf_sb2100b1_file_header_text_2);
		}
	...
	if (status == MB_SUCCESS)
		{
		record_length = strlen(mbf_sb2100b1_file_header_text_1)
		    + strlen(mbf_sb2100b1_file_header_text_2);
	...
	if (status == MB_SUCCESS)
		{			
		/* write the data */
		if (fwrite(mbf_sb2100b1_file_header_text_1, 
			strlen(mbf_sb2100b1_file_header_text_1), 
			1, mbfp) != 1)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
		...
		if (fwrite(mbf_sb2100b1_file_header_text_2, 
			strlen(mbf_sb2100b1_file_header_text_2), 
			1, mbfp) != 1)
			{
			*error = MB_ERROR_WRITE_FAIL;
