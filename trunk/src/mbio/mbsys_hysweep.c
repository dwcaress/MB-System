...
int mbsys_hysweep_alloc(int verbose, void *mbio_ptr, void **store_ptr, int *error) {
	...
	/* HYSWEEP RMB - raw multibeam data
	    ...
	    depending on the bd (beamdata) field.
...
int mbsys_hysweep_deall(int verbose, void *mbio_ptr, void **store_ptr, int *error) {
	...
	if (store->RMB_beam_ranges != NULL)
		status =
		    mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_beam_ranges), error); /* beam ranges (survey units) */
	if (store->RMB_multi_ranges != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_multi_ranges),
		                  error); /* sounding point casting (survey units) (multi-transducer rangers) */
	if (store->RMB_sounding_eastings != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_eastings),
		                  error); /* easting positions of soundings */
	if (store->RMB_sounding_northings != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_northings),
		                  error); /* northing positions of soundings */
	if (store->RMB_sounding_depths != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_depths),
		                  error); /* corrected depths of soundings */
	if (store->RMB_sounding_across != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_across),
		                  error); /* acrosstrack positions of soundings */
	if (store->RMB_sounding_along != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_along),
		                  error); /* alongtrack positions of soundings */
	if (store->RMB_sounding_pitchangles != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_pitchangles),
		                  error); /* beam pitch angles of soundings (degrees, TSS convention) */
	if (store->RMB_sounding_rollangles != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_rollangles),
		                  error); /* beam roll angles of soundings (degrees, TSS convention) */
	if (store->RMB_sounding_takeoffangles != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_takeoffangles),
		                  error); /* beam takeoff angles of soundings (degrees from vertical) */
	if (store->RMB_sounding_azimuthalangles != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_azimuthalangles),
		                  error); /* beam azimuthal angles of soundings (degrees from forward) */
	if (store->RMB_sounding_timedelays != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_timedelays),
		                  error); /* beam delay times (milliseconds) */
	...
	if (store->RMB_sounding_quality != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RMB_sounding_quality),
	...
	if (store->RSS_starboard != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->RSS_starboard),
		                  error); /* starboard sidescan amplitude samples */
	if (store->MSS_table_altitude_sort != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->MSS_table_altitude_sort),
		                  error); /* sidescan working array - altitude = depth - draft + heave */
	if (store->MSS_table_range != NULL)
		status =
		    mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->MSS_table_range), error); /* sidescan working array - range */
	if (store->MSS_table_acrosstrack != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->MSS_table_acrosstrack),
		                  error); /* sidescan working array - acrosstrack */
	if (store->MSS_table_alongtrack != NULL)
		status = mb_freed(verbose, __FILE__, __LINE__, (void **)(&store->MSS_table_alongtrack),
		                  error); /* sidescan working array - alongtrack */
...
int mbsys_hysweep_extract_platform(int verbose, void *mbio_ptr, void *store_ptr, int *kind, void **platform_ptr, int *error) {
	...
	if (*platform_ptr != NULL) {
		...
		for (idevice = 0; idevice < store->num_devices; idevice++) {
			...
			if (device->DV2_device_capability & 0x0004 && device->DV2_device_capability & 0x0020 &&
			    device->DV2_device_capability & 0x0200) {
				sensor_type = MB_SENSOR_TYPE_INS;
				sensor_capability1 =
				    MB_SENSOR_CAPABILITY1_HEADING + MB_SENSOR_CAPABILITY1_ROLLPITCH + MB_SENSOR_CAPABILITY1_POSITION;
			...
			else if (device->DV2_device_capability & 0x0020 && device->DV2_device_capability & 0x0200) {
				sensor_type = MB_SENSOR_TYPE_IMU;
				sensor_capability1 = MB_SENSOR_CAPABILITY1_HEADING + MB_SENSOR_CAPABILITY1_ROLLPITCH;
				platform->source_heading = isensor;
				platform->source_rollpitch = isensor;
			...
			/* Position + heading */
			else if (device->DV2_device_capability & 0x0020 && device->DV2_device_capability & 0x0004) {
				sensor_type = MB_SENSOR_TYPE_POSITION;
				sensor_capability1 = MB_SENSOR_CAPABILITY1_HEADING + MB_SENSOR_CAPABILITY1_POSITION;
				platform->source_position = isensor;
				platform->source_heading = isensor;
			...
			/* set sensor */
			status = mb_platform_add_sensor(verbose, (void *)platform, sensor_type, sensor_name, NULL, NULL, sensor_capability1,
			                                sensor_capability2, num_offsets, 0, error);
			...
			if (status == MB_SUCCESS) {
				for (ioffset = 0; ioffset < num_offsets; ioffset++) {
					...
					if (ioffset == 0 && num_time_latency > 0) {
						status =
						    mb_platform_set_sensor_timelatency(verbose, (void *)platform, isensor, MB_SENSOR_TIME_LATENCY_STATIC,
						                                       (double)offset->OF2_offset_time, 0, NULL, NULL, error);
					}
				}
			}
		}
...
/*--------------------------------------------------------------------*/
int mbsys_hysweep_insert(int verbose, void *mbio_ptr, void *store_ptr, int kind, int time_i[7], double time_d, double navlon,
	...
	if (store->kind == MB_DATA_DATA) {
		...
		mb_proj_inverse(verbose, mb_io_ptr->pjptr, navlon, navlat, &(store->RMBint_lon), &(store->RMBint_lat), error);
		...
		if (nbath == store->RMB_num_beams) {
			...
			if (store->RMB_sounding_depths == NULL)
				status = mb_reallocd(verbose, __FILE__, __LINE__, nbath * sizeof(double), (void **)&(store->RMB_sounding_depths),
				                     error);
			if (store->RMB_sounding_across == NULL)
				status = mb_reallocd(verbose, __FILE__, __LINE__, nbath * sizeof(double), (void **)&(store->RMB_sounding_across),
				                     error);
			if (store->RMB_sounding_along == NULL)
				status = mb_reallocd(verbose, __FILE__, __LINE__, nbath * sizeof(double), (void **)&(store->RMB_sounding_along),
				                     error);
	...
	/* insert data in nav structure */
		...
		store->POS_x = navlon;
		store->POS_y = navlat;
		fprintf(stderr, "INSERT1b %.3f %.2f %.2f\r\n", store->POS_time, store->POS_x, store->POS_y);
...
int mbsys_hysweep_makess(int verbose, void *mbio_ptr, void *store_ptr, int pixel_size_set, double *pixel_size,
	...
	if (store->RMB_ping_number > 0 &&
	    (store->RMB_ping_number == store->RSS_ping_number || store->RMB_ping_number == 10 * store->RSS_ping_number)) {
		...
		for (i = 0; i < store->RMB_num_beams; i++) {
			if (mb_beam_ok(store->RMB_sounding_flags[i])) {
				...
				if (nrangetable == 0 || fabs(store->MSS_table_acrosstrack[nrangetable]) < acrosstracktablemin) {
		...
		for (i = sample_start; i < sample_end; i++) {
			...
			factor =
			    (range - store->MSS_table_range[irange]) / (store->MSS_table_range[irange - 1] - store->MSS_table_range[irange]);
		...
		for (k = first + 1; k < last; k++) {
			if (store->MSS_ss_cnt[k] <= 0) {
				...
				if (k2 - k1 <= pixel_int_use) {
					store->MSS_ss[k] =
					    store->MSS_ss[k1] + (store->MSS_ss[k2] - store->MSS_ss[k1]) * ((double)(k - k1)) / ((double)(k2 - k1));
					store->MSS_ss_across[k] = (k - store->MSS_num_pixels / 2) * (*pixel_size);
					store->MSS_ss_along[k] = store->MSS_ss_along[k1] + (store->MSS_ss_along[k2] - store->MSS_ss_along[k1]) *
