...
int mb_platform_add_sensor(int verbose, void *platform_ptr,
				   ...
				   int capability1, int capability2,
				   ...
				   int *error)
{
	...
	if (platform_ptr != NULL)
		...
		if (num_time_latency > 0)
			...
			status = mb_reallocd(verbose,__FILE__, __LINE__,
						size, (void **) &sensor->time_latency_time_d, error);
			if (status == MB_SUCCESS)
			status = mb_reallocd(verbose,__FILE__, __LINE__,
						size, (void **) &sensor->time_latency_value, error);
			if (status == MB_SUCCESS)
				{
...
/*--------------------------------------------------------------------*/
int mb_platform_set_sensor_offset(int verbose, void *platform_ptr,
...
/*--------------------------------------------------------------------*/
int mb_platform_set_sensor_timelatency(int verbose, void *platform_ptr,
					int isensor,
...
/*--------------------------------------------------------------------*/
int mb_platform_set_source_sensor(int verbose, void *platform_ptr,
								   int source_type, int sensor, int *error)
{
				...
				for (ioffset=0;ioffset<sensor->num_offsets;ioffset++)
					...
					if (sensor->num_time_latency_alloc > 0)
						...
						status = mb_freed(verbose,__FILE__, __LINE__,
							(void **) &sensor->time_latency_time_d, error);
						status = mb_freed(verbose,__FILE__, __LINE__,
							(void **) &sensor->time_latency_value, error);
				...
				if (buffer[0] != '#')
					...
					if (strncmp(buffer, "PLATFORM_TYPE", 13) == 0)
						...
						if (isensor >= 0 && isensor < platform->num_sensors)
							...
							if (platform->sensors[isensor].num_time_latency
								...
								status = mb_mallocd(verbose,__FILE__, __LINE__, size,
											(void **) &platform->sensors[isensor].time_latency_time_d,
											error);
								status = mb_mallocd(verbose,__FILE__, __LINE__, size,
											(void **) &platform->sensors[isensor].time_latency_value,
											error);
								if (status == MB_SUCCESS)
									{
	...
	if (platform_ptr != NULL)
		...
		if ((fp = fopen(platform_file, "w")) != NULL)
			...
			for (isensor=0;isensor<platform->num_sensors;isensor++)
				...
				fprintf(fp, "SENSOR_TYPE	             %2d  %3d  ## %s\n", isensor, platform->sensors[isensor].type, type_string);
				...
				if (mb_check_sensor_capability1_position(platform->sensors[isensor].capability1)) fprintf(fp, " position");
				if (mb_check_sensor_capability1_depth(platform->sensors[isensor].capability1)) fprintf(fp, " depth");
				if (mb_check_sensor_capability1_altitude(platform->sensors[isensor].capability1)) fprintf(fp, " altitude");
				if (mb_check_sensor_capability1_velocity(platform->sensors[isensor].capability1)) fprintf(fp, " velocity");
				if (mb_check_sensor_capability1_acceleration(platform->sensors[isensor].capability1)) fprintf(fp, " acceleration");
				if (mb_check_sensor_capability1_pressure(platform->sensors[isensor].capability1)) fprintf(fp, " pressure");
				if (mb_check_sensor_capability1_rollpitch(platform->sensors[isensor].capability1)) fprintf(fp, " rollpitch");
				if (mb_check_sensor_capability1_heading(platform->sensors[isensor].capability1)) fprintf(fp, " heading");
				if (mb_check_sensor_capability1_heading(platform->sensors[isensor].capability1)) fprintf(fp, " heading");
				if (mb_check_sensor_capability1_unused09(platform->sensors[isensor].capability1)) fprintf(fp, " unused09");
				if (mb_check_sensor_capability1_unused10(platform->sensors[isensor].capability1)) fprintf(fp, " unused10");
				if (mb_check_sensor_capability1_unused11(platform->sensors[isensor].capability1)) fprintf(fp, " unused11");
				if (mb_check_sensor_capability1_unused12(platform->sensors[isensor].capability1)) fprintf(fp, " unused12");
				if (mb_check_sensor_capability1_temperature(platform->sensors[isensor].capability1)) fprintf(fp, " temperature");
				if (mb_check_sensor_capability1_conductivity(platform->sensors[isensor].capability1)) fprintf(fp, " conductivity");
				if (mb_check_sensor_capability1_salinity(platform->sensors[isensor].capability1)) fprintf(fp, " salinity");
				if (mb_check_sensor_capability1_soundspeed(platform->sensors[isensor].capability1)) fprintf(fp, " soundspeed");
				if (mb_check_sensor_capability1_unused17(platform->sensors[isensor].capability1)) fprintf(fp, " unused17");
				if (mb_check_sensor_capability1_unused18(platform->sensors[isensor].capability1)) fprintf(fp, " unused18");
				if (mb_check_sensor_capability1_unused19(platform->sensors[isensor].capability1)) fprintf(fp, " unused19");
				if (mb_check_sensor_capability1_gravity(platform->sensors[isensor].capability1)) fprintf(fp, " gravity");
				if (mb_check_sensor_capability1_unused21(platform->sensors[isensor].capability1)) fprintf(fp, " unused21");
				if (mb_check_sensor_capability1_unused22(platform->sensors[isensor].capability1)) fprintf(fp, " unused22");
				if (mb_check_sensor_capability1_unused23(platform->sensors[isensor].capability1)) fprintf(fp, " unused23");
				if (mb_check_sensor_capability1_magneticfield(platform->sensors[isensor].capability1)) fprintf(fp, " magneticfield");
				if (mb_check_sensor_capability1_unused25(platform->sensors[isensor].capability1)) fprintf(fp, " unused25");
				if (mb_check_sensor_capability1_unused26(platform->sensors[isensor].capability1)) fprintf(fp, " unused26");
				if (mb_check_sensor_capability1_unused27(platform->sensors[isensor].capability1)) fprintf(fp, " unused27");
				if (mb_check_sensor_capability1_unused28(platform->sensors[isensor].capability1)) fprintf(fp, " unused28");
				if (mb_check_sensor_capability1_unused29(platform->sensors[isensor].capability1)) fprintf(fp, " unused29");
				if (mb_check_sensor_capability1_unused30(platform->sensors[isensor].capability1)) fprintf(fp, " unused30");
				if (mb_check_sensor_capability1_unused31(platform->sensors[isensor].capability1)) fprintf(fp, " unused31");
				fprintf(fp, "\n");
				fprintf(fp, "SENSOR_CAPABILITY2          %2d  %10d  ##", isensor, platform->sensors[isensor].capability2);
				if (mb_check_sensor_capability2_topography_echosounder(platform->sensors[isensor].capability2)) fprintf(fp, " topography_echosounder");
				if (mb_check_sensor_capability2_topography_interferometry(platform->sensors[isensor].capability2)) fprintf(fp, " topography_interferometry");
				if (mb_check_sensor_capability2_topography_sass(platform->sensors[isensor].capability2)) fprintf(fp, " topography_sass");
				if (mb_check_sensor_capability2_topography_multibeam(platform->sensors[isensor].capability2)) fprintf(fp, " topography_multibeam");
				if (mb_check_sensor_capability2_topography_photogrammetry(platform->sensors[isensor].capability2)) fprintf(fp, " topography_photogrammetry");
				if (mb_check_sensor_capability2_topography_structurefrommotion(platform->sensors[isensor].capability2)) fprintf(fp, " topography_structurefrommotion");
				if (mb_check_sensor_capability2_topography_lidar(platform->sensors[isensor].capability2)) fprintf(fp, " topography_lidar");
				if (mb_check_sensor_capability2_topography_structuredlight(platform->sensors[isensor].capability2)) fprintf(fp, " topography_structuredlight");
				if (mb_check_sensor_capability2_topography_laserscanner(platform->sensors[isensor].capability2)) fprintf(fp, " topography_laserscanner");
				if (mb_check_sensor_capability2_unused09(platform->sensors[isensor].capability2)) fprintf(fp, " unused09");
				if (mb_check_sensor_capability2_unused10(platform->sensors[isensor].capability2)) fprintf(fp, " unused10");
				if (mb_check_sensor_capability2_unused11(platform->sensors[isensor].capability2)) fprintf(fp, " unused11");
				if (mb_check_sensor_capability2_backscatter_echosounder(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_echosounder");
				if (mb_check_sensor_capability2_backscatter_sidescan(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_sidescan");
				if (mb_check_sensor_capability2_backscatter_interferometry(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_interferometry");
				if (mb_check_sensor_capability2_backscatter_sass(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_sass");
				if (mb_check_sensor_capability2_backscatter_multibeam(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_multibeam");
				if (mb_check_sensor_capability2_backscatter_lidar(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_lidar");
				if (mb_check_sensor_capability2_backscatter_structuredlight(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_structuredlight");
				if (mb_check_sensor_capability2_backscatter_laserscanner(platform->sensors[isensor].capability2)) fprintf(fp, " backscatter_laserscanner");
				if (mb_check_sensor_capability2_unused20(platform->sensors[isensor].capability2)) fprintf(fp, " unused20");
				if (mb_check_sensor_capability2_subbottom_echosounder(platform->sensors[isensor].capability2)) fprintf(fp, " subbottom_echosounder");
				if (mb_check_sensor_capability2_subbottom_chirp(platform->sensors[isensor].capability2)) fprintf(fp, " subbottom_chirp");
				if (mb_check_sensor_capability2_unused23(platform->sensors[isensor].capability2)) fprintf(fp, " unused23");
				if (mb_check_sensor_capability2_photography(platform->sensors[isensor].capability2)) fprintf(fp, " photography");
				if (mb_check_sensor_capability2_stereophotography(platform->sensors[isensor].capability2)) fprintf(fp, " stereophotography");
				if (mb_check_sensor_capability2_video(platform->sensors[isensor].capability2)) fprintf(fp, " video");
				if (mb_check_sensor_capability2_stereovideo(platform->sensors[isensor].capability2)) fprintf(fp, " stereovideo");
				if (mb_check_sensor_capability2_unused28(platform->sensors[isensor].capability2)) fprintf(fp, " unused28");
				if (mb_check_sensor_capability2_unused29(platform->sensors[isensor].capability2)) fprintf(fp, " unused29");
				if (mb_check_sensor_capability2_unused30(platform->sensors[isensor].capability2)) fprintf(fp, " unused30");
				if (mb_check_sensor_capability2_unused31(platform->sensors[isensor].capability2)) fprintf(fp, " unused31");
				fprintf(fp, "\n");
		...
		/* else proceed */
			...
			if ((sensor_rollpitch->offsets[0].attitude_offset_mode == MB_SENSOR_ATTITUDE_OFFSET_STATIC) &&
				...
				mb_platform_math_attitude_platform (verbose,
													...
													sensor_heading->offsets[0].attitude_offset_heading,
													platform_roll, 
													platform_pitch, 
													platform_heading,
													error);
