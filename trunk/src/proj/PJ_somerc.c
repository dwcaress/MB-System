#define PROJ_PARMS__ double K, c, hlf_e, kR, cosp0, sinp0;
#define PJ_LIB__
#include <projects.h>
...
sp = P->e * sin(lp.phi);
phip = 2. * atan(exp(P->c * (log(tan(FORTPI + 0.5 * lp.phi)) - P->hlf_e * log((1. + sp) / (1. - sp))) + P->K)) - HALFPI;
lamp = P->c * lp.lam;
cp = cos(phip);
...
for (i = NITER; i; --i) {
	esp = P->e * sin(phip);
	delp = (con + log(tan(FORTPI + 0.5 * phip)) - P->hlf_e * log((1. + esp) / (1. - esp))) * (1. - esp * esp) * cos(phip) *
	       P->rone_es;
	phip -= delp;
	if (fabs(delp) < EPS)
		break;
...
sp *= P->e;
P->K = log(tan(FORTPI + 0.5 * phip0)) - P->c * (log(tan(FORTPI + 0.5 * P->phi0)) - P->hlf_e * log((1. + sp) / (1. - sp)));
