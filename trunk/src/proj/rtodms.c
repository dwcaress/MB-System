...
*/
static double RES = 1000., RES60 = 60000., CONV = 206264806.24709635515796003417;
...
void set_rtodms(int fract, int con_w) {
	...
	if (fract >= 0 && fract < 9) {
		...
		if (!con_w)
			...
			(void)sprintf(format, "%%dd%%02d'%%0%d.%df\"%%c", fract + 2 + (fract ? 1 : 0), fract);
		dolong = con_w;
	}
}
char *rtodms(char *s, double r, int pos, int neg) {
	int deg, min, sign;
	char *ss = s;
	double sec;
