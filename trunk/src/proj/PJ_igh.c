...
if (z) {
	...
	case 1:
		ok = (lp.lam >= -d180 - EPSLN && lp.lam <= -d40 + EPSLN) ||
		     ((lp.lam >= -d40 - EPSLN && lp.lam <= -d10 + EPSLN) && (lp.phi >= d60 - EPSLN && lp.phi <= d90 + EPSLN));
		break;
	case 2:
		ok = (lp.lam >= -d40 - EPSLN && lp.lam <= d180 + EPSLN) ||
		     ((lp.lam >= -d180 - EPSLN && lp.lam <= -d160 + EPSLN) && (lp.phi >= d50 - EPSLN && lp.phi <= d90 + EPSLN)) ||
		     ((lp.lam >= -d50 - EPSLN && lp.lam <= -d40 + EPSLN) && (lp.phi >= d60 - EPSLN && lp.phi <= d90 + EPSLN));
		break;
	case 3:
		ok = (lp.lam >= -d180 - EPSLN && lp.lam <= -d40 + EPSLN);
		break;
...
#define SETUP(n, proj, x_0, y_0, lon_0)                                                                                          \
	if (!(P->pj[n - 1] = pj_##proj(0)))                                                                                          \
		E_ERROR_0;                                                                                                               \
	if (!(P->pj[n - 1] = pj_##proj(P->pj[n - 1])))                                                                               \
		E_ERROR_0;                                                                                                               \
	P->pj[n - 1]->x0 = x_0;                                                                                                      \
	P->pj[n - 1]->y0 = y_0;                                                                                                      \
	P->pj[n - 1]->lam0 = lon_0;
