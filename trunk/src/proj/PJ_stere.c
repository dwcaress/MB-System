...
static double ssfn_(double phit, double sinphi, double eccen) {
	sinphi *= eccen;
	return (tan(.5 * (HALFPI + phit)) * pow((1. - sinphi) / (1. + sinphi), .5 * eccen));
}
...
oblcon:
	...
	xy.y *= (P->mode == EQUIT) ? sinphi : cosph0 * sinphi - sinph0 * cosphi * coslam;
	break;
case N_POLE:
	coslam = -coslam;
	lp.phi = -lp.phi;
case S_POLE:
	if (fabs(lp.phi - HALFPI) < TOL)
		F_ERROR;
	...
	if (P->es) {
		...
		case S_POLE:
			if (fabs(P->phits - HALFPI) < EPS10)
				P->akm1 = 2. * P->k0 / sqrt(pow(1 + P->e, 1 + P->e) * pow(1 - P->e, 1 - P->e));
			else {
				P->akm1 = cos(P->phits) / pj_tsfn(P->phits, t = sin(P->phits), P->e);
				t *= P->e;
				P->akm1 /= sqrt(1. - t * t);
			}
			break;
		...
		case S_POLE:
		case N_POLE:
			P->akm1 = fabs(P->phits - HALFPI) >= EPS10 ? cos(P->phits) / tan(FORTPI - .5 * P->phits) : 2. * P->k0;
			break;
		}
		P->inv = s_inverse;
		P->fwd = s_forward;
...
ENTRY0(stere)
P->phits = pj_param(P->ctx, P->params, "tlat_ts").i ? pj_param(P->ctx, P->params, "rlat_ts").f : HALFPI;
ENDENTRY(setup(P))
ENTRY0(ups)
