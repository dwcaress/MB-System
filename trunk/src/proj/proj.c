...
static int reversein = 0, /* != 0 reverse input arguments */
    ...
    postscale = 0;
static char *cheby_str,  /* string controlling Chebychev evaluation */
...
process(FILE *fid) {
	...
	for (;;) {
		...
		{
			if (!facs_bad)
				(void)printf("\t<%g %g %g %g %g %g>", facs.h, facs.k, facs.s, facs.omega * RAD_TO_DEG, facs.a, facs.b);
			else
				(void)fputs("\t<* * * * * *>", stdout);
		}
		(void)fputs(bin_in ? "\n" : s, stdout);
			...
			for (arg = *argv;;) {
				...
				case 'l': /* list projections, ellipses or units */
					if (!arg[1] || arg[1] == 'p' || arg[1] == 'P') {
						...
						for (lp = pj_get_list_ref(); lp->id; ++lp) {
							if (strcmp(lp->id, "latlong") == 0 || strcmp(lp->id, "longlat") == 0 ||
				...
				case 'm': /* cartesian multiplier */
					if (--argc <= 0)
						goto noargument;
					postscale = 1;
					if (!strncmp("1/", *++argv, 2) || !strncmp("1:", *argv, 2)) {
