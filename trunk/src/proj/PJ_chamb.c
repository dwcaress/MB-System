...
for (i = 0; i < 3; ++i) { /* dist/azimiths from control */
	v[i] = vect(P->ctx, lp.phi - P->c[i].phi, P->c[i].cosphi, P->c[i].sinphi, cosphi, sinphi, lp.lam - P->c[i].lam);
	if (!v[i].r)
		break;
...
for (i = 0; i < 3; ++i) { /* inter ctl pt. distances and azimuths */
	j = i == 2 ? 0 : i + 1;
	P->c[i].v = vect(P->ctx, P->c[j].phi - P->c[i].phi, P->c[i].cosphi, P->c[i].sinphi, P->c[j].cosphi, P->c[j].sinphi,
	                 P->c[j].lam - P->c[i].lam);
	if (!P->c[i].v.r)
		E_ERROR(-25);
