...
static void Usage() {
	fprintf(stderr, "usage: nad2bin [-f ctable/ctable2/ntv2] binary_output < ascii_source\n");
	exit(1);
}
...
int main(int argc, char **argv) {
	...
	if (EOF == scanf("%d %d %*d %lf %lf %lf %lf", &ct.lim.lam, &ct.lim.phi, &ct.ll.lam, &ct.del.lam, &ct.ll.phi, &ct.del.phi)) {
		perror("scanf");
		exit(1);
	}
	if (!(ct.cvs = (FLP *)malloc(tsize = ct.lim.lam * ct.lim.phi * sizeof(FLP)))) {
		perror("mem. alloc");
		exit(1);
	}
	...
	if (strcmp(format, "ctable") == 0) {
		...
		if (fwrite(&ct, sizeof(ct), 1, fp) != 1 || fwrite(ct.cvs, tsize, 1, fp) != 1) {
		...
		/* -------------------------------------------------------------------- */
		{
			...
			for (row = 0; row < ct.lim.phi; row++) {
				...
				for (i = 0; i < ct.lim.lam; i++) {
					FLP *cvs = ct.cvs + (row)*ct.lim.lam + (ct.lim.lam - i - 1);
				...
				if (fwrite(row_buf, sizeof(float), ct.lim.lam * 4, fp) != 4 * ct.lim.lam) {
					perror("write()");
					exit(2);
				}
			}
