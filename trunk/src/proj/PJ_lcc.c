...
if (fabs(fabs(lp.phi) - HALFPI) < EPS10) {
	...
	rho = P->c * (P->ellips ? pow(pj_tsfn(lp.phi, sin(lp.phi), P->e), P->n) : pow(tan(FORTPI + .5 * lp.phi), -P->n));
...
if ((rho = hypot(xy.x, xy.y = P->rho0 - xy.y)) != 0.0) {
	...
	if (P->ellips) {
		if ((lp.phi = pj_phi2(P->ctx, pow(rho / P->c, 1. / P->n), P->e)) == HUGE_VAL)
			I_ERROR;
	}
	else
...
SPECIAL(fac) {
	...
	fac->k = fac->h = P->k0 * P->n * rho / pj_msfn(sin(lp.phi), cos(lp.phi), P->es);
	fac->conv = -P->n * lp.lam;
}
FREEUP;
if (P)
...
if ((P->ellips = (P->es != 0.))) {
	...
	if (secant) { /* secant cone */
		P->n = log(m1 / pj_msfn(sinphi = sin(P->phi2), cos(P->phi2), P->es));
		P->n /= log(ml1 / pj_tsfn(P->phi2, sinphi, P->e));
	}
	P->c = (P->rho0 = m1 * pow(ml1, -P->n) / P->n);
	P->rho0 *= (fabs(fabs(P->phi0) - HALFPI) < EPS10) ? 0. : pow(pj_tsfn(P->phi0, sin(P->phi0), P->e), P->n);
}
else {
	if (secant)
		P->n = log(cosphi / cos(P->phi2)) / log(tan(FORTPI + .5 * P->phi2) / tan(FORTPI + .5 * P->phi1));
	P->c = cosphi * pow(tan(FORTPI + .5 * P->phi1), P->n) / P->n;
	P->rho0 = (fabs(fabs(P->phi0) - HALFPI) < EPS10) ? 0. : P->c * pow(tan(FORTPI + .5 * P->phi0), -P->n);
}
P->inv = e_inverse;
P->fwd = e_forward;
P->spc = fac;
