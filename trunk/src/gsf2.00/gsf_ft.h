...
typedef struct
{
    time_t	    sec;			      /* seconds from the epoch       */
    long	    nsec;			      /* nanoseconds of the second    */
    long	    addr;			      /* address in the gsf file      */
}
INDEX_REC;
...
typedef struct t_index_data
{
    ...
    long	    start_addr[NUM_REC_TYPES];	      /* start address of record type */
...
typedef struct t_gsfFileTable
{
    ...
    long	    file_size;			   /* The file's size when gsfOpen is called */
    long	    previous_record;		   /* File offset to previous record */
