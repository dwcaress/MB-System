...
int read_recursive2(char *fileName);
        ...
        else
        {
            if (p_flag==1)			/* calculate the nearest in time */
            {
                ...
                for(j=0; j<size_2; j++)
                {
                    ...
                    if(j==0){temp_time=time_hold[0][j];}
            ...
            if(p_flag==2)
            {
                ...
                if(count==0){
                    if (verbose==1)
                        printf("\nnon of the SVP profiles are within the time period, The tool is selecting nearest in position without time considaration\n");
                    if (verbose==1)
                        printf("the shortest distance is number %d from the list\n", n_pos);
                    n = n_pos;
                }
            ...
            if (p_flag==3)
            {
                ...
                for(j = 0;j<size_2;j++){
                    ...
                    printf("distance - range (if positive then SVP out of range if negative then the SVP within range) %d is : %lf\n", j, dist[0][j]);
                ...
                if(count==0){
                    if(p_4_flage==0){
                        if (verbose==1)
                            printf("\nnon of the SVP profiles are within the specified range, The tool is selecting nearest in time without range considaration\n");
                        if (verbose==1)
