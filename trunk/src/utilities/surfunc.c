...
int surfunc(verbose,ndat,xdat,ydat,zdat,
...

{
	...
	void	suggest_sizes_for_surface();
	...
	*/
	...
	briggs = (struct BRIGGS *) memory (CNULL, npoints, sizeof(struct BRIGGS), "surface");
	iu = (char *) memory (CNULL, mx * my, sizeof(char), "surface");
	u = (float *) memory (CNULL, mx * my, sizeof(float), "surface");
	...
	*/
	...
	free ((char *) briggs);
	free (iu);
	if (set_low) free ((char *)lower);
	if (set_high) free ((char *)upper);
	free ((char *) u);
...
float	*zdat;
{
	...
	data = (struct DATA *) memory (CNULL, ndat, 
		sizeof(struct DATA), "surface");
		...
		for (i = 0; i < nx; i += grid, x_w_case++, x_e_case--) {
			...
			for (j = 0; j < ny; j += grid, ij += grid, y_s_case++, y_n_case--) {
				...
				if (constrained) {	/* Must check limits.  Note lower/upper is v2 format and need ij_v2! */
					ij_v2 = (ny - j - 1) * nx + i;
					if (set_low && !bad_float((double)lower[ij_v2]) && sum_ij < lower[ij_v2])
						sum_ij = lower[ij_v2];
					else if (set_high && !bad_float((double)upper[ij_v2]) && sum_ij > upper[ij_v2])
						sum_ij = upper[ij_v2];
				}
...
char *low, *high; {
	...
	if (set_low > 0) {
		lower = (float *) memory (CNULL, nx * ny, sizeof (float), "surface");
		if (set_low < 3)
...
*/
		...
		for (j = ij = 0; j < ny; j++) {
			yy = ny - j - 1;
			for (i = 0; i < nx; i++, ij++) {
				if (bad_float ((double)lower[ij])) continue;
	...
	if (set_high > 0) {
		upper = (float *) memory (CNULL, nx * ny, sizeof (float), "surface");
		if (set_high < 3)
...
*/
		...
		for (j = ij = 0; j < ny; j++) {
			yy = ny - j - 1;
			for (i = 0; i < nx; i++, ij++) {
				if (bad_float ((double)upper[ij])) continue;
...
void	suggest_sizes_for_surface(nx, ny)
int	nx, ny;
{
	...
	int	compare_sugs();	/* Sort suggestions decreasing  */
		...
		for (ny2 = 2; ny2 <= ystop; ny2 *= 2) {
		  for (ny3 = 1; ny3 <= ystop; ny3 *= 3) {
			...
			if (current_time < users_time) {
				n_sug++;
				sug = (struct SUGGESTION *)memory((char *)sug, n_sug, sizeof(struct SUGGESTION), "suggest_sizes_for_surface");
				sug[n_sug-1].nx = nxg;
				sug[n_sug-1].ny = nyg;
	...
	if (n_sug) {
		...
		fprintf(stderr,"surface: Cannot suggest any nx,ny better than your -R -I define.\n");
	}
	return;
}

int	compare_sugs(point_1, point_2)
struct SUGGESTION	*point_1, *point_2;
{
	/* Sorts sugs into DESCENDING order!  */
	if (point_1->factor < point_2->factor)
		return(1);
	else if (point_1->factor > point_2->factor)
		return(-1);
	else
		return(0);
}
