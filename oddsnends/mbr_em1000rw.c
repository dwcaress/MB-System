 ...
 *	$Id: mbr_em1000rw.c,v 5.0 2000-12-01 22:48:41 caress Exp $
 *
 ...
 *--------------------------------------------------------------------*/
/*
 * mbr_em1000rw.c contains the functions for reading and writing
 * multibeam data in the EM1000RW format.  
 * These functions include:
 *   mbr_alm_em1000rw	- allocate read/write memory
 *   mbr_dem_em1000rw	- deallocate read/write memory
 *   mbr_rt_em1000rw	- read and translate data
 *   mbr_wt_em1000rw	- translate and write data
 *
 * Author:	D. W. Caress
 * Date:	August 8, 1994
 ...
 * Convert to ANSI C
 *
 * Revision 4.15  2000/09/30  06:34:20  caress
 * Snapshot for Dale.
 *
 * Revision 4.14  2000/07/19  03:28:02  caress
 ...
 *
 * Revision 4.4  1996/07/26  21:06:00  caress
...
int mbr_info_em1000rw(int verbose, 
			int *system, 
			int *beams_bath_max, 
			int *beams_amp_max, 
			int *pixels_ss_max, 
			...
			int (**copyrecord)(), 
			int *error);
int mbr_alm_em1000rw(int verbose, char *mbio_ptr, int *error);
int mbr_dem_em1000rw(int verbose, char *mbio_ptr, int *error);
int mbr_rt_em1000rw(int verbose, char *mbio_ptr, char *store_ptr, int *error);
int mbr_wt_em1000rw(int verbose, char *mbio_ptr, char *store_ptr, int *error);
...
int mbr_info_em1000rw(int verbose, 
			...
			int *error)
{
	static char res_id[]="$Id: mbr_em1000rw.c,v 5.0 2000-12-01 22:48:41 caress Exp $";
	char	*function_name = "mbr_info_em1000rw";
	...
	*format_alloc = &mbr_alm_em1000rw;
	*format_free = &mbr_dem_em1000rw; 
	*store_alloc = &mbsys_simrad_alloc; 
	*store_free = &mbsys_simrad_deall; 
	*read_ping = &mbr_rt_em1000rw; 
	*write_ping = &mbr_wt_em1000rw; 
...
int mbr_alm_em1000rw(int verbose, char *mbio_ptr, int *error)
{
	...
	mb_io_ptr->structure_size = sizeof(struct mbf_em1000rw_struct);
	...
	mbr_zero_em1000rw(verbose,mb_io_ptr->raw_data,error);
...
int mbr_dem_em1000rw(int verbose, char *mbio_ptr, int *error)
{
	char	*function_name = "mbr_dem_em1000rw";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
...
/*--------------------------------------------------------------------*/
int mbr_zero_em1000rw(int verbose, char *data_ptr, int *error)
{
	char	*function_name = "mbr_zero_em1000rw";
	int	status = MB_SUCCESS;
	...
	data = (struct mbf_em1000rw_struct *) data_ptr;

	/* initialize everything to zeros */
	if (data != NULL)
		{
		data->kind = MB_DATA_NONE;
		data->sonar = MBSYS_SIMRAD_EM1000;
		...
		data->centisecond = 0;
		data->ping_number = 0;
		data->beams_bath = MBF_EM1000RW_MAXBEAMS;
		data->bath_mode = 0;
		data->bath_res = 0;
		data->bath_quality = 0;
		data->keel_depth = 0;
		...
		data->pixels_ss = 0;
		data->ss_mode = 0;
		for (i=0;i<MBF_EM1000RW_MAXBEAMS;i++)
			{
			data->bath[i] = 0;
		...
		for (i=0;i<MBF_EM1000RW_MAXPIXELS;i++)
			{
			data->ss[i] = 0;
			}
		}
...
int mbr_rt_em1000rw(int verbose, char *mbio_ptr, char *store_ptr, int *error)
{
	char	*function_name = "mbr_rt_em1000rw";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_em1000rw_struct *data;
	...
	mb_io_ptr = (struct mb_io_struct *) mbio_ptr;
	data = (struct mbf_em1000rw_struct *) mb_io_ptr->raw_data;
	store = (struct mbsys_simrad_struct *) store_ptr;

	/* read next data from file */
	status = mbr_em1000rw_rd_data(verbose,mbio_ptr,error);
	...
	/* handle problems with sidescan mode values 
		- infer from bath mode values */
	if (status == MB_SUCCESS 
		&& data->kind == MB_DATA_DATA)
		{
...
int mbr_wt_em1000rw(int verbose, char *mbio_ptr, char *store_ptr, int *error)
{
	char	*function_name = "mbr_wt_em1000rw";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_em1000rw_struct *data;
	char	*data_ptr;
	...
	status = mbr_em1000rw_wr_data(verbose,mbio_ptr,data_ptr,error);
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_data(int verbose, char *mbio_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_data";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_em1000rw_struct *data;
	char	*data_ptr;
	...
	*error = MB_ERROR_NO_ERROR;
	while (done == MB_NO)
		...
		*type = (short int) mb_swap_short(*type);
		...
		/*fprintf(stderr,"\nstart of mbr_em1000rw_rd_data loop:\n");
	...
	/*fprintf(stderr,"call nothing, try again\n");*/
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_start type %x\n",*type);*/
			status = mbr_em1000rw_rd_start(
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_stop type %x\n",*type);*/
			status = mbr_em1000rw_rd_stop(
				verbose,mbfp,data,error);
			if (status == MB_SUCCESS)
				{
				done = MB_YES;
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_parameter type %x\n",*type);*/
			status = mbr_em1000rw_rd_parameter(
				verbose,mbfp,data,error);
			if (status == MB_SUCCESS)
				{
				done = MB_YES;
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_pos type %x\n",*type);*/
			status = mbr_em1000rw_rd_pos(
				verbose,mbfp,data,error);
			if (status == MB_SUCCESS)
				{
				done = MB_YES;
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_svp type %x\n",*type);*/
			status = mbr_em1000rw_rd_svp(
				verbose,mbfp,data,error);
			if (status == MB_SUCCESS)
				{
				done = MB_YES;
	...
	/*fprintf(stderr,"call nothing, expect %x but got type %x\n",expect,*type);*/
			...
			*label_save_flag = MB_YES;
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_bath type %x\n",*type);*/
			status = mbr_em1000rw_rd_bath(
				verbose,mbfp,data,error);
			if (status == MB_SUCCESS)
				{
				data->kind = MB_DATA_DATA;
	...
	/*fprintf(stderr,"call nothing, expect %x but got type %x\n",expect,*type);*/
			...
			*label_save_flag = MB_YES;
			...
			{
	/*fprintf(stderr,"call mbr_em1000rw_rd_ss type %x\n",*type);*/
			status = mbr_em1000rw_rd_ss(
		...
		/*fprintf(stderr,"end of mbr_em1000rw_rd_data loop:\n");
		fprintf(stderr,"done:%d\n",done);
		fprintf(stderr,"expect:%x\n",expect);
		fprintf(stderr,"type:%x\n",*type);*/
		}
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_start(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_start";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_stop(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_stop";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_parameter(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_parameter";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_pos(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_pos";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_svp(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_svp";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_bath(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_bath";
	int	status = MB_SUCCESS;
	char	line[EM_1000_BATH_SIZE+3];
	char	beamarray[11];
	short int *short_ptr;
	char	*char_ptr;
	int	i, j;
	...
	/* read record into char array */
	status = fread(line,1,EM_1000_BATH_SIZE+3,mbfp);
	if (status == EM_1000_BATH_SIZE+3)
		status = MB_SUCCESS;
	else
		{
		status = MB_FAILURE;
	...
	if (status == MB_SUCCESS)
		...
		short_ptr = (short int *) &line[26]; data->xducer_pitch = *short_ptr;
		short_ptr = (short int *) &line[28]; data->ping_heave = *short_ptr;
		short_ptr = (short int *) &line[30]; data->sound_vel = *short_ptr;
		for (i=0;i<data->beams_bath;i++)
			{
			for (j=0;j<11;j++)
		...
		data->xducer_pitch = (short int) mb_swap_short(*short_ptr);
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_rd_ss(int verbose, FILE *mbfp, 
		struct mbf_em1000rw_struct *data, 
		int first, int *more, int *error)
{
	char	*function_name = "mbr_em1000rw_rd_ss";
	int	status = MB_SUCCESS;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_data(int verbose, char *mbio_ptr, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_data";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_em1000rw_struct *data;
	FILE	*mbfp;
	...
	if (data->kind == MB_DATA_COMMENT)
		{
		status = mbr_em1000rw_wr_parameter(verbose,mbfp,data,error);
		}
	else if (data->kind == MB_DATA_START)
		{
		status = mbr_em1000rw_wr_start(verbose,mbfp,data,error);
		}
	else if (data->kind == MB_DATA_STOP)
		{
		status = mbr_em1000rw_wr_stop(verbose,mbfp,data,error);
		}
	else if (data->kind == MB_DATA_NAV)
		{
		status = mbr_em1000rw_wr_pos(verbose,mbfp,data,error);
		}
	else if (data->kind == MB_DATA_VELOCITY_PROFILE)
		{
		status = mbr_em1000rw_wr_svp(verbose,mbfp,data,error);
		}
	else if (data->kind == MB_DATA_DATA)
		{
		status = mbr_em1000rw_wr_bath(verbose,mbfp,data,error);
		status = mbr_em1000rw_wr_ss(verbose,mbfp,data,error);
		}
	else
		{
		status = MB_FAILURE;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_start(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_start";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_START_SIZE+3];
	short int label;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_stop(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_stop";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_STOP_SIZE+3];
	short int label;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_parameter(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_parameter";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_PARAMETER_SIZE+3];
	short int label;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_pos(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_pos";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_POS_SIZE+3];
	short int label;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_svp(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_svp";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_SVP_SIZE+3];
	short int label;
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_bath(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_bath";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_1000_BATH_SIZE+3];
	char	beamarray[11];
	...
	if (status == MB_SUCCESS)
		...
		*short_ptr = (short int) data->pitch;
		short_ptr = (short int *) &line[26];
		*short_ptr = (short int) data->xducer_pitch;
		short_ptr = (short int *) &line[28];
		...
		*short_ptr = (short int) mb_swap_short((short int)data->pitch);
		short_ptr = (short int *) &line[26];
		*short_ptr = (short int) mb_swap_short((short int)data->xducer_pitch);
		short_ptr = (short int *) &line[28];
...
/*--------------------------------------------------------------------*/
int mbr_em1000rw_wr_ss(int verbose, FILE *mbfp, char *data_ptr, int *error)
{
	char	*function_name = "mbr_em1000rw_wr_ss";
	int	status = MB_SUCCESS;
	struct mbf_em1000rw_struct *data;
	char	line[EM_12S_SS_SIZE+3];
	short int label;
	...
	int	datagram;
	int	num_beams;
	int	datagram_start[MBF_EM1000RW_MAXBEAMS+1];
	...
	for (i=0;i<MBF_EM1000RW_MAXBEAMS;i++)
		...
		else if (i == MBF_EM1000RW_MAXBEAMS - 1)
			{
			datagram_end[num_datagrams] = i;
			datagram_size[num_datagrams] = new_datagram_size;
			num_datagrams++;
