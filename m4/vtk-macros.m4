...
dnl See https://vtk.org/Wiki/VTK_Autoconf
dnl ======================================================================================
...
[
  ...
  AC_ARG_WITH([vtk-version],
    ...
    [VTK's include directory name is vtk-suffix, e.g. vtk-5.0/. What's the suffix? (Default -8.2)])],
    [vtk_suffix=$withval],
    [vtk_suffix="-8.2"])
...
[
  ...
  if test "$with_vtk" != no; then
    ...
    AC_CHECK_FILE([$VTK_PREFIX/include/vtk$vtk_suffix/vtkActor.h], [vtkFound="OK"])
    ...
    if test -z "$vtkFound"; then
      ...
      if test -n "$1"; then
        ...
        AC_ECHO([if VTK version is at least $maj.$min.$rel])        
