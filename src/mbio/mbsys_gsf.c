	...
	*kind = store->kind;
	...
	if (*kind == MB_DATA_DATA) {
		...
		if (mb_ping->depth != NULL)
			...
			*nbath = 0;
		if (mb_ping->mc_amplitude != NULL || mb_ping->mr_amplitude != NULL)
	...
	if (*kind == MB_DATA_DATA) {
	  ...
	  double vertical, range, beam_foot, sint, angle;
		if (mb_ping->brb_inten != NULL) {
			...
			if (mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8101_SPECIFIC ||
			    mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8111_SPECIFIC ||
			    mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8124_SPECIFIC ||
			    mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8125_SPECIFIC ||
			    mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8150_SPECIFIC ||
			    mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_RESON_8160_SPECIFIC) {
				ss_spacing = 750.0 / ((double)mb_ping->sensor_data.gsfReson8100Specific.sample_rate);
			}
			else if (mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM3000_SPECIFIC ||
			         mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM1002_SPECIFIC ||
			         mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM300_SPECIFIC ||
			         mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM120_SPECIFIC ||
			         mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM3002_SPECIFIC ||
			         mb_ping->sensor_id == GSF_SWATH_BATHY_SUBRECORD_EM3000D_SPECIFIC ||
	      ...
	      gsfTimeSeriesIntensity *snippet = &(mb_ping->brb_inten->time_series[i]);
				...
				beam_foot = range * sin(DTR * mb_io_ptr->beamwidth_xtrack) / cos(DTR * angle);
...
int mbsys_gsf_insert(int verbose, void *mbio_ptr, void *store_ptr, int kind, int time_i[7], double time_d, double navlon,
	...
	if (store->kind == MB_DATA_DATA) {
		...
		if (nbath > mb_ping->number_beams) {
			mb_ping->beam_flags = (unsigned char *)realloc(mb_ping->beam_flags, nbath * sizeof(unsigned char));
			mb_ping->depth = (double *)realloc(mb_ping->depth, nbath * sizeof(double));
		...
		if (namp > mb_ping->number_beams && mb_ping->mc_amplitude != NULL) {
		...
		else if (namp > mb_ping->number_beams && mb_ping->mr_amplitude != NULL) {
		...
		if (mb_ping->travel_time != NULL && mb_ping->beam_angle != NULL) {
			if (mb_ping->beam_angle_forward != NULL) {
				for (int i = 0; i < *nbeams; i++) {
					...
					if (mb_ping->beam_angle[i] < 0.0 && mb_ping->across_track[i] < 0.0 &&
					    (fabs(mb_ping->beam_angle_forward[i]) < 90.0 || fabs(mb_ping->beam_angle_forward[i]) > 270.0)) {
...
int mbsys_gsf_insert_svp(int verbose, void *mbio_ptr, void *store_ptr, int nsvp, double *depth, double *velocity, int *error) {
	...
	if (store->kind == MB_DATA_VELOCITY_PROFILE) {
		...
		if (nsvp > svp->number_points || svp->depth == NULL || svp->sound_speed == NULL) {
