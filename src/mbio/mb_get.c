		...
		*nss = 0;
		for (int i = 0; i < mb_io_ptr->beams_bath_max; i++) {
			...
			if (mb_io_ptr->bath_num[i] > 0) {
				...
				bathacrosstrack[i] = (mb_io_ptr->bath_acrosstrack[i]) / (mb_io_ptr->bath_num[i]);
				bathalongtrack[i] = (mb_io_ptr->bath_alongtrack[i]) / (mb_io_ptr->bath_num[i]);
				*nbath = i + 1;
			}
			else {
		...
		for (int i = 0; i < mb_io_ptr->pixels_ss_max; i++) {
			if (mb_io_ptr->ss_num[i] > 0) {
				...
				ssalongtrack[i] = mb_io_ptr->ss_alongtrack[i] / mb_io_ptr->ss_num[i];
				*nss = i + 1;
			}
			else {
				ss[i] = 0.0;
