...
int mbr_rt_sbsiolsi(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	struct mbf_sbsiolsi_struct *dataplus = (struct mbf_sbsiolsi_struct *)mb_io_ptr->raw_data;
	struct mbf_sbsiolsi_data_struct *data = &(dataplus->data);
...
int mbr_register_sbsiolsi(int verbose, void *mbio_ptr, int *error) {
	...
	/* set format info parameters */
	const int status = mbr_info_sbsiolsi(
