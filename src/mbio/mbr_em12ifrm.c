...
int mbr_info_em12ifrm(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = MBF_EM12IFRM_MAXPIXELS;
	...
	strncpy(format_description,
	        ...
	        "    Simrad EM12S and EM12D,\n                     bathymetry, amplitude, and sidescan\n                      81 "
	        "beams, variable pixels, binary,\n                      read-only, IFREMER.\n",
...
assert(strlen(mb_io_ptr->file) < MB_PATH_MAXLINE - 12);
	...
	if ((tag_ptr = strstr(mb_io_ptr->file, ".SO")) != NULL) {
		...
		if (imgfile_stat == 0 && (imgfile_status.st_mode & S_IFMT) != S_IFDIR) {
	...
	if ((tag_ptr = strstr(mb_io_ptr->file, ".SO")) != NULL) {
		...
		if (navfile_stat == 0 && (navfile_status.st_mode & S_IFMT) != S_IFDIR) {
...
int mbr_em12ifrm_rd_data(int verbose, void *mbio_ptr, int *error) {
	...
	if (!done && *save_data == MB_DATA_NONE) {
		...
		if (status == MB_SUCCESS && data->kind == MB_DATA_DATA) {
			...
			/* get time */
			mb_get_int(&data->day, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->month, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->year, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->hour, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->minute, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->second, &line[shift], 2);
			shift += 3;
			mb_get_int(&data->centisecond, &line[shift], 2);
			shift += 11;
			...
			for (int i = 0; i < MBF_EM12IFRM_MAXBEAMS; i++) {
				...
				shift += 1;
				data->quality[i] = (int)line[shift];
				shift += 1;
				data->heave[i] = (int)line[shift];
				shift += 2;
			}
		...
		/* deal with comment */
		else if (status == MB_SUCCESS && data->kind == MB_DATA_COMMENT) {
			shift = 6;
			mb_get_int(&len, &line[shift], 3);
			shift += 4;
		...
		*ss_ping_number = 0;
		while (!done && *ss_available && *ss_ping_number <= data->ping_number) {
			...
			if (status == MB_SUCCESS) {
				...
				/* get time */
				mb_get_int(ss_day, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_month, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_year, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_hour, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_minute, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_second, &line[shift], 2);
				shift += 3;
				mb_get_int(ss_centisecond, &line[shift], 2);
				shift += 11;
				...
				*ss_num_beams = (int)line[shift];
				...
				for (int i = 0; i < *ss_num_beams; i++) {
					...
					if (data->pixels_ssraw + data->beam_samples[beamlist[i]] > MBF_EM12IFRM_MAXRAWPIXELS)
				...
				while (status == MB_SUCCESS && line[0] != '\n') {
					if ((read_status = fread(line, 1, 1, mb_io_ptr->mbfp2)) != 1) {
						status = MB_FAILURE;
						*error = MB_ERROR_EOF;
					}
				}
				...
				if ((result = fgets(line, MBF_EM12IFRM_RECORD_SIZE, mb_io_ptr->mbfp3)) != line) {
					...
					*nav_available = false;
					...
					if (strncmp(&line[shift], "NACOU", 5) == 0) {
						/* get time */
						shift = 7;
						mb_get_int(&data->pos_day, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_month, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_year, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_hour, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_minute, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_second, &line[shift], 2);
						shift += 3;
						mb_get_int(&data->pos_centisecond, &line[shift], 2);
						shift += 10;

						/* get navigation */
						NorS = line[shift];
						shift += 2;
						mb_get_int(&latdeg, &line[shift], 2);
						shift += 3;
						mb_get_double(&latmin, &line[shift], 8);
						shift += 9;
						EorW = line[shift];
						shift += 2;
						mb_get_int(&londeg, &line[shift], 3);
						shift += 4;
						mb_get_double(&lonmin, &line[shift], 8);
						shift += 42;
						mb_get_double(&data->line_heading, &line[shift], 6);
						shift += 6;
...
int mbr_em12ifrm_wr_data(int verbose, void *mbio_ptr, char *data_ptr, int *error) {
	...
	if (data->kind == MB_DATA_DATA) {
		...
		if (data->swath_id == EM_SWATH_CENTER)
			...
			char_value = 'C';
		sprintf(&line[shift], "$12SO%c,%2.2d/%2.2d/%2.2d,%2.2d:%2.2d:%2.2d.%2.2d0,VOIE%c,", char_value, data->day, data->month,
		        data->year, data->hour, data->minute, data->second, data->centisecond, char_value);
		shift += 35;
		...
		for (int i = 0; i < data->beams_bath; i++) {
			...
			shift += 1;
			line[shift] = (char)data->quality[i];
			shift += 1;
			line[shift] = (char)data->heave[i];
			shift += 1;
			line[shift] = (char)0;
			shift += 1;
		}
