...
int mbr_3ddepthp_rd_data(int verbose, void *mbio_ptr, void *store_ptr, int *error)
{
	...
	/* get saved values */
	file_header_readwritten = (int *) &mb_io_ptr->save1;
	need_parameter = (int *) &mb_io_ptr->save3;
	...
	*error = MB_ERROR_NO_ERROR;
	...
	/* if first read then read 6 byte file header */
	if (*file_header_readwritten == MB_NO)
		{
		/* read file header into buffer */
		read_len = (size_t)MBF_3DDEPTHP_FILEHEADER_SIZE;
		...
		if (status == MB_SUCCESS)
			...
			/* check magic number */
			if (magic_number == MBF_3DDEPTHP_MAGICNUMBER)
				{
				*file_header_readwritten = MB_YES;
				*need_parameter = MB_YES;
				status = MB_SUCCESS;
				*error = MB_ERROR_NO_ERROR;
				}
			else
	...
	/* read next record according to the file version */
	if (status == MB_SUCCESS)
		{
//fprintf(stderr,"READ NEXT RECORD verions %d %d\n",store->file_version,store->sub_version);

		/* deal with the original format and the parameter record at the start of file */
		if (store->file_version == 1 && store->sub_version == 0
			&& *need_parameter == MB_YES)
			{
			/* read file header into buffer */
			read_len = (size_t)MBF_3DDEPTHP_PARAMETER_SIZE;
			status = mb_fileio_get(verbose, mbio_ptr, buffer, &read_len, error);
			...
			if (status == MB_SUCCESS)
				...
				mb_get_binary_int(MB_YES, (void *) &buffer[index], &(store->scans_per_file)); index += 4;
				mb_get_binary_short(MB_YES, (void *) &buffer[index], &(store->unused)); index += 2;
				store->current_scan = -1;
						
				/* calculate number of pulses according to mode */
				...
				/* no longer need parameter */
				*need_parameter = MB_NO;
				
				store->kind = MB_DATA_PARAMETER;
				}
			}

		/* deal with the original format after the parameter record
			- LIDAR scans only with no record id's */
		else if (store->file_version == 1 && store->sub_version == 0)
			{
...
//fprintf(stderr,"READ %d pulses, save status:%d current_scan:%d\n",store->num_pulses,mb_io_ptr->save2,store->current_scan);
		...
		/* deal with the MBARI extended format - all records have record id's
			- includes comments, nav, attitude, sonardepth, and LIDAR scans */
		else if (store->file_version == 1 && store->sub_version == 1)
			{
...
//fprintf(stderr,"RECORD ID: %x %d\n",store->record_id,store->record_id);
			...
			/* read the parameter record if needed */
...
int mbr_3ddepthp_wr_data(int verbose, void *mbio_ptr, void *store_ptr, int *error)
{
	...
	*error = MB_ERROR_NO_ERROR;
	...
	/* if first write then write file header */
	if (*file_header_readwritten == MB_NO)
		{
		...
		write_len = (size_t)MBF_3DDEPTHP_FILEHEADER_SIZE;
	...
	if (status == MB_SUCCESS)
		...
		if (store->kind == MB_DATA_PARAMETER)
			...
			mb_put_binary_int(MB_YES, store->scans_per_file, &buffer[index]); index += 4;
			mb_put_binary_short(MB_YES, store->unused, &buffer[index]); index += 2;
