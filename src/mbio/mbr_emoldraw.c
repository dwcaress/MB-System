...
/* swap bytes if necessary */
	...
	if (type != EM_START && type != EM_STOP && type != EM_PARAMETER && type != EM_POS && type != EM_SVP && type != EM_12DS_BATH &&
	    type != EM_12DP_BATH && type != EM_12S_BATH && type != EM_121_BATH && type != EM_1000_BATH && type != EM_12DP_SS &&
	    type != EM_12DS_SS && type != EM_12S_SS && type != EM_12DP_SSP && type != EM_12DS_SSP && type != EM_12S_SSP) {
		status = MB_FAILURE;
		startid = (char *)&type;
...
int mbr_emoldraw_rd_svp(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		mb_get_binary_short(false, &line[14], &short_val);
		store->svp_num = short_val;
		for (int i = 0; i < store->svp_num; i++) {
			mb_get_binary_short(false, &line[16 + 4 * i], &short_val);
			store->svp_depth[i] = short_val;
			mb_get_binary_short(false, &line[18 + 4 * i], &short_val);
...
int mbr_emoldraw_rd_em1000bath(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		mb_get_binary_short(false, &line[18], &short_val);
		ping->keel_depth = (int)short_val;
		mb_get_binary_short(false, &line[20], &short_val);
		ping->heading = (int)short_val;
		mb_get_binary_short(false, &line[22], &short_val);
		ping->roll = (int)short_val;
		mb_get_binary_short(false, &line[24], &short_val);
		ping->pitch = (int)short_val;
		mb_get_binary_short(false, &line[26], &short_val);
		ping->xducer_pitch = (int)short_val;
		mb_get_binary_short(false, &line[28], &short_val);
		ping->ping_heave = (int)short_val;
		mb_get_binary_short(false, &line[30], &short_val);
		ping->sound_vel = (int)short_val;
		for (int i = 0; i < MBSYS_EM1000_MAXBEAMS; i++) {
			for (int j = 0; j < 11; j++)
				beamarray[j] = line[32 + 11 * i + j];
			short_ptr = (short int *)beamarray;
			mb_get_binary_short(false, &short_ptr[0], (short *)&ping->bath[i]);
			mb_get_binary_short(false, &short_ptr[1], &ping->bath_acrosstrack[i]);
			mb_get_binary_short(false, &short_ptr[2], &ping->bath_alongtrack[i]);
			mb_get_binary_short(false, &short_ptr[3], &ping->tt[i]);
...
int mbr_emoldraw_rd_em121bath(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		mb_get_binary_short(false, &line[30], &short_val);
		ping->pitch = (int)short_val;
		mb_get_binary_short(false, &line[32], &short_val);
		ping->ping_heave = (int)short_val;
		mb_get_binary_short(false, &line[34], &short_val);
		ping->sound_vel = (int)short_val;
...
int mbr_emoldraw_rd_ss(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int swath_id, int first, int *more,
	...
	if (status == MB_SUCCESS) {
		...
		mb_get_binary_short(false, &line[14], &short_val);
		ping->ping_number = (int)short_val;
		/*		mb_get_binary_short(false, &line[16], &short_val); ping->sound_vel = (int) short_val;*/
		...
		if (num_datagrams < 1 || num_datagrams > 255 || datagram < 1 || datagram > 255 || num_beams < 1 ||
		    num_beams > MBSYS_SIMRAD_MAXBEAMS) {
			num_beams = 0;
		}
		...
		for (int i = 0; i < num_beams; i++) {
			...
			mb_get_binary_short(false, &line[24 + 6 * i], &ping->beam_samples[beamlist[i]]);
			mb_get_binary_short(false, &line[26 + 6 * i], &ping->beam_center_sample[beamlist[i]]);
	...
	if (status == MB_SUCCESS) {
		...
		for (int i = 0; i < num_beams; i++) {
			...
			for (int j = 0; j < ping->beam_samples[beamlist[i]]; j++) {
				...
				mb_get_binary_short(false, (short *)char_ptr, &ping->ssp[ping->pixels_ssraw]);
				ioffset += 3;
				ping->pixels_ssraw++;
			}
		}
	...
	*error = MB_ERROR_NO_ERROR;
	while (done == MB_NO) {
		...
		if (*label_save_flag == MB_NO) {
			...
			/* look for label */
			if (status == MB_SUCCESS && (read_len = fread(label, 1, 2, mb_io_ptr->mbfp)) != 2) {
				status = MB_FAILURE;
				*error = MB_ERROR_EOF;
			}
			...
			skip = 0;
			while (status == MB_SUCCESS && mbr_emoldraw_chk_label(verbose, mbio_ptr, *type) != MB_SUCCESS) {
				/* get next byte */
				label[0] = label[1];
				if ((read_len = fread(&label[1], 1, 1, mb_io_ptr->mbfp)) != 1) {
					status = MB_FAILURE;
					*error = MB_ERROR_EOF;
				}
				skip++;
...
/* swap bytes if necessary */
		...
		*type = (short int)mb_swap_short(*type);
		...
		if (status == MB_SUCCESS &&
		    (*type == EM_12S_BATH || *type == EM_12DP_BATH || *type == EM_12DS_BATH || *type == EM_121_BATH ||
		     *type == EM_1000_BATH || *type == EM_12S_SS || *type == EM_12DP_SS || *type == EM_12DS_SS || *type == EM_12S_SSP ||
		     *type == EM_12DP_SSP || *type == EM_12DS_SSP) &&
		    store->ping == NULL) {
		...
		if (status == MB_FAILURE && expect == EM_NONE) {
		...
		else if (*type != EM_START && *type != EM_STOP && *type != EM_PARAMETER && *type != EM_POS && *type != EM_SVP &&
		         *type != EM_12S_BATH && *type != EM_12DP_BATH && *type != EM_12DS_BATH && *type != EM_121_BATH &&
		         *type != EM_1000_BATH && *type != EM_12S_SS && *type != EM_12DP_SS && *type != EM_12DS_SS &&
		         *type != EM_12S_SSP && *type != EM_12DP_SSP && *type != EM_12DS_SSP) {
#ifdef MBR_EMOLDRAW_DEBUG
...
#endif
			store->sonar = MBSYS_SIMRAD_EM12S;
			status = mbr_emoldraw_rd_em12bath(verbose, mbfp, store, EM_SWATH_CENTER, error);
			if (status == MB_SUCCESS) {
				(*num_bathrec)++;
#ifdef MBR_EMOLDRAW_DEBUG
		...
		else if (*type == EM_12DP_BATH && expect != EM_NONE && expect != EM_12DP_BATH) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
...
#endif
			store->sonar = MBSYS_SIMRAD_EM12D;
			status = mbr_emoldraw_rd_em12bath(verbose, mbfp, store, EM_SWATH_PORT, error);
			if (status == MB_SUCCESS) {
				(*num_bathrec)++;
#ifdef MBR_EMOLDRAW_DEBUG
				fprintf(stderr, "num_bathrec:%d\n", *num_bathrec);
		...
		else if (*type == EM_12DS_BATH && expect != EM_NONE && expect != EM_12DS_BATH) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
...
#endif
			store->sonar = MBSYS_SIMRAD_EM12D;
			status = mbr_emoldraw_rd_em12bath(verbose, mbfp, store, EM_SWATH_STARBOARD, error);
			if (status == MB_SUCCESS) {
				(*num_bathrec)++;
#ifdef MBR_EMOLDRAW_DEBUG
				fprintf(stderr, "num_bathrec:%d\n", *num_bathrec);
		...
		else if (*type == EM_121_BATH && expect != EM_NONE && expect != EM_12S_BATH) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
			...
			*label_save_flag = MB_YES;
			...
			status = mbr_emoldraw_rd_em121bath(verbose, mbfp, store, error);
			if (status == MB_SUCCESS) {
				(*num_bathrec)++;
#ifdef MBR_EMOLDRAW_DEBUG
				fprintf(stderr, "num_bathrec:%d\n", *num_bathrec);
		...
		else if (*type == EM_1000_BATH && expect != EM_NONE && expect != EM_12S_BATH) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
...
#endif
			status = mbr_emoldraw_rd_em1000bath(verbose, mbfp, store, error);
			if (status == MB_SUCCESS) {
				(*num_bathrec)++;
#ifdef MBR_EMOLDRAW_DEBUG
				fprintf(stderr, "num_bathrec:%d\n", *num_bathrec);
		...
		else if (*type == EM_12S_SS && expect != EM_NONE && expect != EM_12S_SS) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
		...
		else if (*type == EM_12DS_SS && expect != EM_NONE && expect != EM_12DS_SS) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
		...
		else if (*type == EM_12DS_SSP && expect != EM_NONE && expect != EM_12DS_SS) {
#ifdef MBR_EMOLDRAW_DEBUG
			fprintf(stderr, "call nothing, expect %x but got type %x\n", expect, *type);
#endif
			store->kind = MB_DATA_DATA;
...
int mbr_rt_emoldraw(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS && store->kind == MB_DATA_DATA) {
		...
		/* get ping time */
		mb_fix_y2k(verbose, store->year, &ptime_i[0]);
...
int mbr_emoldraw_wr_start(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	if (status == MB_SUCCESS) {
		/* construct record */
		sprintf(line, "%2.2d%2.2d%2.2d,", store->par_day, store->par_month, store->par_year);
		sprintf(line + 7, "%2.2d%2.2d%2.2d%2.2d,", store->par_hour, store->par_minute, store->par_second, store->par_centisecond);
...
int mbr_emoldraw_wr_pos(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	if (status == MB_SUCCESS) {
		/* construct record */
		sprintf(line, "%2.2d%2.2d%2.2d,", store->pos_day, store->pos_month, store->pos_year);
		sprintf(line + 7, "%2.2d%2.2d%2.2d%2.2d,", store->pos_hour, store->pos_minute, store->pos_second, store->pos_centisecond);
		...
		sprintf(line + 39, "%11.1f,%9.1f,%2.2d,", store->utm_northing, store->utm_easting, store->utm_zone);
		...
		sprintf(line + 76, "%1.1d,%1.1d,%4.1f,%5.1f", store->utm_system, store->pos_quality, store->speed, store->line_heading);
...
int mbr_emoldraw_wr_ss(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	for (int i = 0; i < MBSYS_EM1000_MAXBEAMS; i++) {
		...
		if (new_datagram_size > 551 && i == MBSYS_EM1000_MAXBEAMS - 1) {
			...
			datagram_end[num_datagrams] = MBSYS_EM1000_MAXBEAMS - 1;
			datagram_size[num_datagrams] = 22 + 6 + ping->beam_samples[i];
			num_datagrams++;
		}
	...
	for (datagram = 0; datagram < num_datagrams; datagram++) {
		num_beams = datagram_end[datagram] - datagram_start[datagram] + 1;
...
int mbr_emoldraw_wr_ssp(int verbose, FILE *mbfp, struct mbsys_simrad_struct *store, int *error) {
	...
	for (int i = 0; i < MBSYS_SIMRAD_MAXBEAMS; i++) {
		...
		if (new_datagram_size > 1465 && i == MBSYS_SIMRAD_MAXBEAMS - 1) {
			...
			num_datagrams++;
			datagram_start[num_datagrams] = i;
			datagram_end[num_datagrams] = MBSYS_SIMRAD_MAXBEAMS - 1;
			datagram_size[num_datagrams] = 22 + 6 + 3 * ping->beam_samples[i];
			num_datagrams++;
		}
...
int mbr_emoldraw_wr_data(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	else if (store->kind == MB_DATA_DATA && (store->sonar == MBSYS_SIMRAD_EM12S || store->sonar == MBSYS_SIMRAD_EM12D)) {
#ifdef MBR_EMOLDRAW_DEBUG
	...
	else if (store->kind == MB_DATA_DATA && (store->sonar == MBSYS_SIMRAD_EM1000)) {
#ifdef MBR_EMOLDRAW_DEBUG
	...
	else if (store->kind == MB_DATA_DATA && (store->sonar == MBSYS_SIMRAD_EM121)) {
#ifdef MBR_EMOLDRAW_DEBUG
