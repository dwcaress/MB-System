		...
		if (store->kind == MB_DATA_COMMENT) {
			...
			if (store->gps_nsat > 0)
				sprintf(line, "%4.4d-%2.2d-%2.2dT%2.2d:%2.2d:%2.2d.%3.3dZ\t%11.6f\t%10.6f\t%d\t%d\t%.1f\t%d\n", store->time_i[0],
				        store->time_i[1], store->time_i[2], store->time_i[3], store->time_i[4], store->time_i[5],
				        1000 * store->time_i[6], store->longitude, store->latitude, store->gps_quality, store->gps_nsat,
				        store->gps_dilution, store->gps_height);
			else
				sprintf(line, "%4.4d-%2.2d-%2.2dT%2.2d:%2.2d:%2.2d.%3.3dZ\t%11.6f\t%10.6f\n", store->time_i[0], store->time_i[1],
				        store->time_i[2], store->time_i[3], store->time_i[4], store->time_i[5], 1000 * store->time_i[6],
...
int mbr_register_hir2rnav(int verbose, void *mbio_ptr, int *error) {
	...
	/* set format info parameters */
	const int status = mbr_info_hir2rnav(
