 ...
 *	$Id: mbsys_elac.c,v 4.3 1995-07-13 19:13:36 caress Exp $
 *
 *    Copyright (c) 1994 by 
 ...
 *   mbsys_elac_nav_get - extract navigation and attitude 
 *			    sensor data from
 *                          mbsys_elac_struct structure
 *   mbsys_elac_nav_put - insert navigation and attitude 
 *			    sensor data into
 *                          mbsys_elac_struct structure
...
int	*error;
{
 static char res_id[]="$Id: mbsys_elac.c,v 4.3 1995-07-13 19:13:36 caress Exp $";
...
/*--------------------------------------------------------------------*/
int mbsys_elac_nav_get(verbose,mbio_ptr,store_ptr,kind,
		time_i,time_d,navlon,navlat,speed,heading,
...
int	*error;
{
	char	*function_name = "mbsys_elac_nav_get";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_elac_struct *store;
	double	depthscale, dacrscale, daloscale,reflscale;
...
/*--------------------------------------------------------------------*/
int mbsys_elac_nav_put(verbose,mbio_ptr,store_ptr,
		time_i,time_d,navlon,navlat,speed,heading,
		roll,pitch,heave,error)
int	verbose;
char	*mbio_ptr;
...
int	*error;
{
	char	*function_name = "mbsys_elac_nav_put";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbsys_elac_struct *store;
	int	kind;
