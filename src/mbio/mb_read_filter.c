/*--------------------------------------------------------------------
 *    The MB-system:	mb_read_filter.c	2/8/2009
 ...
 *--------------------------------------------------------------------*/
/*
 * mb_read_filter.c reads swath data from a file that has been initialized 
 * by mb_read_init(). Any filtering set using mb_read_filter_add() is
 * applied by mb_read_filter() before returning.
 * Crosstrack distances are mapped into lon and lat.
 *
 * Author:	D. W. Caress
		...
		int *error)
{
	...
	char	*function_name = "mb_read_filter";
