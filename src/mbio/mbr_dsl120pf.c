...
int mbr_zero_dsl120pf(int verbose, char *data_ptr, int *error) {
	...
	struct mbf_dsl120pf_struct *data = (struct mbf_dsl120pf_struct *)data_ptr;
	...
	if (data != NULL) {
		...
		for (int i = 0; i < MBF_DSL120PF_COMMENT_LENGTH; i++)
			data->comment[i] = '\0';
	}
...
int mbr_dsl120pf_wr_amp(int verbose, void *mbio_ptr, FILE *mbfp, int *error) {
	...
	mb_put_binary_int(false, data->rec_hdr_len, &buffer[index]);
	index += 4;
	mb_put_binary_int(false, (int)0, &buffer[index]);
	index += 4;
	mb_put_binary_int(false, data->num_data_types, &buffer[index]);
	index += 4;
	mb_put_binary_int(false, data->ping, &buffer[index]);
	...
	mb_put_binary_float(false, data->roll, &buffer[index]);
	index += 4;
	mb_put_binary_float(false, (float)-0.1, &buffer[index]);
	index += 4;
	mb_put_binary_float(false, data->ang_offset, &buffer[index]);
	index += 4;
	mb_put_binary_int(false, data->transmit_pwr, &buffer[index]);
