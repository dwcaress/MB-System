...
int mbr_rt_oicgeoda(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		if ((read_size = fread(&buffer[4], 1, MBF_OICGEODA_HEADER_SIZE - 4, mb_io_ptr->mbfp)) != MBF_OICGEODA_HEADER_SIZE - 4) {
			status = MB_FAILURE;
			*error = MB_ERROR_EOF;
		}
	}
	...
	if (status == MB_SUCCESS && header->client_size > 0) {
		if ((read_size = fread(dataplus->client, 1, (int)(header->client_size), mb_io_ptr->mbfp)) == (int)(header->client_size)) {
	...
	if (status == MB_SUCCESS && header->num_chan > 0) {
		for (int i = 0; i < header->num_chan; i++) {
			...
			if (data_size > data->rawsize[i] || data->raw[i] == NULL) {
				if (data->raw[i] != NULL)
					status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->raw[i]), error);
...
/* byte swap the data if necessary */
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA) {
		...
		for (ichan = 0; ichan < header->num_chan; ichan++) {
			if (header->channel[ichan].type == OIC_TYPE_SIDESCAN) {
				if (header->channel[ichan].side == OIC_PORT && header->ss_chan_port == -1) {
					header->ss_chan_port = ichan;
					header->fish_altitude_samples = 2.0 * header->fish_altitude * header->channel[ichan].num_samples /
					                                header->sound_velocity / header->fish_ping_period;
					header->pixels_ss += header->channel[ichan].num_samples - MIN(header->fish_altitude_samples - 1, 0);
				}
				else if (header->channel[ichan].side == OIC_STARBOARD && header->ss_chan_stbd == -1) {
				...
				if (header->channel[ichan].side == OIC_PORT && header->bath_chan_port == -1) {
				...
				else if (header->channel[ichan].side == OIC_STARBOARD && header->bath_chan_stbd == -1) {
	...
	if (status == MB_SUCCESS && dataplus->kind == MB_DATA_DATA) {
		...
		if (header->beams_bath > data->beams_bath_alloc || data->bath == NULL) {
			...
			if (data->angle != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->angle), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(float), (void **)&(data->bath), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(float),
			                    (void **)&(data->bathacrosstrack), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(float), (void **)&(data->bathalongtrack),
			                    error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(float), (void **)&(data->tt), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_bath * sizeof(float), (void **)&(data->angle), error);
		}
		if (header->beams_amp > data->beams_amp_alloc || data->amp == NULL) {
			data->beams_amp_alloc = header->beams_amp;
			if (data->amp != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->amp), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->beams_amp * sizeof(float), (void **)&(data->amp), error);
		}
		if (header->pixels_ss > data->pixels_ss_alloc || data->ss == NULL) {
			...
			if (data->ssalongtrack != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->ssalongtrack), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->pixels_ss * sizeof(float), (void **)&(data->ss), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, header->pixels_ss * sizeof(float), (void **)&(data->ssacrosstrack),
			                    error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, header->pixels_ss * sizeof(float), (void **)&(data->ssalongtrack), error);
		}
		...
		if (header->bath_chan_port >= 0 && header->channel[header->bath_chan_port].type == OIC_TYPE_ANGLE &&
			...
			status = mb_rollpitch_to_takeoff(verbose, alpha, beta, &theta, &phi, error);
			xx = rr * sin(DTR * theta);
			zz = rr * cos(DTR * theta);
			data->bathacrosstrack[j] = xx * cos(DTR * phi);
			data->bathalongtrack[j] = xx * sin(DTR * phi);
		...
		if (header->bath_chan_port >= 0) {
			...
			if (header->channel[ichan].size == OIC_SIZE_3FLOAT) {
				...
				dx = header->fish_range / header->channel[ichan].num_samples;
			...
			for (int i = 0; i < header->channel[ichan].num_samples; i++) {
				if (header->channel[ichan].size == OIC_SIZE_3FLOAT) {
					...
					rr = 0.5 * header->sound_velocity * float_ptr[3 * i];
		...
		if (header->ss_chan_port >= 0) {
          ...
          if (int_ptr[i] != 0)
            ...
            data->ss[j] = MB_SIDESCAN_NULL;
				...
				alpha = header->fish_pitch;
				rr = 0.5 * header->sound_velocity * sample_interval * i;
				status = mb_rollpitch_to_takeoff(verbose, alpha, beta, &theta, &phi, error);
				xx = rr * sin(DTR * theta);
				zz = rr * cos(DTR * theta);
		...
		if (header->ss_chan_stbd >= 0) {
			...
			for (int i = header->fish_altitude_samples + 1; i < header->channel[ichan].num_samples; i++) {
				...
				if (header->channel[ichan].size == OIC_SIZE_CHAR)
					...
					data->ss[j] = float_ptr[i];
				beta = asin(((double)header->fish_altitude_samples) / ((double)i)) / DTR;
				alpha = header->fish_pitch;
				rr = 0.5 * header->sound_velocity * sample_interval * i;
				status = mb_rollpitch_to_takeoff(verbose, alpha, beta, &theta, &phi, error);
				xx = rr * sin(DTR * theta);
	...
	if (status == MB_SUCCESS && store != NULL) {
		...
		for (int i = 0; i < store->num_chan; i++) {
			...
			/* allocate data if needed */
			if (data->rawsize[i] > store->rawsize[i] || store->raw[i] == NULL) {
				if (store->raw[i] != NULL)
		...
		/* depths and sidescan */
		if (header->beams_bath > store->beams_bath_alloc || store->beamflag == NULL || store->bath == NULL ||
		...
		if (header->beams_amp > store->beams_amp_alloc || store->amp == NULL) {
			store->beams_amp_alloc = header->beams_amp;
			if (store->amp != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(store->amp), error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, store->beams_amp_alloc * sizeof(float), (void **)&(store->amp), error);
		}
		if (header->pixels_ss > store->pixels_ss_alloc || store->ss == NULL || store->ssacrosstrack == NULL ||
...
int mbr_wt_oicgeoda(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (store != NULL) {
		...
		for (int i = 0; i < header->num_chan; i++) {
			...
			/* allocate data if needed */
			if (store->rawsize[i] > data->rawsize[i] || data->raw[i] == NULL) {
				if (data->raw[i] != NULL)
					status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->raw[i]), error);
		...
		/* depths and sidescan */
		if (header->beams_bath > data->beams_bath_alloc || data->bath == NULL || data->bathacrosstrack == NULL ||
		    data->bathalongtrack == NULL || data->tt == NULL || data->angle == NULL) {
			data->beams_bath_alloc = header->beams_bath;
			if (data->bath != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->bath), error);
			if (data->bathacrosstrack != NULL)
			...
			if (data->angle != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->angle), error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(float), (void **)&(data->bath), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(float),
			                    (void **)&(data->bathacrosstrack), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(float),
			                    (void **)&(data->bathalongtrack), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(float), (void **)&(data->tt), error);
			status =
			    mb_mallocd(verbose, __FILE__, __LINE__, data->beams_bath_alloc * sizeof(float), (void **)&(data->angle), error);
		}
		if (header->beams_amp > data->beams_amp_alloc || data->amp == NULL) {
			data->beams_amp_alloc = header->beams_amp;
			if (data->amp != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->amp), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, data->beams_amp_alloc * sizeof(float), (void **)&(data->amp), error);
		}
		if (header->pixels_ss > data->pixels_ss_alloc || data->ss == NULL || data->ssacrosstrack == NULL ||
		    data->ssalongtrack == NULL) {
			data->pixels_ss_alloc = header->pixels_ss;
			if (data->ss != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->ss), error);
			...
			if (data->ssalongtrack != NULL)
				status = mb_freed(verbose, __FILE__, __LINE__, (void **)&(data->ssalongtrack), error);
			status = mb_mallocd(verbose, __FILE__, __LINE__, data->pixels_ss_alloc * sizeof(float), (void **)&(data->ss), error);
	...
	if (status == MB_SUCCESS) {
		...
		index += 1;
		buffer[index] = header->fish_status;
		index += 1;
		buffer[index] = header->nav_used;
		index += 1;
		...
		for (int i = 0; i < MBF_OICGEODA_MAX_CHANNELS; i++) {
			...
			index += 1;
			buffer[index] = header->channel[i].size;
			index += 1;
	...
	if ((write_size = fwrite(buffer, 1, MBF_OICGEODA_HEADER_SIZE, mb_io_ptr->mbfp)) == MBF_OICGEODA_HEADER_SIZE) {
		status = MB_SUCCESS;
		*error = MB_ERROR_NO_ERROR;
	}
	else {
	...
	if (status == MB_SUCCESS && header->client_size > 0) {
		if ((write_size = fwrite(dataplus->client, 1, (int)(header->client_size), mb_io_ptr->mbfp)) !=
