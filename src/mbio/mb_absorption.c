...
int mb_seabird_density(int verbose, double salinity, double temperature, double pressure, double *density, int *error) {
	...
	double k, kw, aw, bw;
	double val;
	double B0 = 8.24493e-1, B1 = -4.0899e-3, B2 = 7.6438e-5, B3 = -8.2467e-7, B4 = 5.3875e-9;
	double C0 = -5.72466e-3, C1 = 1.0227e-4, C2 = -1.6546e-6;
	double D0 = 4.8314e-4;
	double A0 = 999.842594, A1 = 6.793952e-2, A2 = -9.095290e-3, A3 = 1.001685e-4;
	double A4 = -1.120083e-6, A5 = 6.536332e-9;
	double FQ0 = 54.6746, FQ1 = -0.603459, FQ2 = 1.09987e-2, FQ3 = -6.1670e-5;
	double G0 = 7.944e-2, G1 = 1.6483e-2, G2 = -5.3009e-4;
	double i0 = 2.2838e-3, i1 = -1.0981e-5, i2 = -1.6078e-6;
	double J0 =1.91075e-4;
	double M0 = -9.9348e-7, M1 = 2.0816e-8, M2 = 9.1697e-10;
	double E0 = 19652.21, E1 = 148.4206, E2 = -2.327105, E3 = 1.360477e-2, E4 = -5.155288e-5;
	double H0 = 3.239908, H1 = 1.43713e-3, H2 = 1.16092e-4, H3 = -5.77905e-7;
	double K0 = 8.50935e-5, K1 =-6.12293e-6, K2 = 5.2787e-8;
	...
	*density = A0 + A1 * temperature + A2 * t2 + A3 * t3 + A4 * t4 + A5 * t5
	...
	kw = E0 + E1 * temperature + E2 * t2 + E3 * t3 + E4 * t4;
	aw = H0 + H1 * temperature + H2 * t2 + H3 * t3;
	bw = K0 + K1 * temperature + K2 * t2;
	k = kw + (FQ0 + FQ1 * temperature + FQ2 * t2 + FQ3 * t3) * salinity
...
int mb_seabird_salinity(int verbose, double conductivity, double temperature, double pressure, double *salinity, int *error) {
	...
	double C0 = 6.766097e-1, C1 = 2.00564e-2, C2 = 1.104259e-4, C3 = -6.9698e-7, C4 = 1.0031e-9;
	...
	if (conductivity <= 0.0)
		...
		val = 1 + B1 * temperature + B2 * temperature * temperature + B3 * R + B4 * R * temperature;
		if (val)
...
int mb_seabird_soundspeed(int verbose, int algorithm, double salinity,
	...
	/* algorithm 2: Wilson JASA, 1960, 32, 1357  */
		...
		double pr, sd, a, v0, v1;
		pr = 0.1019716 * (pressure + 10.1325);
		sd = salinity - 35.0;
		a = (((7.9851e-6 * temperature - 2.6045e-4) * temperature - 4.4532e-2) * temperature + 4.5721) * temperature + 1449.14;
		*soundspeed = (7.7711e-7 * temperature - 1.1244e-2) * temperature + 1.39799;
		v0 = (1.69202e-3 * sd + *soundspeed) * sd + a;
	...
	/* algorithm 3 (default): Delgrosso JASA, Oct. 1974, Vol 56, No 4 */
		...
		double c000, dct, dcs, dcp, dcstp;
		c000 = 1402.392;
		pressure = pressure / 9.80665; /* convert pressure from decibars to KG / CM**2 */
		dct = (0.501109398873e1 - (0.550946843172e-1 - 0.22153596924e-3 * temperature) * temperature) * temperature;
		dcs = (0.132952290781e1 + 0.128955756844e-3 * salinity) * salinity;
		dcp = (0.156059257041e0 + (0.244998688441e-4 - 0.83392332513e-8 * pressure) * pressure) * pressure;
		dcstp =  -0.127562783426e-1 * temperature * salinity
