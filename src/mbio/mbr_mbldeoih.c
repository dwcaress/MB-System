...
int mbr_register_mbldeoih(int verbose, void *mbio_ptr, int *error)
{
	...
	if (verbose >= 2)
		...
		fprintf(stderr,"dbg2       dimensions:         %lu\n",(size_t)mb_io_ptr->mb_io_dimensions);
		fprintf(stderr,"dbg2       sidescantype:       %lu\n",(size_t)mb_io_ptr->mb_io_sidescantype);
...
int mbr_rt_mbldeoih(int verbose, void *mbio_ptr, void *store_ptr, int *error)
{
	...
	if (status == MB_SUCCESS && store->kind == MB_DATA_DATA)
		...
		if (version == 4)
			...
			store->spare2 = buffer[index]; index++;
...
int mbr_wt_mbldeoih(int verbose, void *mbio_ptr, void *store_ptr, int *error)
{
	...
	if (status == MB_SUCCESS && store->kind == MB_DATA_DATA)
		...
		/* otherwise write curent version file */
			...
			buffer[index] = store->spare2; index++;
