...
int mbr_rt_samesurf(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		for (int i = 0; i < MIN(MBSYS_SURF_MAXCPOS, store->NrPositionsensors); i++) {
			...
			if (CenterPositionPtr != NULL) {
				...
				if (mb_io_ptr->projection_initialized) {
					...
					mb_proj_inverse(verbose, mb_io_ptr->pjptr, easting, northing, &lon, &lat, error);
					store->CenterPosition[i].centerPositionX = (float)(DTR * lon - store->GlobalData.referenceOfPositionX);
					store->CenterPosition[i].centerPositionY = (float)(DTR * lat - store->GlobalData.referenceOfPositionY);
				}
			}
		}
		...
		store->NrTxSets = 0;
		MultibeamTransmitterParametersPtr = SAPI_getMultibeamTransmitterParameters(&store->NrTxSets);
	...
	if (verbose >= 4 && store->kind == MB_DATA_DATA) {
		...
		for (int i = 0; i < MIN(MBSYS_SURF_MAXCPOS, store->NrPositionsensors); i++) {
			...
			fprintf(stderr, "dbg4       CenterPosition[%2d].centerPositionX:         %f\n", i,
			        store->CenterPosition[i].centerPositionX);
			fprintf(stderr, "dbg4       CenterPosition[%2d].centerPositionY:         %f\n", i,
			        store->CenterPosition[i].centerPositionY);
