	...
	if (datalist->open == MB_YES && done == MB_NO) {
		while (done == MB_NO) {
			...
			if (datalist->datalist == NULL) {
				...
				while (rdone == MB_NO) {
					...
					/* get filename */
						...
						/* read datalist item */
						nscan = sscanf(&(buffer[istart]), "%s %d %lf %lf/%lf/%lf/%lf", path, format, weight, 
                                       file_bounds, &file_bounds[1], &file_bounds[2], &file_bounds[3]);
			...
			if (done == MB_NO && datalist->open == MB_YES && datalist->datalist != NULL) {
				...
				if (datalist2->open == MB_YES) {
					/* recursively call mb_read_datalist */
					status = mb_datalist_read3(verbose, (void *)datalist->datalist, pstatus, path, ppath, dpath, format, weight,
					                           has_bounds, file_bounds, error);
