...
int mbsys_templatesystem_pingnumber(int verbose, void *mbio_ptr, unsigned int *pingnumber, int *error) {
	...
	/* get data structure pointer */
	struct mbsys_templatesystem_struct *store = (struct mbsys_templatesystem_struct *)mb_io_ptr->store_data;
	...
	*kind = store->kind;
	...
	if (*kind == MB_DATA_DATA) {
		...
		*nss = store->number_pixels;
		for (int i = 0; i < store->number_pixels; i++) {
...
int mbsys_templatesystem_insert_svp(int verbose, void *mbio_ptr, void *store_ptr, int nsvp, double *depth, double *velocity,
	...
	if (store->kind == MB_DATA_VELOCITY_PROFILE) {
		...
		if (status == MB_SUCCESS) {
			...
			for (int i = 0; i < store->number_svp; i++) {
...
int mbsys_templatesystem_copy(int verbose, void *mbio_ptr, void *store_ptr, void *copy_ptr, int *error) {
	...
	struct mbsys_templatesystem_struct *store = (struct mbsys_templatesystem_struct *)store_ptr;
	struct mbsys_templatesystem_struct *copy = (struct mbsys_templatesystem_struct *)copy_ptr;
