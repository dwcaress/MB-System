...
int mbr_info_hsmdldih(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = 319;
	...
	strncpy(format_description,
	        "Format name:          MBF_HSMDLDIH\nInformal Description: Atlas HSMD medium depth multibeam processed "
...
int mbr_hsmdldih_rd_data(int verbose, void *mbio_ptr, int *error) {
	...
	if (status == MB_SUCCESS && data->transid != MBF_HSMDLDIH_COM) {
		(*Header_count)++;
		if (*Header_count == 1)
...
int mbr_rt_hsmdldih(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	if (status == MB_SUCCESS && data->kind == MB_DATA_DATA && data->lon == 0.0 && data->lat == 0.0 && mb_io_ptr->nfix >= 1) {
		time_i[0] = data->year;
		time_i[1] = data->month;
		time_i[2] = data->day;
		time_i[3] = data->hour;
...
int mbr_wt_hsmdldih(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	...
	const int status = mbr_hsmdldih_wr_data(verbose, mbio_ptr, data_ptr, error);
...
int mbr_register_hsmdldih(int verbose, void *mbio_ptr, int *error) {
	...
	/* set format info parameters */
	const int status = mbr_info_hsmdldih(
