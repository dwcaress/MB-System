...
int mbr_info_cbat8101(int verbose, int *system, int *beams_bath_max, int *beams_amp_max, int *pixels_ss_max, char *format_name,
	...
	*pixels_ss_max = 0;
	...
	strncpy(format_description,
	        ...
	        "       101 beam bathymetry and amplitude,\n                      binary, SeaBeam Instruments.\n",
...
int mbr_zero_cbat8101(int verbose, void *data_ptr, int *error) {
	...
	if (data != NULL) {
		...
		for (int i = 0; i < MBF_CBAT8101_COMMENT_LENGTH; i++)
			data->comment[i] = '\0';
	...
	if (status == MB_SUCCESS) {
		...
		if (!swap) {
			...
			for (int i = 0; i < data->beams_bath; i++) {
				...
				short_ptr = ((short *)beamarray) + 1;
				data->bath_acrosstrack[i] = (short)mb_swap_short(*short_ptr);
				short_ptr = ((short *)beamarray) + 2;
				data->bath_alongtrack[i] = (short)mb_swap_short(*short_ptr);
				short_ptr = ((short *)beamarray) + 3;
	...
	*error = MB_ERROR_NO_ERROR;
	...
	while (!done) {
		...
		if (status == MB_FAILURE) {
			...
			status = mbr_cbat8101_rd_comment(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_COMMENT;
			}
			...
			status = mbr_cbat8101_rd_parameter(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_PARAMETER;
			}
			...
			status = mbr_cbat8101_rd_nav(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_NAV;
			}
			...
			status = mbr_cbat8101_rd_svp(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_VELOCITY_PROFILE;
			}
			...
			status = mbr_cbat8101_rd_bath(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_DATA;
			}
			...
			status = mbr_cbat8101_rd_short_svp(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_VELOCITY_PROFILE;
			}
			...
			status = mbr_cbat8101_rd_heading(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_HEADING;
			}
			...
			status = mbr_cbat8101_rd_attitude(verbose, mbfp, mb_io_ptr->byteswapped, data, error);
			if (status == MB_SUCCESS) {
				done = true;
				data->kind = MB_DATA_ATTITUDE;
			}
...
int mbr_cbat8101_wr_comment(int verbose, FILE *mbfp, int swap, void *data_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		...
		for (int i = len; i < MBF_CBAT8101_COMMENT_LENGTH; i++)
			line[i] = '\0';
	...
	if (status == MB_SUCCESS) {
		...
		for (int i = 0; i < data->svp_num; i++) {
			...
			if (!swap) {
				...
				*short_ptr = (short)mb_swap_short((short)data->svp_depth[i]);
		...
		if (!swap) {
			...
			for (int i = 0; i < MBF_CBAT8101_MAXBEAMS; i++) {
				...
				*short_ptr = (short)mb_swap_short((short)data->bath[i]);
				short_ptr = ((short *)beamarray) + 1;
				*short_ptr = (short)mb_swap_short((short)data->bath_acrosstrack[i]);
				short_ptr = ((short *)beamarray) + 2;
				*short_ptr = (short)mb_swap_short((short)data->bath_alongtrack[i]);
				short_ptr = ((short *)beamarray) + 3;
				*short_ptr = (short)mb_swap_short((short)data->tt[i]);
				short_ptr = ((short *)beamarray) + 4;
				*short_ptr = (short)mb_swap_short((short)data->angle[i]);
				char_ptr = (unsigned char *)(beamarray + 10);
				*char_ptr = (char)data->quality[i];
				char_ptr = (unsigned char *)(beamarray + 11);
				*char_ptr = (unsigned char)data->amp[i];
