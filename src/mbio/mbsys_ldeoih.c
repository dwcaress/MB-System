...
int mbsys_ldeoih_alloc(int verbose, void *mbio_ptr, void **store_ptr, int *error) {
	...
	status = mb_mallocd(verbose, __FILE__, __LINE__, sizeof(struct mbsys_ldeoih_struct), (void **)store_ptr, error);
		...
		if (namp > store->beams_amp_alloc) {
			...
			if (store != NULL) {
				if (store->amp != NULL)
					status = mb_freed(verbose, __FILE__, __LINE__, (void **)&store->amp, error);
				status =
				    mb_mallocd(verbose, __FILE__, __LINE__, store->beams_amp_alloc * sizeof(short), (void **)&store->amp, error);
		...
		if (nss > store->pixels_ss_alloc) {
			...
			if (store != NULL) {
				...
				if (store->ss_alongtrack != NULL)
					status = mb_freed(verbose, __FILE__, __LINE__, (void **)&store->ss_alongtrack, error);
				status =
				    mb_mallocd(verbose, __FILE__, __LINE__, store->pixels_ss_alloc * sizeof(short), (void **)&store->ss, error);
