...
int mbr_alm_hsds2raw(int verbose, void *mbio_ptr, int *error) {
	...
	if (status == MB_SUCCESS) {
		if (strlen(mb_io_ptr->file) >= 5 && strncmp(&(mb_io_ptr->file[strlen(mb_io_ptr->file) - 4]), ".fsw", 4) == 0) {
			strcpy(mb_io_ptr->file2, mb_io_ptr->file);
			strcpy(&(mb_io_ptr->file2[strlen(mb_io_ptr->file) - 4]), ".nav");
		}
		if (strlen(mb_io_ptr->file) >= 5 && strncmp(&(mb_io_ptr->file[strlen(mb_io_ptr->file) - 4]), ".fsw", 4) == 0) {
			strcpy(mb_io_ptr->file3, mb_io_ptr->file);
			strcpy(&(mb_io_ptr->file3[strlen(mb_io_ptr->file) - 4]), ".ang");
		}
	}
...
int mbr_hsds2raw_rd_data(int verbose, void *mbio_ptr, void *store_ptr, int *error) {
	bool xdr_status;
	int read_status;  // TODO(schwehr): bool?
	int nskip;
	int length = 0;
	int telegram_id = 0;
	int telegram_cnt = 0;
	...
	double telegram_utc_time_d = 0.0;
	double telegram_loc_time_d = 0.0;
	int telegram_block_no = 0;
	int telegram_block_cnt = 0;
	int telegram_max_no = 0;
	int telegram_act_no = 0;
	int telegram_data_status = 0;
	int telegram_sensor_status = 0;
	int tt_max_lead_cnt = 0;
	int tt_act_lead_cnt = 0;
	int ss_act_side_cnt = 0;
	...
	*error = MB_ERROR_NO_ERROR;
	...
	if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_START) {
		if (xdr_status)
	...
	store->tt_beam_cnt = 0;
	bool done = false;
	while (xdr_status && !done) {
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &telegram_id);

		/* telegram id ok - just read send and receive strings */
		if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_TRAVELTIMES) {
			if (xdr_status)
				xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &telegram_cnt);
			if (xdr_status)
				xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
		...
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
		if (xdr_status)
			xdr_status = xdr_opaque((XDR *)mb_io_ptr->xdrs, (char *)store->tt_lamplitude, MBSYS_ATLAS_MAXBEAMTELEGRAM);
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
		if (xdr_status)
			xdr_status = xdr_opaque((XDR *)mb_io_ptr->xdrs, (char *)store->tt_lstatus, MBSYS_ATLAS_MAXBEAMTELEGRAM);

		/* set done if done */
		if (xdr_status != true || telegram_act_no == telegram_max_no)
			done = true;
	...
	while (xdr_status && !done) {
		...
		/* telegram id ok - just read send and receive strings */
		if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_SIDESCAN) {
			if (xdr_status)
				xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &telegram_cnt);
			if (xdr_status)
				xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
	...
	/* telegram id ok - just read send and receive strings */
	if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_TRACKINGWINDOWS) {
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &telegram_cnt);
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
	...
	/* telegram id ok - just read send and receive strings */
	if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_BACKSCATTER) {
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &telegram_cnt);
		if (xdr_status)
			xdr_status = xdr_int((XDR *)mb_io_ptr->xdrs, &length);
	...
	if (status == MB_SUCCESS && (store->tt_beam_cnt > MBSYS_ATLAS_MAXBEAMS || store->ss_max_side_bb_cnt > MBSYS_ATLAS_MAXPIXELS ||
	...
	if ((XDR *)mb_io_ptr->xdrs2 != NULL &&
		...
		while (!done) {
			...
			/* telegram id ok - just read send and receive strings */
			if (xdr_status && telegram_id == MBSYS_ATLAS_TELEGRAM_SYSTEM) {
				if (xdr_status)
			...
			/* add to nav fix list */
			if (xdr_status) {
				mb_navint_add(verbose, mbio_ptr, sys_pos_time, (RTD * sys_pos_lon), (RTD * sys_pos_lat), error);
				if (mb_io_ptr->fix_time_d[mb_io_ptr->nfix - 1] >= store->tt_transmit_time_d)
					done = true;
			}
			else
				done = true;
	...
	if (mb_io_ptr->nfix > 0) {
		mb_navint_interp(verbose, mbio_ptr, store->tt_transmit_time_d, store->start_heading, 0.0, &(store->pr_navlon),
		                 &(store->pr_navlat), &pspeed, error);
