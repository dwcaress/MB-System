 ...
 *	$Id: mbr_mr1prhig.c,v 4.10 1998-10-05 17:46:15 caress Exp $
 *
 *    Copyright (c) 1994 by 
...
int	*error;
{
	static char res_id[]="$Id: mbr_mr1prhig.c,v 4.10 1998-10-05 17:46:15 caress Exp $";
	...
	/* else check for ping data to be copied from mb_io_ptr */
		...
		for (i=pixel_center-1;i>-1;i--)
			...
			if (mb_io_ptr->new_ss[i] != 0)
				{
				data->ss_port[data->port_sscount] 
					= mb_io_ptr->new_ss[i];
				data->port_sscount++;
				}
			}
		...
		for (i=pixel_center+1;i<mb_io_ptr->pixels_ss;i++)
			...
			if (mb_io_ptr->new_ss[i] != 0)
				{
				data->ss_stbd[data->stbd_sscount] 
					= mb_io_ptr->new_ss[i];
				data->stbd_sscount++;
				}
			}
