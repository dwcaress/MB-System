 ...
 *	$Id: mbr_mr1prhig.c,v 5.1 2001-01-22 07:43:34 caress Exp $
 *
...
int mbr_info_mr1prhig(int verbose, 
			...
			int *error)
{
	static char res_id[]="$Id: mbr_mr1prhig.c,v 5.1 2001-01-22 07:43:34 caress Exp $";
	...
	*format_alloc = &mbr_alm_mr1prhig;
	*format_free = &mbr_dem_mr1prhig; 
	*store_alloc = &mbsys_mr1_alloc; 
	*store_free = &mbsys_mr1_deall; 
	*read_ping = &mbr_rt_mr1prhig; 
	*write_ping = &mbr_wt_mr1prhig; 
...
int mbr_alm_mr1prhig(int verbose, char *mbio_ptr, int *error)
{
	...
	mbr_zero_mr1prhig(verbose,data_ptr,error);
...
int mbr_zero_mr1prhig(int verbose, char *data_ptr, int *error)
{
	...
	data = (struct mbf_mr1prhig_struct *) data_ptr;
...
int mbr_wt_mr1prhig(int verbose, char *mbio_ptr, char *store_ptr, int *error)
{
	...
	status = mbr_mr1prhig_wr_data(verbose,mbio_ptr,data_ptr,error);
