 ...
 * multibeam data in the SB2120XS format.  
 * These functions include:
 *   mbr_alm_sb2120xs	- allocate read/write memory
 *   mbr_dem_sb2120xs	- deallocate read/write memory
 *   mbr_rt_sb2120xs	- read and translate data
 *   mbr_wt_sb2120xs	- translate and write data
 *
 * Author:	P. A. Cohen
 * Date:	March 27, 2000
 * Author:	D. W. Caress
...
#include "../../include/mbsys_sb2120.h"
#include "../../include/mbf_sb2120xs.h"
...
int mbr_register_sb2120xs(int verbose, char *mbio_ptr, 
		int *error);
int mbr_info_sb2120xs(int verbose, 
			int *system, 
			int *beams_bath_max, 
			int *beams_amp_max, 
			int *pixels_ss_max, 
...
int mbr_dem_sb2120xs(int verbose, char *mbio_ptr, int *error);
int mbr_rt_sb2120xs(int verbose, char *mbio_ptr, char *store_ptr, int *error);
int mbr_wt_sb2120xs(int verbose, char *mbio_ptr, char *store_ptr, int *error);
...
int mbr_register_sb2120xs(int verbose, char *mbio_ptr, int *error)
{
	...
	status = mbr_info_sb2120xs(verbose, 
			&mb_io_ptr->system, 
	...
	mb_io_ptr->mb_io_format_alloc = &mbr_alm_sb2120xs;
	mb_io_ptr->mb_io_format_free = &mbr_dem_sb2120xs; 
	mb_io_ptr->mb_io_store_alloc = &mbsys_sb2120_alloc; 
	...
	mb_io_ptr->mb_io_insert_nav = &mbsys_sb2120_insert_nav; 
	mb_io_ptr->mb_io_extract_altitude = &mbsys_sb2120_extract_altitude; 
	mb_io_ptr->mb_io_insert_altitude = NULL; 
	mb_io_ptr->mb_io_extract_svp = &mbsys_sb2120_extract_svp; 
	mb_io_ptr->mb_io_insert_svp = &mbsys_sb2120_insert_svp; 
	mb_io_ptr->mb_io_ttimes = &mbsys_sb2120_ttimes; 
	mb_io_ptr->mb_io_copyrecord = &mbsys_sb2120_copy; 
			...
			int *error)
{
	...
	*system = MB_SYS_SB2120;
	*beams_bath_max = 151;
	*beams_amp_max = 151;
	*pixels_ss_max = 2000;
	strncpy(format_name, "SB2120XS", MB_NAME_LENGTH);
	strncpy(system_name, "SB2120", MB_NAME_LENGTH);
	strncpy(format_description, "Format name:          MBF_SB2120XS\nInformal Description: SeaBeam 2120 series vendor format\nAttributes:           SeaBeam 2120, 20 KHz, XSE, bathymetry, amplitude \n                      and sidescan, 151 beams and 2000 pixels, ascii \n                      with binary sidescan, SeaBeam Instruments.\n", MB_DESCRIPTION_LENGTH);
	*numfile = 1;
...
int mbr_alm_sb2120xs(int verbose, char *mbio_ptr, int *error)
{
	...
	mb_io_ptr->structure_size = sizeof(struct mbf_sb2120xs_struct);
	mb_io_ptr->data_structure_size = 0;
	status = mb_malloc(verbose,mb_io_ptr->structure_size,
				&mb_io_ptr->raw_data,error);
	status = mb_malloc(verbose,MBF_SB2120XS_BUFFER_SIZE,
				&mb_io_ptr->hdr_comment,error);
	mbsys_sb2120_alloc(verbose,mbio_ptr,
				&mb_io_ptr->store_data,error);

	/* initialize everything to zeros */
	mbr_zero_sb2120xs(verbose,mb_io_ptr->raw_data,error);
...
int mbr_dem_sb2120xs(int verbose, char *mbio_ptr, int *error)
{
	char	*function_name = "mbr_dem_sb2120xs";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
...
/*--------------------------------------------------------------------*/
int mbr_zero_sb2120xs(int verbose, char *data_ptr, int *error)
{
	char	*function_name = "mbr_zero_sb2120xs";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	i, j;
	...
	data = (struct mbf_sb2120xs_struct *) data_ptr;
	...
	if (data != NULL)
		...
		for (i=0;i<MBF_SB2120XS_MAXSVP;i++)
		    {
		...
		for (i=0;i<MBF_SB2120XS_DESCRIPTION_LENGTH;i++)
		...
		for (i=0;i<MBF_SB2120XS_MAXBEAMS;i++)
		    {
		    data->beams[i].tt = 0.0;
		...
		for (i=0;i<MBF_SB2120XS_MAXPIXELS;i++)
		    data->ss[i] = 0; /* sidescan amplitude in dB */
	
		/* comment */
		for (i=0;i<MBF_SB2120XS_COMMENT_LENGTH;i++)
		    data->comment[i] = 0;
		}
...
int mbr_rt_sb2120xs(int verbose, char *mbio_ptr, char *store_ptr, int *error)
{
	char	*function_name = "mbr_rt_sb2120xs";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_sb2120xs_struct *data;
	...
	mb_io_ptr = (struct mb_io_struct *) mbio_ptr;
	data = (struct mbf_sb2120xs_struct *) mb_io_ptr->raw_data;
	store = (struct mbsys_sb2120_struct *) store_ptr;

	/* read next data from file */
	status = mbr_sb2120xs_rd_data(verbose,mbio_ptr,error);
...
int mbr_wt_sb2120xs(int verbose, char *mbio_ptr, char *store_ptr, int *error)
{
	char	*function_name = "mbr_wt_sb2120xs";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_sb2120xs_struct *data;
	char	*data_ptr;
	...
	status = mbr_sb2120xs_wr_data(verbose,mbio_ptr,data_ptr,error);
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_data(int verbose, char *mbio_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_data";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_sb2120xs_struct *data;
	char	*data_ptr;
		...
		*frame_save = MB_NO;
		...
		if (status == MB_SUCCESS)
			...
			*error = MB_ERROR_EOF;
			...
			if (buffer_size > 0
			    && buffer_size <= MBF_SB2120XS_BUFFER_SIZE - 4)
			    buffer_size += 4;
			else
			    {
			    status = MB_FAILURE;
...
fprintf(stderr, "frame byte count:%d\n", buffer_size);
*/
		if (frame_id == MBF_SB2120XS_NAV_FRAME)
		    {
/*fprintf(stderr, "READ NAV\n");*/
		    /* read extra 32 bytes to handle bug in Hydrostar software */
		    /* comment out on 4/18/2000 by PAC , XSE 1.6 */
		    if (buffer_size < 124)
			{
		    ...
		    status = mbr_sb2120xs_rd_nav(
				    verbose,buffer_size,
				    buffer,data,error);
		    done = MB_YES;
		    }
		else if (frame_id == MBF_SB2120XS_SVP_FRAME)
		    {
/*fprintf(stderr, "READ SVP\n");*/
		    data->kind = MB_DATA_VELOCITY_PROFILE;
		    status = mbr_sb2120xs_rd_svp(
				    verbose,buffer_size,
				    buffer,data,error);
		    done = MB_YES;
		    }
		else if (frame_id == MBF_SB2120XS_SHP_FRAME)
		    {
/*fprintf(stderr, "READ PARAMETER\n");*/
		    data->kind = MB_DATA_PARAMETER;
		    status = mbr_sb2120xs_rd_ship(
				    verbose,buffer_size,
				    buffer,data,error);
		    done = MB_YES;
		    }
		else if (frame_id == MBF_SB2120XS_COM_FRAME)
		    {
/*fprintf(stderr, "READ COMMENT\n");*/
		    data->kind = MB_DATA_COMMENT;
		    status = mbr_sb2120xs_rd_comment(
				    verbose,buffer_size,
				    buffer,data,error);
		    done = MB_YES;
		    }
		else if (frame_id == MBF_SB2120XS_SSN_FRAME)
		    {
/*fprintf(stderr, "READ SIDESCAN\n");*/
		    data->kind = MB_DATA_DATA;
		    status = mbr_sb2120xs_rd_sidescan(
				    verbose,buffer_size,
				    buffer,data,error);
		    data->sid_frame = MB_YES;
		    *frame_expect = MBF_SB2120XS_MBM_FRAME;
		    done = MB_NO;
		    }
		else if (frame_id == MBF_SB2120XS_MBM_FRAME)
		    {
/*fprintf(stderr, "READ MULTIBEAM\n");*/
		    data->kind = MB_DATA_DATA;
		    status = mbr_sb2120xs_rd_multibeam(
				    verbose,buffer_size,
				    buffer,data,error);
		    data->mul_frame = MB_YES;
		    if (frame_id == *frame_expect)
			*frame_expect = MBF_SB2120XS_NONE_FRAME;
		    done = MB_YES;
		    }
		else if (frame_id == MBF_SB2120XS_MBM_FRAME)
		    {
/*fprintf(stderr, "READ OTHER\n");*/
		...
		else if (*frame_expect != MBF_SB2120XS_NONE_FRAME
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_nav(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_nav";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
	    ...
	    if (done == MB_NO
		&& group_id == MBF_SB2120XS_NAV_GROUP_GEN)
		{
		mb_get_binary_int(SWAPFLAG, &buffer[index], &data->nav_source); index += sizeof(int);
		...
		&& group_id == MBF_SB2120XS_NAV_GROUP_POS)
		{
		...
		&& group_id == MBF_SB2120XS_NAV_GROUP_MOTIONGT)
		{
		...
		&& group_id == MBF_SB2120XS_NAV_GROUP_MOTIONTW)
		{
		...
		&& group_id == MBF_SB2120XS_NAV_GROUP_TRACK)
		{
		index += byte_count - 4;
		}
	    }
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_svp(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_svp";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_VELOCITY)
		{
		mb_get_binary_int(SWAPFLAG, &buffer[index], &data->svp_nsvp); index += sizeof(int);
		for (i=0;i<data->svp_nsvp;i++)
		    {
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_CONDUCTIVITY)
		{
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_SALINITY)
		{
		mb_get_binary_int(SWAPFLAG, &buffer[index], &data->svp_nctd); index += sizeof(int);
		for (i=0;i<data->svp_nctd;i++)
		    {
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_TEMP)
		{
		mb_get_binary_int(SWAPFLAG, &buffer[index], &data->svp_nctd); index += sizeof(int);
		for (i=0;i<data->svp_nctd;i++)
		    {
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_PRESSURE)
		{
		mb_get_binary_int(SWAPFLAG, &buffer[index], &data->svp_nctd); index += sizeof(int);
		for (i=0;i<data->svp_nctd;i++)
		    {
		...
		&& group_id == MBF_SB2120XS_SVP_GROUP_SSV)
		{
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_ship(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_ship";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
	    ...
	    if (done == MB_NO
		&& group_id == MBF_SB2120XS_SHP_GROUP_PARAMETER)
		{
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_sidescan(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_sidescan";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
	    ...
	    /* handle general group */
	    if (done == MB_NO
		&& group_id == MBF_SB2120XS_SSN_GROUP_GEN)
		{
		...
		&& group_id == MBF_SB2120XS_SSN_GROUP_AMPVSLAT)
		{
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_multibeam(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_multibeam";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
	    ...
	    /* handle general group */
	    if (done == MB_NO
		&& group_id == MBF_SB2120XS_MBM_GROUP_GEN)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_BEAM)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_TT)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_QUALITY)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_AMP)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_DELAY)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_LATERAL)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_ALONG)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_DEPTH)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_ANGLE)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_HEAVE)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_ROLL)
		{
		...
		&& group_id == MBF_SB2120XS_MBM_GROUP_PITCH)
		{
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_rd_comment(int verbose, int buffer_size, char *buffer, 
		struct mbf_sb2120xs_struct *data, int *error)
{
	char	*function_name = "mbr_sb2120xs_rd_comment";
	int	status = MB_SUCCESS;
	int	byte_count;
	int	group_id;
	int	done;
	...
	while (index <= buffer_size 
	    ...
	    /* handle general group */
	    if (done == MB_NO
		&& group_id == MBF_SB2120XS_COM_GROUP_GEN)
		{
		for (i=0;i<byte_count-4;i++)
		    {
		    if (i<MBF_SB2120XS_COMMENT_LENGTH-1)
			data->comment[i] = buffer[index];
		    index++;
		    }
		data->comment[MIN(byte_count-4, MBF_SB2120XS_COMMENT_LENGTH-1)] = 0;
		}
	    }
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_data(int verbose, char *mbio_ptr, char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_data";
	int	status = MB_SUCCESS;
	struct mb_io_struct *mb_io_ptr;
	struct mbf_sb2120xs_struct *data;
	FILE	*mbfp;
	...
	if (data->kind == MB_DATA_COMMENT)
		{
		status = mbr_sb2120xs_wr_comment(verbose,&buffer_size,buffer,data,error);
		...
		{
		status = mbr_sb2120xs_wr_nav(verbose,&buffer_size,buffer,data,error);
		if ((write_size = fwrite(buffer,1,buffer_size,mbfp)) != buffer_size)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
		...
		{
		status = mbr_sb2120xs_wr_svp(verbose,&buffer_size,buffer,data,error);
		if ((write_size = fwrite(buffer,1,buffer_size,mbfp)) != buffer_size)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
		...
		{
		status = mbr_sb2120xs_wr_ship(verbose,&buffer_size,buffer,data,error);
		if ((write_size = fwrite(buffer,1,buffer_size,mbfp)) != buffer_size)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
		...
		if (data->sid_frame == MB_YES)
		    {
		    status = mbr_sb2120xs_wr_sidescan(verbose,&buffer_size,buffer,data,error);
		    if ((write_size = fwrite(buffer,1,buffer_size,mbfp)) != buffer_size)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
		...
		if (data->mul_frame == MB_YES)
		    {
		    status = mbr_sb2120xs_wr_multibeam(verbose,&buffer_size,buffer,data,error);
		    if ((write_size = fwrite(buffer,1,buffer_size,mbfp)) != buffer_size)
			{
			*error = MB_ERROR_WRITE_FAIL;
			status = MB_FAILURE;
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_nav(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_nav";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_NAV_FRAME, &buffer[index]); index += sizeof(int);
	...
	/* get pos group */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_NAV_GROUP_POS, &buffer[index]); index += sizeof(int);
	...
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_NAV_GROUP_MOTIONGT, &buffer[index]); index += sizeof(int);
	...
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_NAV_GROUP_MOTIONTW, &buffer[index]); index += sizeof(int);
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_svp(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_svp";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_FRAME, &buffer[index]); index += sizeof(int);
	...
	if (data->svp_nsvp > 0)
	    ...
	    /* get depth array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_DEPTH, &buffer[index]); index += sizeof(int);
	    ...
	    /* get depth array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_VELOCITY, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->svp_nsvp, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->svp_nsvp;i++)
		{
	...
	if (data->svp_nctd > 0)
	    ...
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_CONDUCTIVITY, &buffer[index]); index += sizeof(int);
	    ...
	    /* get salinity array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_SALINITY, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->svp_nctd, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->svp_nctd;i++)
		{
	    ...
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_TEMP, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->svp_nctd, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->svp_nctd;i++)
		{
	    ...
	    /* get pressure array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_PRESSURE, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->svp_nctd, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->svp_nctd;i++)
		{
	...
	if (data->svp_ssv > 0.0)
	    ...
	    /* get ssv */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SVP_GROUP_SSV, &buffer[index]); index += sizeof(int);
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_ship(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_ship";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SHP_FRAME, &buffer[index]); index += sizeof(int);
	...
	/* get parameter group */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SHP_GROUP_PARAMETER, &buffer[index]); index += sizeof(int);
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_multibeam(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_multibeam";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_FRAME, &buffer[index]); index += sizeof(int);
	...
	/* get general group */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_GEN, &buffer[index]); index += sizeof(int);
	...
	if (data->mul_group_beam == MB_YES)
	    ...
	    /* get beam array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_BEAM, &buffer[index]); index += sizeof(int);
	...
	if (data->mul_group_tt == MB_YES)
	    ...
	    /* get tt array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_TT, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_quality == MB_YES)
	    ...
	    /* get quality array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_QUALITY, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_amp == MB_YES)
	    ...
	    /* get amplitude array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_AMP, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_delay == MB_YES)
	    ...
	    /* get delay array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_DELAY, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_lateral == MB_YES)
	    ...
	    /* get lateral array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_LATERAL, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_along == MB_YES)
	    ...
	    /* get along array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_ALONG, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_depth == MB_YES)
	    ...
	    /* get depth array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_DEPTH, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_angle == MB_YES)
	    ...
	    /* get angle array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_ANGLE, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_heave == MB_YES)
	    ...
	    /* get heave array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_HEAVE, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_roll == MB_YES)
	    ...
	    /* get roll array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_ROLL, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
	...
	if (data->mul_group_pitch == MB_YES)
	    ...
	    /* get pitch array */
	    mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_MBM_GROUP_PITCH, &buffer[index]); index += sizeof(int);
	    mb_put_binary_int(SWAPFLAG, data->mul_num_beams, &buffer[index]); index += sizeof(int);
	    for (i=0;i<data->mul_num_beams;i++)
		{
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_sidescan(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_sidescan";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SSN_FRAME, &buffer[index]); index += sizeof(int);
	...
	/* get general group */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SSN_GROUP_GEN, &buffer[index]); index += sizeof(int);
	...
	/* get amplitude array */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_SSN_GROUP_AMPVSLAT, &buffer[index]); index += sizeof(int);
...
/*--------------------------------------------------------------------*/
int mbr_sb2120xs_wr_comment(int verbose, int *buffer_size, char *buffer, 
		char *data_ptr, int *error)
{
	char	*function_name = "mbr_sb2120xs_wr_comment";
	int	status = MB_SUCCESS;
	struct mbf_sb2120xs_struct *data;
	int	index;
	int	size;
	...
	/* get frame time */
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_COM_FRAME, &buffer[index]); index += sizeof(int);
	...
	mb_put_binary_int(SWAPFLAG, MBF_SB2120XS_COM_GROUP_GEN, &buffer[index]); index += sizeof(int);
