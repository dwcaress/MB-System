	...
	*error = MB_ERROR_NO_ERROR;
	...
	if (*prstatus == MB_PR_FILE_NEEDS_PROCESSING) {
		...
		if (ofilemodtime > 0 && ofilemodtime >= ifilemodtime && ofilemodtime >= pfilemodtime && ofilemodtime >= navfilemodtime &&
		    ofilemodtime >= navadjfilemodtime && ofilemodtime >= attitudefilemodtime && ofilemodtime >= sonardepthfilemodtime &&
...
int mb_pr_readpar(int verbose, char *file, int lookforfiles, struct mb_process_struct *process, int *error) {
	...
	char pa2file[MBP_FILENAMESIZE];
	char buffer[MBP_FILENAMESIZE], dummy[MBP_FILENAMESIZE], unknown[MBP_FILENAMESIZE], *result;
	char *lastslash;
	FILE *fp;
	struct stat statbuf;
	int status = MB_SUCCESS;
	...
	process->mbp_n_unknown_num = 0;
	for (i = 0; i < MBP_MAX_UNKNOWN_NUM; i++) {
		process->mbp_unknown_num_param[i][0] = '\0';
		process->mbp_unknown_num_value[i] = 0.0;
		}
	for (i = 0; i < MBP_MAX_UNKNOWN_STR; i++) {
		process->mbp_unknown_str_param[i][0] = '\0';
		process->mbp_unknown_str_value[i][0] = '\0';
		}

	for (pfile = 0; pfile < 2; pfile ++) {
		if (pfile == 0)
		fp = fopen(pa2file, "r");
		else
		fp = fopen(parfile, "r");
		...
		if (fp != NULL) {
			while ((result = fgets(buffer, MBP_FILENAMESIZE, fp)) == buffer) {
				if (buffer[0] != '#') {
					...
					sscanf(buffer, "%s %d", dummy, &process->mbp_sscorr_stddev);
					}
	...
	if (lookforfiles > 2) {
		...
		if ((lastslash = strrchr(process->mbp_sonardepthfile, '/')) != NULL && strlen(lastslash) > 1) {
			strcpy(dummy, &(lastslash[1]));
		...
		if ((lastslash = strrchr(process->mbp_ampcorr_reffile, '/')) != NULL && strlen(lastslash) > 1) {
			strcpy(dummy, &(lastslash[1]));
	    ...
	    if ((lastslash = strrchr(process->mbp_sap_profile, '/')) != NULL && strlen(lastslash) > 1) {
			strcpy(dummy, &(lastslash[1]));
	...
	/* reset sonardepth file */
	if (len > 1 && strlen(process->mbp_sonardepthfile) > 1 && process->mbp_sonardepthfile[0] != '/' &&
	    process->mbp_sonardepthfile[1] != ':') {
	...
	if (len > 1 && strlen(process->mbp_sap_profile) > 1 && process->mbp_sap_profile[0] != '/') {
...
int mb_pr_writepar(int verbose, char *file, struct mb_process_struct *process, int *error) {
	...
	/* set error */
		...
		*error = MB_ERROR_OPEN_FAIL;
        ...
        /* open second parameter file */
	if (process->mbp_file2_modified == MB_YES) {
	    if (process->mbp_n_unknown_num <= MBP_MAX_UNKNOWN_NUM && process->mbp_n_unknown_str <= MBP_MAX_UNKNOWN_STR) {
		if ((fp = fopen(pa2file, "w")) != NULL) {
		    fprintf(fp, "## MB-System processing parameter file\n");
		    fprintf(fp, "## Written by %s version %s\n", function_name, rcs_id);
		    fprintf(fp, "## MB-system Version %s\n", MB_VERSION);
		    fprintf(fp, "## Generated by user <%s> on cpu <%s> at <%s>\n##\n", user, host, date);
		    ...
		    fprintf(fp, "##\n## Unrecognised parameters:\n");
		    for (i = 0; i < process->mbp_n_unknown_num; i++) {
				if (process->mbp_unknown_num_value[i] == (int)process->mbp_unknown_num_value[i])
					fprintf(fp, "%s %d\n", process->mbp_unknown_num_param[i], (int)process->mbp_unknown_num_value[i]);
				else
					fprintf(fp, "%s %f\n", process->mbp_unknown_num_param[i], process->mbp_unknown_num_value[i]);
			}
		    for (i = 0; i < process->mbp_n_unknown_str; i++) {
				fprintf(fp, "%s %s\n", process->mbp_unknown_str_param[i], process->mbp_unknown_str_value[i]);
			}
		...
		/* set error */
		    ...
		    *error = MB_ERROR_OPEN_FAIL;
		    status = MB_FAILURE;
		    if (verbose > 0)
				fprintf(stderr, "\nUnable to Open Second Parameter File <%s> for writing\n", pa2file);
		    }
		}
	    else {
			*error = MB_ERROR_OPEN_FAIL;
			status = MB_FAILURE;
			if (verbose > 0)
				fprintf(stderr, "\nUnable to preserve unknown parameters for <%s>\n", pa2file);
		}
	}
	...
	if (process->mbp_svp_mode == MBP_SVP_ON)
		...
		process->mbp_bathrecalc_mode = MBP_BATHRECALC_ROTATE;
	         ...
	         (process->mbp_draft_mode != MBP_DRAFT_OFF || process->mbp_nav_draft != MBP_NAV_OFF ||
	          process->mbp_sonardepth_mode != MBP_SONARDEPTH_OFF || process->mbp_lever_mode != MBP_LEVER_OFF ||
...
int mb_pr_default_output(int verbose, struct mb_process_struct *process, int *error) {
	...
	status = mb_get_format(verbose, process->mbp_ifile, fileroot, &format, error);
...
int mb_pr_get_output(int verbose, int *format, char *ifile, char *ofile, int *error) {
	...
	/* figure out data format and fileroot if possible */
	status = mb_get_format(verbose, ifile, fileroot, &tformat, error);
...
int mb_pr_update_ampcorr(int verbose, char *file, int mbp_ampcorr_mode, char *mbp_ampcorrfile, int mbp_ampcorr_type,
						 int mbp_ampcorr_symmetry, double mbp_ampcorr_angle, int mbp_ampcorr_slope, int	mbp_ampcorr_area, char *mbp_ampsscorr_topofile,
                         int *error) {
...
int mb_pr_update_sscorr(int verbose, char *file, int mbp_sscorr_mode, char *mbp_sscorrfile, int mbp_sscorr_type,
						int mbp_sscorr_symmetry, double mbp_sscorr_angle, int mbp_sscorr_slope, int	mbp_sscorr_area, char *mbp_ampsscorr_topofile,
                        int *error) {
...
/*--------------------------------------------------------------------*/
int mb_pr_update_abscorr(int verbose, char *file, int mbp_sap_mode, int mbp_sap_src, int mbp_sap_use, char *mbp_sap_profile,
                         double mbp_sa_old, double mbp_sa_new, int *error) {
...
int mb_pr_get_ampcorr(int verbose, char *file, int *mbp_ampcorr_mode, char *mbp_ampcorrfile, int *mbp_ampcorr_type,
                      int *mbp_ampcorr_symmetry, double *mbp_ampcorr_angle, int *mbp_ampcorr_slope, 
                      int *mbp_ampcorr_area, char *mbp_ampsscorr_topofile,
                      int *error) {
...
/*--------------------------------------------------------------------*/
int mb_pr_get_abscorr(int verbose, char *file, int *mbp_sap_mode, int *mbp_sap_src, int *mbp_sap_use, char *mbp_sap_profile, double *mbp_sa_old, double *mbp_sa_new, int *error) {
