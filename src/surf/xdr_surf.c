...
#ifdef __LP64__
  return xdr_short(xdrs, &gp->typ) && xdr_u_int(xdrs, &gp->nr);
#else
...
XdrSurf xdr_SurfDescriptor(XDR *xdrs, SurfDescriptor *gp,
  ...
  short newVers;
  short oldVers;
  XdrSurf status = xdr_SurfCheckVersion(xdrs, gp, SURF_VERSION, &newVers, &oldVers);
...
XdrSurf xdr_PositionSensorArray(XDR *xdrs, SurfPositionSensorArray *gp, short oldVers) {
  ...
  if (strncmp(gp->positionSensorName, (const char*)buffer, sizeS) == 0) {
    return xdr_SurfPositionPolarfix(xdrs, (SurfPositionPolarfix*)gp->sensorUnion);
  }
  if (oldVers < vers30) {
    XdrSurf ret = xdr_SurfUnknownPositionSensor(xdrs, (char*)&gp->sensorUnion);
    SurfPositionAnySensor *toAny = (SurfPositionAnySensor*) gp->sensorUnion;
    toAny->sensorAntennaPositionAhead = 0.0f;
    toAny->sensorAntennaPositionStar  = 0.0f;
    toAny->sensorAntennaPositionHeight= 0.0f;
    return ret;
  }

  return
      xdr_SurfPositionAnySensor(xdrs, (SurfPositionAnySensor*)gp->sensorUnion);
}
...
XdrSurf xdr_SurfMultiBeamAngleTable(XDR *xdrs,
  ...
  XdrSurf ret = xdr_u_short(xdrs, &gp->actualNumberOfBeams);
  for(u_short n = 0; n < maxBeamNr; n++) {
...
XdrSurf xdr_SurfFreeText (XDR *xdrs,
  ...
  return xdr_bytes(xdrs, &bp, &sizeFT, sizeFT);
}

static XdrSurf xdr_SurfReductionParameters(XDR *xdrs,
                                           SurfReductionParameters *gp) {
  return
...
static XdrSurf xdr_SurfLastFilterParameters(XDR *xdrs,
                                            SurfLastFilterParameters *gp) {
  return
...
XdrSurf xdr_SurfMultiBeamTT(XDR *xdrs, SurfMultiBeamTT *gp) {
  return xdr_float(xdrs, &gp->travelTimeOfRay);
}
...
XdrSurf xdr_SurfSidescanData(XDR *xdrs, SurfSidescanData *gp,
  ...
  if (nrSsData > (u_short)0) {
    ...
    if (
#ifdef __LP64__
      xdr_u_int(xdrs, &gp->sidescanFlag) != SURF_SUCCESS ||
#else
      xdr_u_long(xdrs, &gp->sidescanFlag) != SURF_SUCCESS ||
#endif
