...
int main( int argc, char **argv )
{
 ...
 //initialize memory for different tests
  ...
  char fileName[512];
  char *dataPath = NULL;
  dataPath = (dataPath = getenv("TRN_DATAFILES"))? dataPath : (char *)"./";
  printf("%s\n", dataPath);
  for(i = 0; i < numTests; i++)
  {
    printf("Read measurements...\n");
    ...
    dataKft = readDataFromFile(charCat(fileName, dataPath,dataK_files[i]),
     ...
     {  
        runTerrainNav(dataKft, dataMeas, watVel, map_files[i], 0, 15, 15, 
...
void runTerrainNav(const Matrix &dataKft, const Matrix &dataMeas, 
                   const Matrix &watVel, char* mapFile, 
                   int interpMethod, int kSubSample, 
		   						 int mSubSample, poseT *tercomEst, poseT *mmseEst, 
                   bool realTime, int filterType, int dataK_init)
{
  ...
  // If using remote TRN, use just the file name relative
  // to what TRN_MAPFILES is set to on the server.
  //
  if (trnHost) {
    tercom = new TerrainNavClient(trnHost, trnPort,
				  mapFile, (char*)"mappingAUV_specs.cfg", filterType, mapType);
  }
  else {
    char mapPath[512];
    char *dataPath = NULL;
    dataPath = (dataPath = getenv("TRN_MAPFILES"))? dataPath : (char *)"./";
    sprintf(mapPath, "%s/%s", dataPath, mapFile);
    printf("%s\n", mapPath);
    tercom = new TerrainNav(mapPath, (char*)"imagingAUV_specs.cfg", filterType, mapType);
  }

  if (!tercom->initialized()) {
    printf("Sorry, TerrainNav failed to initialize.\n");
    exit(1);
  }
  ...
  while(i <= N)
      ...
      else
	{
           if( j <= M)
           {
              ...
              default:
		printf("No valid datatype specified: %d.  Exiting...\n",
		       dataType);
		break;
		//return;
              }
