...
int
Octree<ValueType>::
AddPoints(const Vector points[], const unsigned int numPoints) {
...
void
Octree<ValueType>::
FillSmallestResolutionLeafAtPointIfEmpty(const Vector& point, ValueType fillValue){
	if(ContainsPoint(point)){
		int depth;
		Path path = FindPathToPoint(point);
		OctreeNode* nodePointer = GetPointerToLeafOnPath(depth, path);
		if(nodePointer->value == EmptyValue){
			unsigned int bitmask = 1 << (MaxDepth - depth - 1);
			for(; depth < MaxDepth; depth ++){
				...
				if(nodePointer->children != NULL){
					for(int index = 0; index < 8; index ++){
						delete nodePointer->children[index];
					}
...
FillIfEmpty(const Vector& point, ValueType fillValue){
	if(ContainsPoint(point)){
		OctreeNode* nodePointer = GetPointerToLeafOnPath(FindPathToPoint(point));
		if(nodePointer->value == EmptyValue){
			nodePointer->value = fillValue;
		}
	}
...
LoadFromFile(const char* filename) {
	...
	//OffMapValue
	if(std::fread(&OffMapValue, sizeof(ValueType), 1, loadFile) != 1){return false;}
	//EmptyValue
...
CalculateBoundsFromPath(Vector& nodeLowerBounds, Vector& nodeUpperBounds, const Path& path, const int depth) const {
	...
	multiplier /= (static_cast<double>(1 << depth));
...
int
Octree<ValueType>::
GetPathChildNumber(const Path& path, const int depth) const{
	int childNumber =
		(((path.x & (1 << (MaxDepth - depth - 1))) != 0) << 2)
		| (((path.y & (1 << (MaxDepth - depth - 1))) != 0) << 1)
		| ((path.z & (1 << (MaxDepth - depth - 1))) != 0);
