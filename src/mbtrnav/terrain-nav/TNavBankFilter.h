 ...
 * DATE          : 05/06/09
 * DESCRIPTION   : TNavBankFilter is an inheritance class of TNavFilter.h 
...
//moving these from #defines to a config file would be a good thing for testing different configurations on the boat
//they are only used to set corresponding TNavBankFilter private variables in initVariables()
#define NUM_FILTERS			2
//PMF_GRID_RESOLUTION * PMF_GRID_SIZE is the size of the initial search area
#define PMF_GRID_SIZE			100		//square this for the number of particles in the PMF
#define PMF_GRID_RESOLUTION		1		//resolution of the PMF in meters
/*
values for NUM_FILTERS: 

1 - for testing convergence with all the beams

2 - for for testing basic filter agreement

11 - one for each beam

other int - for computational load testing

values for PMF_GRID_SIZE: Square this to get the number of particles. 100
results in 10000 particles. Higher than this requires increasing
MAX_NUM_PARTICLES in TNavBankFilter.h and will increase computation and
memory requirements.

values for PMF_GRID_RESOLUTION:
I've tested with either 1 or 2. It doesn't have to be an int.
*/



//#defines for the bank filter
#define MAX_NUM_FILTERS 12
  ...
  void displayParticleInfo()
    {
      ...
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Weight: %f\n", weight);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Position (N,E,D): ");
      for(i = 0; i < 3; i++)
	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%.3f\t", position[i]);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\nParticle Attitude in degrees (phi,theta,psi): ");
      for(i = 0; i < 3; i++)
	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%.2f\t", attitude[i]*180.0/PI);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\nParticle Terrain Pose: ");
      for(i = 0; i < 3; i++)
	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%f\t", terrainState[i]);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\nParticle DVL Alignment in degrees: ");
      for(i = 0; i < 3; i++)
	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%f\t", alignState[i]*180.0/PI);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\nParticle Gyro Bias (^o/sec) in y,z: ");
      for(i = 0; i < 2; i++)
	 logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%f\t", gyroBias[i]*180.0/PI);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Compass Bias in degrees: %f\n", compassBias*180.0/PI);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Psi Berg in degrees: %f\n", psiBerg*180.0/PI);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Expected Measurement Difference: ");

      for (std::vector<double>::size_type ix = 0; ix < expectedMeasDiff.size(); ix++)
	 logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%f\t", expectedMeasDiff[ix]);
      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\n");
//	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"Particle Expected Measurement Difference: ");
//      for(i = 0; i < 4; i++)
//	logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"%f\t", expectedMeasDiff[i]);
//      logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),"\n");
    }
 ...
 *
 * Intended use:
 *      Initialize the TNavBankFilter object
 *               TNavFilter *tNavFilter;
 *               tNavFilter = new TNavBankFilter();
 *
 ...
 */
class TNavBankFilter : public TNavFilter
{
 public:
  ...
  */
  ...
  /* Constructor: TNavBankFilter()
   * Usage: trnFilter = new TNavBankFilter("mapName.txt", vehicle.txt", 
   * "saveDir",windowVar,1);
   * -------------------------------------------------------------------------*/
  /*! Initializes a new TNavBankFilter object with TerrainMap given by 
   ...
   */
  //TNavBankFilter(char *mapName, char *vehicleSpecs, char *directory, const double *windowVar, const int &mapType); //Map Reload Issue
  TNavBankFilter(TerrainMap* terrainMap, char *vehicleSpecs, char *directory, const double *windowVar, const int &mapType);



  /* Destructor: ~TNavBankFilter()
   * Usage: delete trnFilter;
   * -------------------------------------------------------------------------*/
  /*! Frees all storage associated with the TNavBankFilter object.
   */
  virtual ~TNavBankFilter();
   ...
   * Usage: set*(newValue)
   * -------------------------------------------------------------------------*/
  /*! Sets the value of the parameter to the newValue, the old value is returned. 
   *  Returns -1 if nf is out of bounds value. 
   */
  int setNumFilters(int nf)
  {
    if (nf > MAX_NUM_FILTERS || nf < 1) return -1;
    int old = numFilters; numFilters = nf;
    return old;
  }
  int setPmfGridSize(int pgs)
  {
    int old = PmfGridSize; PmfGridSize = pgs; return old;
  }
  double setPmfGridResolution(double pgr)
  {
    double old = PmfGridResolution; PmfGridResolution = pgr; return old;
  }

  //Public structures and components of a TNavBankFilter object:
  /*********************************************************/
 ...
 private:
  ...
  /* Function: computeMMSE
   * Usage: computeMMSE(mleEstimate, filterIndex);
   * -------------------------------------------------------------------------*/
  /*! Computes and returns a pose estimate based on minimum mean square 
   * estimation - i.e., the probabilistic mean of the data.  This function also
   * computes the covariance of the particle distribution about the MMSE
   * estimate. Specifies which filter to use from the bank of filters.
   */
  void computeMMSE(poseT* mmsePose, int filterIndex);
   ...
   * Usage: initVariables();
   * -------------------------------------------------------------------------*/
  /*! Initializes private variables associated with a TNavBankFilter object
   */
  void initVariables();
  ...
  int numFilters;
  int PmfGridSize;
  double PmfGridResolution;
  WeightArray weights[MAX_NUM_FILTERS];
  double depthBias [MAX_NUM_FILTERS][MAX_NUM_PARTICLES];
  double depthBiasCov[MAX_NUM_FILTERS];
  
  int filterConfiguration;
  int filterComparison;
  double filterComparisonGaussianDistanceThreshold;
  double filterComparisonWeightRatioThreshold;
  
  double windowedNis[MAX_NUM_FILTERS][MAX_NUM_PARTICLES][20];
