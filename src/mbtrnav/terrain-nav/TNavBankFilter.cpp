...
measUpdate(measT& currMeas) {
	...
	if(successfulMeas) {
		...
		if(mapStatus == MAPBOUNDS_OUT_OF_BOUNDS) {
			...
			if(!temp) logs(TL_OMASK(TL_TNAV_BANK_FILTER, TL_LOG),
...
#                       */
				...
				*/
				...
				else
				{
					...
					double modMapVar = 0.01;//TODO what are we doing here? ...look three lines later
...
homerMeasUpdate(const measT& currMeas) {
	...
	for(i = 0; i < nParticles; i++) {
		weight = this->weights[0].weights[i];
		sumWeights += weight;
...
void
TNavBankFilter::
motionUpdateParticle(particleT& particle, poseT& diffPose) {
	double currDvlAttitude[3] = {dvlAttitude[0], dvlAttitude[1],
		dvlAttitude[2]
	};
	double currAttitude[3] = {particle.attitude[0], particle.attitude[1],
