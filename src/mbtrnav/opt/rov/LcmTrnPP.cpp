...
LcmTrnPP::LcmTrnPP(pcf::lcm_interface &lcm)
:mLcm(nullptr)
, mInputList()
, mSemList()
, mPubList()
...
void LcmTrnPP::tostream(std::ostream &os, int wkey, int wval)
{
    ...
    if(mCtx.size()>0){
        ...
        for(it = mCtx.begin(); it != mCtx.end(); it++){
            LcmTrnCtx *ctx = static_cast< LcmTrnCtx *>(*it);
...
std::string LcmTrnPP::tostring(int wkey, int wval)
{
...
void LcmTrnPP::show(int wkey, int wval)
{
...
std::vector<LcmTrnCtx *>::iterator LcmTrnPP::ctx_list_begin()
{
    return mCtx.begin();
}

std::vector<LcmTrnCtx *>::iterator LcmTrnPP::ctx_list_end()
{
    return mCtx.end();
}

int LcmTrnPP::add_pub(const std::string &channel)
{
...
pcf::lcm_publisher *LcmTrnPP::get_pub(const std::string& channel)
{
    if(!mPubList.empty()){
...
std::list<lcm_pub> &LcmTrnPP::pub_list()
{
    return mPubList;
}

int LcmTrnPP::start_lcm_pubs()
{
...
int LcmTrnPP::add_sem(const std::string& channel, int count)
{
...
int LcmTrnPP::test_sem(const std::string& channel, int to_msec, msg_callback cb, int &r_stat, void *parg, bool clear_pending)
{
    trn_lcm_input *inp = get_input(channel);
    if(inp != nullptr){
...
void LcmTrnPP::sem_worker_fn(const std::string& channel, int to_msec, msg_callback cb, void *parg, bool clear_pending, std::promise<int> rv_promise, std::promise<int> st_promise)
{
    TRN_TRACE();
    int stat = -1;
    TRN_TRACE();
...
int LcmTrnPP::list_test_sem(bool clear_pending, int &r_tested, int &r_called, int &r_error)
{
    int stat = 0;
    std::list<sem_reg>::iterator it = mSemList.begin();
    ...
    while(it != mSemList.end()){
        ...
        if(cb != nullptr && parg != nullptr){
            ...
            std::thread *worker = new std::thread(&LcmTrnPP::sem_worker_fn, this, channel, to_msec, cb, parg, clear_pending, std::move(rv_prom),  std::move(st_prom));
...
sem_reg *LcmTrnPP::lookup_sem(const std::string &channel, const std::string &cb_key)
{
    sem_reg *retval = nullptr;
...
int LcmTrnPP::list_add_sem(const std::string& channel, int to_msec, msg_callback cb, void *parg, int count)
{
    trn_lcm_input *inp = get_input(channel);
    if(inp != nullptr){
...
msg_callback LcmTrnPP::lookup_callback(const std::string &key)
{
...
void LcmTrnPP::register_callback(const char *key, msg_callback cb)
{
    std::list<callback_kv>::iterator it;
...
void LcmTrnPP::set_callback_res(trnxpp_cfg *cfg)
{
    mCallbackRes.cfg = cfg;
    mCallbackRes.xpp = this;
}

callback_res_t *LcmTrnPP::callback_res()
{
    return &mCallbackRes;
}

beam_geometry *LcmTrnPP::lookup_geo(const std::string chan, int type)
{
...
int LcmTrnPP::add_input(const std::string& name, uint32_t depth)
{
...
int LcmTrnPP::add_input(const std::string& name, trn_lcm_input *sub)
{
    if(sub != nullptr){
        mLcm->add_subscriber(*sub);
        lcm_input item = std::make_tuple(std::string(name), sub);
...
trn_lcm_input *LcmTrnPP::get_input(const std::string& channel)
{
    if(!mInputList.empty()){
        std::list<lcm_input>::iterator it;
...
trn_lcm_input *LcmTrnPP::create_input(const std::string &channel, int buf_depth)
{
...
trn::bath_input *LcmTrnPP::get_bath_input(const std::string chan)
{
...
trn::mb1_info *LcmTrnPP::get_mb1_info(const std::string chan)
{
...
trn::mb1_input *LcmTrnPP::get_mb1_input(const std::string chan)
{
...
trn::depth_info *LcmTrnPP::get_depth_info(const std::string chan)
{
...
trn::depth_input *LcmTrnPP::get_depth_input(const std::string chan)
{
...
trn::bath_info *LcmTrnPP::get_bath_info(const std::string chan)
{
...
trn::nav_input *LcmTrnPP::get_nav_input(const std::string chan)
{
...
trn::nav_info *LcmTrnPP::get_nav_info(const std::string chan)
{
...
trn::att_input *LcmTrnPP::get_att_input(const std::string chan)
{
...
trn::att_info *LcmTrnPP::get_att_info(const std::string chan)
{
...
trn::vel_input *LcmTrnPP::get_vel_input(const std::string chan)
{
...
trn::vel_info *LcmTrnPP::get_vel_info(const std::string chan)
{
...
trn_host *LcmTrnPP::lookup_trn_host(const std::string &key)
{
...
std::list<trn_host>::iterator LcmTrnPP::trn_host_list_begin()
{
    return mTrnHostList.begin();
}

std::list<trn_host>::iterator LcmTrnPP::trn_host_list_end()
{
    return mTrnHostList.end();
}

int LcmTrnPP::parse_trn(const char *str)
{
    int retval = -1;
...
int LcmTrnPP::parse_input(const char *str)
{
    int retval = -1;
...
int LcmTrnPP::parse_ctx_dmap(const char *map_spec, std::map<std::string, double>& kvmap)
{
    int retval = -1;
    if(NULL != map_spec){
...
int LcmTrnPP::parse_ctx_umap(const char *map_spec, std::map<std::string, uint64_t>& kvmap)
{
    int retval = -1;
    if(NULL != map_spec){
        // parse extra parameters:
...
int LcmTrnPP::parse_ctx_imap(const char *map_spec, std::map<std::string, int64_t>& kvmap)
{
    int retval = -1;
    if(NULL != map_spec){
        // parse extra parameters:
...
int LcmTrnPP::parse_ctx_input(char *opt_s, const char *key, int *r_idx, char **r_chan, char **r_cb, int *r_tmout)
{
    int retval = -1;
...
void LcmTrnPP::add_ctx_sem(char *chan, char *cb_key, int tmout)
{
...
int LcmTrnPP::parse_ctx(const char *str)
{
    int retval = -1;
        ...
        while(NULL != opt_s && (flags & ERR)==0 )
        {
            ...
            if(strstr(opt_s, "key:") != NULL) {
                ...
                if(strlen(val_key) > 0) {
                    ...
                    trn_host *thost = lookup_trn_host(val_key);
...
int LcmTrnPP::parse_sem(const char *str)
{
    int retval = -1;
...
void LcmTrnPP::parse_config(trnxpp_cfg *cfg)
{
    if(cfg == nullptr){
