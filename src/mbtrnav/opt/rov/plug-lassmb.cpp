...
// process multibeam sounding from ocean imaging toolsled (mounted on rotating arm)
// using mbtrnpp, since it assumes they are distributed
// in a linear array.
// expects:
// bi[0] - vehicle bath (DVL) OPTIONAL
// bi[1] - oi sled bath (MULTIBEAM)
// ai[0] - vehicle att
// ai[0] - sled att
...
// input: OI sled DVL or multibeam
...
int cb_proto_mblass(void *pargs)
{
    ...
    for(it = xpp->ctx_list_begin(); it != xpp->ctx_list_end(); it++)
    {
        ...
        if(ctx == nullptr || !ctx->has_callback("cb_proto_mblass"))
        {
            TRN_TRACE();
