...
LcmTrnCtx::LcmTrnCtx(const LcmTrnCtx &other)
:
...
void LcmTrnCtx::tostream(std::ostream &os, int wkey, int wval)
{
...
std::string LcmTrnCtx::tostring(int wkey, int wval)
{
...
void LcmTrnCtx::show(int wkey, int wval)
{
    tostream(std::cerr, wkey, wval);
}

void LcmTrnCtx::set_ctx_key(const char *key)
{
    mCtxKey = std::string(key);
}

std::string &LcmTrnCtx::ctx_key()
{
    return mCtxKey;
}

void LcmTrnCtx::set_utm_zone(long int utm)
{
    mUtmZone = utm;
    if(mGeoCon != nullptr)
        delete(mGeoCon);
...
long int LcmTrnCtx::utm_zone()
{
    return mUtmZone;
}

void LcmTrnCtx::set_geo_crs(std::string crs)
{
    mGeoCRS = crs;
    if(mGeoCon != nullptr)
        delete(mGeoCon);
...
std::string LcmTrnCtx::geo_crs()
{
    return mGeoCRS;
}

GeoCon *LcmTrnCtx::geocon()
{
    return mGeoCon;
}

void LcmTrnCtx::add_callback_key(const std::string &key){
    if(!has_callback(key))
        mCallbackKeys.push_back(key);
}

bool LcmTrnCtx::has_callback(const std::string &key)
{
...
int LcmTrnCtx::decmod()
{
    return mDecMod;
}

void LcmTrnCtx::set_decmod(int n)
{
    mDecMod = n;
}

int LcmTrnCtx::cbcount()
{
    return mCBCount;
}

void LcmTrnCtx::set_cbcount(int n)
{
    mCBCount = n;
}

void LcmTrnCtx::inc_cbcount()
{
    mCBCount++;
}


void LcmTrnCtx::set_bath_input(int i, const std::string &inp)
{
...
std::string *LcmTrnCtx::bath_input_chan(int i)
{
    return mBathInputKeys.size() > i ? &mBathInputKeys.at(i) : nullptr;
}

void LcmTrnCtx::set_mb1_input(int i, const std::string &inp)
{
...
std::string *LcmTrnCtx::mb1_input_chan(int i)
{
    return mMB1InputKeys.size() > i ? &mMB1InputKeys.at(i) : nullptr;
}

void LcmTrnCtx::set_depth_input(int i, const std::string &inp)
{
...
std::string *LcmTrnCtx::depth_input_chan(int i)
{
    return mDepthInputKeys.size() > i ? &mDepthInputKeys.at(i) : nullptr;
}

void LcmTrnCtx::set_nav_input_chan(int i, const std::string &inp)
{
...
std::string *LcmTrnCtx::nav_input_chan(int i)
{
    return mNavInputKeys.size() > i ? &mNavInputKeys.at(i) : nullptr;
}

void LcmTrnCtx::set_att_input_chan(int i, const std::string &inp)
{
...
std::string *LcmTrnCtx::att_input_chan(int i)
{
    return mAttInputKeys.size() > i ? &mAttInputKeys.at(i) : nullptr;
}

void LcmTrnCtx::set_vel_input_chan(int i, const std::string &inp)
{
...
pcf::lcm_publisher *LcmTrnCtx::get_pub(std::list<lcm_pub> &pubs, const std::string& channel)
{
    if(!pubs.empty()){
...
std::string *LcmTrnCtx::vel_input_chan(int i)
{
    return mVelInputKeys.size() > i ? &mVelInputKeys.at(i) : nullptr;
}
////////////
void LcmTrnCtx::set_trnest_csv_path(const std::string &inp)
{
    mTrnEstCsvPath = std::string(inp);
}

std::string LcmTrnCtx::trnest_csv_path()
{
    return mTrnEstCsvPath;
}

int LcmTrnCtx::init_trnest_csv_file(trnxpp_cfg *cfg)
{
    int retval = -1;
...
FILE *LcmTrnCtx::trnest_csv_open()
{
    if(mTrnEstCsvFile == nullptr){
...
FILE *LcmTrnCtx::trnest_csv_file()
{
    return mTrnEstCsvFile;
}

int LcmTrnCtx::write_trnest_csv(double &stime, poseT &pt, poseT &mle, poseT &mmse)
{
    int retval = 0;
...
/////////////////
void LcmTrnCtx::set_mbest_csv_path(const std::string &inp)
{
    mMBEstCsvPath = std::string(inp);
}

std::string LcmTrnCtx::mbest_csv_path()
{
    return mMBEstCsvPath;
}

int LcmTrnCtx::init_mbest_csv_file(trnxpp_cfg *cfg)
{
    int retval = -1;
...
FILE *LcmTrnCtx::mbest_csv_file()
{
    return mMBEstCsvFile;
}

int LcmTrnCtx::write_mbest_csv(trnu_pub_t &mbest)
{
    int retval = 0;
...
//////////////////
void LcmTrnCtx::set_mb1_csv_path(const std::string &inp)
{
    mMB1CsvPath = std::string(inp);
}

std::string LcmTrnCtx::mb1_csv_path()
{
    return mMB1CsvPath;
}

int LcmTrnCtx::init_mb1_csv_file(trnxpp_cfg *cfg)
{
    int retval = -1;
...
FILE *LcmTrnCtx::mb1_csv_file()
{
    return mMB1CsvFile;
}

int LcmTrnCtx::write_mbest_csv(double &stime, trnu_pub_t &mbest)
{
    int retval = 0;
...
int LcmTrnCtx::write_mb1_csv(mb1_t *snd, trn::bath_info *bi, trn::att_info *ai, trn::vel_info *vi)
{
    int retval = 0;
...
int LcmTrnCtx::write_mb1_csv(mb1_t *snd, trn::mb1_info *bi, trn::att_info *ai, trn::vel_info *vi)
{
    int retval = 0;
...
int LcmTrnCtx::write_csv_orig(trn::bath_info *bi, trn::att_info *ai, trn::nav_info *ni, trn::vel_info *vi)
{
    int retval = 0;
...
void LcmTrnCtx::set_mb1_bin_path(const std::string &inp)
{
    mMB1BinPath = std::string(inp);
}

std::string LcmTrnCtx::mb1_bin_path()
{
    return mMB1BinPath;
}

int LcmTrnCtx::init_mb1_bin_file(trnxpp_cfg *cfg)
{
    int retval = -1;
...
FILE *LcmTrnCtx::mb1_bin_file()
{
    return mMB1BinFile;
}

size_t LcmTrnCtx::write_mb1_bin(mb1_t *snd)
{
    size_t retval = -1;
...
/////////////////
void LcmTrnCtx::set_rawbath_csv_path(const std::string &inp)
{
    mRawBathCsvPath = std::string(inp);
}

std::string LcmTrnCtx::rawbath_csv_path()
{
    return mRawBathCsvPath;
}

int LcmTrnCtx::init_rawbath_csv_file(trnxpp_cfg *cfg)
{
    int retval = -1;
...
FILE *LcmTrnCtx::rawbath_csv_open()
{
    if(mRawBathCsvFile == nullptr){
...
FILE *LcmTrnCtx::rawbath_csv_file()
{
    return mRawBathCsvFile;
}

int LcmTrnCtx::write_rawbath_csv(trn::bath_info *bi, trn::nav_info *ni, trn::att_info *ai, trn::vel_info *vi, long int utm, double alt_depth)
{
    int retval = 0;
...
trn_host *LcmTrnCtx::lookup_udpm_host(std::string key)
{
...
udpm_sub_t *LcmTrnCtx::lookup_udpm_sub(std::string key)
{
...
int LcmTrnCtx::add_udpm_host(std::string key, trn_host *host)
{
    int retval = -1;

    trn_host *list_host = lookup_udpm_host(key);
...
int LcmTrnCtx::get_udpms_update(const std::string &key, trnxpp_cfg *cfg, byte *dest, size_t len)
{
    int retval = 0;
...
int LcmTrnCtx::start_udpmsub(const std::string &key, trnxpp_cfg *cfg)
{
    int retval = -1;
...
trn_host *LcmTrnCtx::lookup_mb1svr_host(std::string key)
{
    trn_host *retval = nullptr;
    std::list<trn_host>::iterator it;
    for(it = mMB1SvrList.begin(); it != mMB1SvrList.end(); it++){
...
int LcmTrnCtx::add_mb1svr_host(std::string key, trn_host *host)
{
    int retval = -1;

    trn_host *list_host = lookup_mb1svr_host(key);
...
int LcmTrnCtx::set_mb1svr_inst(std::string key, trn::mb1_server *inst)
{
    int retval = -1;
    std::list<trn_host>::iterator it;
    for(it = mMB1SvrList.begin(); it != mMB1SvrList.end(); it++){
...
int LcmTrnCtx::start_mb1svr(const std::string &key, trnxpp_cfg *cfg)
{
    int retval = -1;
...
int LcmTrnCtx::mb1svr_count()
{
    return mMB1SvrList.size();
}

int LcmTrnCtx::pub_mb1(mb1_t *sounding, std::list<lcm_pub> &pubs, trnxpp_cfg *cfg)
{
    int retval = -1;
...
trn_host *LcmTrnCtx::lookup_trncli_host(std::string key)
{
    trn_host *retval = nullptr;
    std::list<trn_host>::iterator it;
    int i=0;
...
int LcmTrnCtx::add_trn_host(std::string key, trn_host *host)
{
    //std::lock_guard<std::mutex> guard(mTrnCliMutex);
    int retval = -1;
...
int LcmTrnCtx::trncli_connect(const std::string &key, int retries, uint32_t delay_sec, bool *quit)
{
    int retval = -1;
    int rem = retries;
...
int LcmTrnCtx::start_trncli(const std::string &key, trnxpp_cfg *cfg, bool force_reconnect, bool *user_int)
{
    int retval = -1;
...
int LcmTrnCtx::start_trn(trnxpp_cfg *cfg, bool *user_int)
{
    int retval = -1;
...
void LcmTrnCtx::tcli_start_worker_fn(TrnClient *trncli, std::promise<bool> *con_promise)
{
    try{
        TrnAttr &att_ref = trncli->getTrnAttr();
        TrnAttr *patt = &att_ref;
...
bool LcmTrnCtx::trncli_check_connection(int idx, TrnClient *trnc, trnxpp_cfg *cfg)
{
    //std::lock_guard<std::mutex> guard(mTrnCliMutex);
    bool retval = false;
    ...
    if(!connection_pending.at(idx)){
        ...
        //            std::thread *worker = new std::thread(&LcmTrnCtx::tcli_start_worker_fn, this, trnc,  std::move(*con_promise));
        //            std::thread *worker = new std::thread(&LcmTrnCtx::tcli_start_worker_fn,  trnc,  std::move(*con_promise));
        std::thread *worker = new std::thread(&LcmTrnCtx::tcli_start_worker_fn,  trnc,  con_promise);
...
int LcmTrnCtx::trncli_count()
{
    return mTrnCliList.size();
}

void LcmTrnCtx::dump_trnhosts()
{
...
int LcmTrnCtx::pub_trn(double nav_time, poseT *pt, measT *mt, int trn_type, std::list<lcm_pub> &pubs,  trnxpp_cfg *cfg)
{
    //std::lock_guard<std::mutex> guard(mTrnCliMutex);
    int retval = -1;
...
//            }
        ...
        // publish LCM outputs
        if(lcm_is_enabled(LCM_TRN_MOTN)){
        ...
        if(lcm_is_enabled(LCM_TRN_MEAS)){
        ...
        if(lcm_is_enabled(LCM_TRN_STAT)){
...
void LcmTrnCtx::parse_lcm_flags(const char *flags)
{
    mLcmFlags = 0;
...
void LcmTrnCtx::set_lcm_flags(uint32_t mask)
{
    mLcmFlags |= mask;
}

void LcmTrnCtx::clr_lcm_flags(uint32_t mask)
{
    mLcmFlags &= (~mask);
}

uint32_t LcmTrnCtx::lcm_flags()
{
    return mLcmFlags;
}

std::string LcmTrnCtx::lcm_flags_str()
{
    std::ostringstream ss;
...
bool LcmTrnCtx::lcm_is_enabled(lcm_flag_t mask)
{
