                                     ...
                                     const std::string& homeDir)
{
  ...
  for (dirs = 0; dirs < max_dirs; dirs++) {
    snprintf(name, sizeof(name), "%s/%4d.%03d.%03d", homeDir.c_str(),
		 ...
		 DataLog::FileFormat fileFormat)
{
  ...
  snprintf(_fileName, sizeof(_fileName), "%s/%s.log", auvLogDir, _name);
...
void DataLog::openFile()
{
  ...
  FILE *logfile;
  char origName[100];
  snprintf(origName, sizeof(origName), "%s", "");
  ...
  if (access() == Write) {
    ...
    while ((logfile = fopen(_fileName, "r")) != NULL) {
      ...
      snprintf(_fileName, sizeof(_fileName), "%s.%d",origName,numtries);
      fclose(logfile);
      numtries++;
      // exit(1);
    }
