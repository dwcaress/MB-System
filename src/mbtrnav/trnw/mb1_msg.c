...
{
    mb1_t *instance = (mb1_t *)malloc(sizeof(mb1_t));
    if(NULL!=instance){
        memset(instance,0,sizeof(mb1_t));
        instance->header.type = type;
        instance->header.size = MB1_BIN_MAX_BYTES;//MB1_BIN_HEADER_BYTES+MB1_MAX_BEAMS*MB1_BIN_SOUNDING_BYTES+MB1_BIN_CHECKSUM_BYTES;
...
void mb1_show(mb1_t *self, bool verbose, uint16_t indent)
{
    if (NULL != self) {
        ...
        fprintf(stderr,"%*s[type           %6s%04X]\n",indent,(indent>0?" ":"")," ", self->header.type);
        fprintf(stderr,"%*s[size           %10u]\n",indent,(indent>0?" ":""), self->header.size);
