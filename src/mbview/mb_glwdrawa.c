...
static XtResource resources[] = {
    ...
    {mbGLwNbufferSize, mbGLwCBufferSize, XtRInt, sizeof(int), offset(bufferSize), XtRImmediate, (XtPointer)0},

    {mbGLwNlevel, mbGLwCLevel, XtRInt, sizeof(int), offset(level), XtRImmediate, (XtPointer)0},

    {mbGLwNrgba, mbGLwCRgba, XtRBoolean, sizeof(Boolean), offset(rgba), XtRImmediate, (XtPointer)FALSE},

    {mbGLwNdoublebuffer, mbGLwCDoublebuffer, XtRBoolean, sizeof(Boolean), offset(doublebuffer), XtRImmediate, (XtPointer)FALSE},

    {mbGLwNstereo, mbGLwCStereo, XtRBoolean, sizeof(Boolean), offset(stereo), XtRImmediate, (XtPointer)FALSE},

    {mbGLwNauxBuffers, mbGLwCAuxBuffers, XtRInt, sizeof(int), offset(auxBuffers), XtRImmediate, (XtPointer)0},

    {mbGLwNredSize, mbGLwCColorSize, XtRInt, sizeof(int), offset(redSize), XtRImmediate, (XtPointer)1},

    {mbGLwNgreenSize, mbGLwCColorSize, XtRInt, sizeof(int), offset(greenSize), XtRImmediate, (XtPointer)1},

    {mbGLwNblueSize, mbGLwCColorSize, XtRInt, sizeof(int), offset(blueSize), XtRImmediate, (XtPointer)1},

    {mbGLwNalphaSize, mbGLwCAlphaSize, XtRInt, sizeof(int), offset(alphaSize), XtRImmediate, (XtPointer)0},

    {mbGLwNdepthSize, mbGLwCDepthSize, XtRInt, sizeof(int), offset(depthSize), XtRImmediate, (XtPointer)0},

    {mbGLwNstencilSize, mbGLwCStencilSize, XtRInt, sizeof(int), offset(stencilSize), XtRImmediate, (XtPointer)0},

    {mbGLwNaccumRedSize, mbGLwCAccumColorSize, XtRInt, sizeof(int), offset(accumRedSize), XtRImmediate, (XtPointer)0},

    {mbGLwNaccumGreenSize, mbGLwCAccumColorSize, XtRInt, sizeof(int), offset(accumGreenSize), XtRImmediate, (XtPointer)0},

    {mbGLwNaccumBlueSize, mbGLwCAccumColorSize, XtRInt, sizeof(int), offset(accumBlueSize), XtRImmediate, (XtPointer)0},

    {mbGLwNaccumAlphaSize, mbGLwCAccumAlphaSize, XtRInt, sizeof(int), offset(accumAlphaSize), XtRImmediate, (XtPointer)0},

    /* the attribute list */
    {mbGLwNattribList, mbGLwCAttribList, XtRPointer, sizeof(int *), offset(attribList), XtRImmediate, (XtPointer)NULL},

    /* the visual info */
    {mbGLwNvisualInfo, mbGLwCVisualInfo, mbGLwRVisualInfo, sizeof(XVisualInfo *), offset(visualInfo), XtRImmediate,
     (XtPointer)NULL},

    /* miscellaneous resources */
    {mbGLwNinstallColormap, mbGLwCInstallColormap, XtRBoolean, sizeof(Boolean), offset(installColormap), XtRImmediate,
     (XtPointer)TRUE},

    {mbGLwNallocateBackground, mbGLwCAllocateColors, XtRBoolean, sizeof(Boolean), offset(allocateBackground), XtRImmediate,
     (XtPointer)FALSE},

    {mbGLwNallocateOtherColors, mbGLwCAllocateColors, XtRBoolean, sizeof(Boolean), offset(allocateOtherColors), XtRImmediate,
     (XtPointer)FALSE},

    {mbGLwNinstallBackground, mbGLwCInstallBackground, XtRBoolean, sizeof(Boolean), offset(installBackground), XtRImmediate,
     (XtPointer)TRUE},

    {mbGLwNginitCallback, mbGLwCCallback, XtRCallback, sizeof(XtCallbackList), offset(ginitCallback), XtRImmediate,
     (XtPointer)NULL},

    {mbGLwNinputCallback, mbGLwCCallback, XtRCallback, sizeof(XtCallbackList), offset(inputCallback), XtRImmediate,
     (XtPointer)NULL},

    {mbGLwNresizeCallback, mbGLwCCallback, XtRCallback, sizeof(XtCallbackList), offset(resizeCallback), XtRImmediate,
     (XtPointer)NULL},

    {mbGLwNexposeCallback, mbGLwCCallback, XtRCallback, sizeof(XtCallbackList), offset(exposeCallback), XtRImmediate,
     (XtPointer)NULL},

    /* Changes to Motif primitive resources */
    {XmNtraversalOn, XmCTraversalOn, XmRBoolean, sizeof(Boolean), XtOffset(mbGLwDrawingAreaWidget, primitive.traversal_on),
    ...
    {XmNhighlightOnEnter, XmCHighlightOnEnter, XmRBoolean, sizeof(Boolean),
     XtOffset(mbGLwDrawingAreaWidget, primitive.highlight_on_enter), XmRImmediate, (XtPointer)FALSE},

    {XmNhighlightThickness, XmCHighlightThickness, XmRHorizontalDimension, sizeof(Dimension),
     XtOffset(mbGLwDrawingAreaWidget, primitive.highlight_thickness), XmRImmediate, (XtPointer)0},
};
...
static XtResource initializeResources[] = {
    ...
    {XtNcolormap, XtCColormap, XtRColormap, sizeof(Colormap), XtOffset(mbGLwDrawingAreaWidget, core.colormap), XtRCallProc,
     (XtPointer)createColormap},
};
...
static XtResource backgroundResources[] = {
    {XmNbackground, XmCBackground, XmRPixel, sizeof(Pixel), XtOffset(mbGLwDrawingAreaWidget, core.background_pixel), XmRString,
     (XtPointer) "lightgrey"},
    /*XmRCallProc,(XtPointer)_XmBackgroundColorDefault},*/

    {XmNbackgroundPixmap, XmCPixmap, XmRXmBackgroundPixmap, sizeof(Pixmap),
     XtOffset(mbGLwDrawingAreaWidget, core.background_pixmap), XmRImmediate, (XtPointer)XmUNSPECIFIED_PIXMAP},
};
...
static XtResource otherColorResources[] = {
    {XmNforeground, XmCForeground, XmRPixel, sizeof(Pixel), XtOffset(mbGLwDrawingAreaWidget, primitive.foreground), XmRString,
     (XtPointer) "lighgrey"},
    /*XmRCallProc, (XtPointer) _XmForegroundColorDefault},*/

    {XmNhighlightColor, XmCHighlightColor, XmRPixel, sizeof(Pixel), XtOffset(mbGLwDrawingAreaWidget, primitive.highlight_color),
     XmRString, (XtPointer) "lightgrey"},
    /*XmRCallProc,(XtPointer)_XmHighlightColorDefault},*/

    {XmNhighlightPixmap, XmCHighlightPixmap, XmRPrimHighlightPixmap, sizeof(Pixmap),
     XtOffset(mbGLwDrawingAreaWidget, primitive.highlight_pixmap), XmRImmediate, (XtPointer)XmUNSPECIFIED_PIXMAP},
    /*XmRCallProc,(XtPointer)_XmPrimitiveHighlightPixmapDefault},*/
};

#undef offset

mbGLwDrawingAreaClassRec mbglwDrawingAreaClassRec = {{/* core fields */
...
static void createAttribList(mbGLwDrawingAreaWidget w) {
	...
	*ptr++ = w->mbglwDrawingArea.level;
	if (w->mbglwDrawingArea.rgba)
		*ptr++ = GLX_RGBA;
	if (w->mbglwDrawingArea.doublebuffer)
		*ptr++ = GLX_DOUBLEBUFFER;
	if (w->mbglwDrawingArea.stereo)
		*ptr++ = GLX_STEREO;
	*ptr++ = GLX_AUX_BUFFERS;
...
static void createVisualInfo(mbGLwDrawingAreaWidget w) {
	...
	w->mbglwDrawingArea.visualInfo =
	    glXChooseVisual(XtDisplay(w), XScreenNumberOfScreen(XtScreen(w)), w->mbglwDrawingArea.attribList);
	if (!w->mbglwDrawingArea.visualInfo)
...
static void createColormap(mbGLwDrawingAreaWidget w, int offset, XrmValue *value) {
	...
	cmapCache[cacheEntries].cmap = XCreateColormap(XtDisplay(w), RootWindow(XtDisplay(w), w->mbglwDrawingArea.visualInfo->screen),
	                                               w->mbglwDrawingArea.visualInfo->visual, AllocNone);
