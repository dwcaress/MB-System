...
/// @fn mbtrn_server_t * mbtrn_server_new(msock_socket_t * s, mfile_file_t * mb_data)
    ...
    if (pself) {
        ...
        if (self) {
            if (self->auto_free) {
                ...
                mfile_file_destroy(&self->in_file);
...
static  int s_server_handle_request(mbtrn_server_t *svr, char *req, int client_fd)
{
    ...
    if (strncmp(req,"STOP",4)==0) {
        PDPRINT((stderr,"STOP received\n"));
        send(client_fd,"ACK",strlen("ACK"),0);
        svr->stop=true;
    }else if(strncmp(req,"REQ",3)==0){
        PDPRINT((stderr,"REQ received\n"));
        send(client_fd,"ACK",strlen("ACK"),0);
    }else if(strncmp(req,"SUB",3)==0){
        PDPRINT((stderr,"SUB received\n"));
    }else if( (*(uint16_t *)req)==5 ){
        PDPRINT((stderr,"7K message received\n"));
        ...
        PDPRINT((stderr,"ERR - invalid request [%s]\n",req));
        retval=-1;
    }
    return retval;
}
...
static void *s_server_main(void *arg)
{
    ...
    if ( (NULL!=svr) && (NULL!=s)) {
       ...
       while (!svr->stop && !stop_req) {
           read_fds = master;
          // PDPRINT((stderr,"pending on select\n"));
                      ...
                      // PDPRINT((stderr,"readfs [%d/%d] selected\n",i,fdmax));
                       ...
                       if (i==s->fd) {
                           ...
                           if (newfd != -1) {
                               ...
                               // accept failed
                               PDPRINT((stderr,"accept failed [%d/%s]\n",errno,strerror(errno)));
                           }
                       }else{
                           PDPRINT((stderr,"server waiting for client data fd[%d]\n",i));
                           if (( nbytes = recv(i, iobuf, sizeof iobuf, 0)) <= 0) {
                               PDPRINT((stderr,"handle client data fd[%d] nbytes[%d]\n",i,nbytes));
                               ...
                               PDPRINT((stderr,"server received request on socket [%d] [%s] len[%d]\n",i,iobuf,nbytes));
...
//                       PDPRINT((stderr,"readfs fd[%d/%d] ISSET:%s\n",i,fdmax,(FD_ISSET(i,&read_fds)?"TRUE":"FALSE")));
                   }
               }
           }else{
               // select failed
               PDPRINT((stderr,"select failed [%d/%s]\n",errno,strerror(errno)));
           }
       }
    }
    if (stop_req) {
        PDPRINT((stderr,"Test server - interrupted - stop flag set\n"));
        s->status=1;
    }else{
        PDPRINT((stderr,"Test server - normal exit\n"));
        s->status=0;
    }
...
//            sleep(1);
//        }
//        PDPRINT((stderr,"stop flag set - exiting\n"));
//    }
