...
int mbtrnpp_logparameters(int verbose,
                                FILE *logfp,
                                char *input,
                                int format,
                                char *output,
                                ...
                                int n_buffer_max,
                                int *error);
int mbtrnpp_logstatistics(int verbose,
                                FILE *logfp,
                                int n_pings_read,
                                ...
                                int *error);
int mbtrnpp_input_open(int verbose, void *mbio_ptr, char *inputname, int *error);
...
//fprintf(stderr,"CHECKING AT TOP OF LOOP: logfp:%p log_file_open_time_d:%.6ff now_time_d:%.6f\n", logfp, log_file_open_time_d, now_time_d);
            if (logfp == NULL || (now_time_d - log_file_open_time_d) > MBTRNPREPROCESS_LOGFILE_TIMELENGTH){
                if (logfp != NULL) {
                    status = mbtrnpp_logstatistics(verbose, logfp,
                                            n_pings_read,
                                            n_soundings_read,
                ...
                if (status == MB_SUCCESS) {
                    ...
                    status = mbtrnpp_logparameters(verbose, logfp,
...
int mbtrnpp_input_read(int verbose, void *mbio_ptr, size_t size, char *buffer, int *error) {
  ...
  status = MB_SUCCESS;
   ...
   /* Read the requested number of bytes (= size) off the input and  place
     ...
     * The internal buffer should be allocated in mbtrnpp_input_init() and stored
...
int mbtrnpp_input_close(int verbose, void *mbio_ptr, int *error) {
  ...
  status = MB_SUCCESS;
     ...
     * mbtrnpp_input_read(). Deallocate the internal, hidden buffer and any
     * other resources that were allocated by mbtrnpp_input_init(). */
