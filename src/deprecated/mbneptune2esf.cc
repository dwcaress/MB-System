 ...
 *      Bremen Germany
 *
 *    This program mbneptune2esf was created by:
 *    Gordon J. Keith
					...
					for (int i = 0; i < nlines; i++) {
						...
						if (MB_SUCCESS == status) {
							for (int j = 0; j < npings; j++) {
								...
								if (MB_SUCCESS == status &&
								    (0 != strncmp(word, "PING", 4) || 0 != strncmp(buff, "NO_OF_BEAMS", 11) ||
		...
		if (usable_rule) {
			...
			if (error == MB_ERROR_NO_ERROR)
				status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(char),
				                           (void **)&cur_ping.beamflagorg, &error);
			if (error == MB_ERROR_NO_ERROR)
				status =
				    mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&cur_ping.bath, &error);
			if (error == MB_ERROR_NO_ERROR)
				status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double),
				                           (void **)&cur_ping.bathacrosstrack, &error);
			if (error == MB_ERROR_NO_ERROR)
				status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double),
				                           (void **)&cur_ping.bathalongtrack, &error);
			if (error == MB_ERROR_NO_ERROR)
				status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&cur_ping.bathx,
				                           &error);
			if (error == MB_ERROR_NO_ERROR)
			...
			while (!done) {
				...
				status =
				    mb_get(verbose, mbio_ptr, &kind, &pingsread, cur_ping.time_i, &cur_ping.time_d, &cur_ping.navlon,
				           &cur_ping.navlat, &cur_ping.speed, &cur_ping.heading, &distance, &altitude, &sonardepth,
				           &cur_ping.beams_bath, &beams_amp, &pixels_ss, cur_ping.beamflag, cur_ping.bath, amp,
				           cur_ping.bathacrosstrack, cur_ping.bathalongtrack, ss, ssacrosstrack, ssalongtrack, comment, &error);
				if (verbose >= 2) {
							...
							if (mb_beam_ok(cur_ping.beamflag[beam_no])) {
								...
								if (mode <= 2) {
									...
									mb_ess_save(verbose, &esf, cur_ping.time_d,
									            beam_no + pingmultiplicity * MB_ESF_MULTIPLICITY_FACTOR, MBP_EDIT_FILTER, &error);
								}
								else {
									cur_ping.beamflag[beam_no] = MB_FLAG_NULL;
									nzero++;
									mb_ess_save(verbose, &esf, cur_ping.time_d,
									            beam_no + pingmultiplicity * MB_ESF_MULTIPLICITY_FACTOR, MBP_EDIT_ZERO, &error);
								}
							}
						...
						if (status == MB_SUCCESS || done) {
							for (int i = 0; i < cur_ping.beams_bath; i++) {
								if (cur_ping.beamflag[i] != cur_ping.beamflagorg[i]) {
									...
									mb_esf_save(verbose, &esf, cur_ping.time_d, i + pingmultiplicity * MB_ESF_MULTIPLICITY_FACTOR,
									            action, &error);
								}
							}
						}
