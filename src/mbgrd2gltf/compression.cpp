/*--------------------------------------------------------------------
 *    The MB-system:	compression.cpp	5/11/2023
 *
...
size_t get_size_in_bytes(const Matrix<float>& m) {
  return bytes_per_vertex * m.size_x() * m.size_y() +
         bytes_per_triangle * (m.size_x() - 1) * (m.size_y() - 1);
}

size_t get_compressed_bytes(const Matrix<float>& m, double stride_ratio) {
  size_t size_in_bytes = get_size_in_bytes(m);
  double compressed_size = (double)size_in_bytes / stride_ratio;

  return (size_t)compressed_size;
}

float get_average(const Matrix<float>& altitudes, size_t x_start, size_t y_start, size_t x_count,
                  size_t y_count) {
...
Matrix<float> compress(const Matrix<float>& altitudes, const Options& options) {
  size_t size_in_bytes = options.is_max_size_set()
                             ...
                             : get_compressed_bytes(altitudes, options.stride_ratio());

  Size compressed_size = get_compressed_size(altitudes, size_in_bytes);

  if (compressed_size.x < 2 || compressed_size.y < 2)
    throw std::invalid_argument("stride ratio was set high for bathymetry ");
