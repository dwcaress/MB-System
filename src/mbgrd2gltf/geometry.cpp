    ...
    for (size_t x = 0; x < end_x; ++x) {
      ...
      if (bottom_left.is_valid() && top_right.is_valid()) {
        if (top_left.is_valid())
          out.emplace_back(Triangle{bottom_left.index(), top_left.index(), top_right.index()});

        if (bottom_right.is_valid())
          out.emplace_back(Triangle{bottom_left.index(), top_right.index(), bottom_right.index()});
      } else if (bottom_right.is_valid() && top_left.is_valid()) {
        if (bottom_left.is_valid())
          out.emplace_back(Triangle{bottom_right.index(), bottom_left.index(), top_left.index()});

        if (top_right.is_valid())
          out.emplace_back(Triangle{bottom_right.index(), top_left.index(), top_right.index()});
      }
    }
  }
