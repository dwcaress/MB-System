...
namespace mbgrd2gltf {
	namespace model {
        ...
        static TileBuild build_tile_buffers(const std::vector<float>& masterVerts, const std::vector<Triangle>& tris) {
            TileBuild tb;
            if (tris.empty()) return tb;
            ...
            for (const auto& t : tris) {
                addVertex(t.a()); addVertex(t.b()); addVertex(t.c());
            }
            ...
            if (tb.useUint32) tb.idx32.reserve(tris.size() * 3);
            else              tb.idx16.reserve(tris.size() * 3);
            ...
            for (const auto& t : tris) {
                ...
                if (tb.useUint32) {
                    tb.idx32.push_back(a); tb.idx32.push_back(b); tb.idx32.push_back(c);
                } else {
        ...
        void write_gltf(const Geometry& geometry, const Options& options) {
            ...
            if (options.is_draco_compressed()) {
                if (!tiles.empty()) {
                    ...
                    std::vector<TileBuild> built; built.reserve(tiles.size());
                    ...
                    for (auto& tb : built) {
                        ...
                        if (tb.useUint32) {
                            ...
                            for (auto v : tb.idx16) idx.push_back(static_cast<uint32_t>(v));
                        }
                        ...
                        if (start != bin.size()) bin.resize(start);
                    ...
                    if (encode_ok) {
                        ...
                        tinygltf::Buffer buffer; buffer.data = std::move(bin);
                        model.buffers.push_back(std::move(buffer));

                        // Mesh + material + node + scene
                        model.meshes.push_back(std::move(mesh));
                        tinygltf::Material mat; mat.doubleSided = true; model.materials.push_back(mat);

                        tinygltf::Scene scene; tinygltf::Node node;
                        ...
                        if (!gltf.WriteGltfSceneToFile(&model, output_filepath,
                                                    options.is_binary_output(), true, true,
                    ...
                    if (dracoCompressed(model, geometry, options, master_vertex_buffer, index_buffer)) {
                        ...
                        model.scenes.push_back(scene);
                        model.defaultScene = 0;
                        model.asset.version = "2.0"; model.asset.generator = "tinygltf";
                        tinygltf::TinyGLTF gltf;
                        if (!gltf.WriteGltfSceneToFile(&model, output_filepath,
                                                    options.is_binary_output(), true, true,
                                                    options.is_binary_output())) {
            ...
            if (tiles.empty()) {
                ...
                tinygltf::Mesh mesh; mesh.primitives.push_back(get_primitive());
                model.meshes.push_back(std::move(mesh));
            ...
            for (auto& tb : built) {
                ...
                size_t start = align4(bin.size());
                if (start != bin.size()) bin.resize(start);
                if (tb.useUint32) append_bytes(bin, tb.idx32);
                else              append_bytes(bin, tb.idx16);
                const size_t idxBytes = tb.useUint32 ? tb.idx32.size() * sizeof(uint32_t)
                                                    : tb.idx16.size() * sizeof(uint16_t);
