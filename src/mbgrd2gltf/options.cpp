...
double parse_value(const char* token, const char* var_name) {
  ...
  if (return_value == EOF || return_value == 0)
    throw std::invalid_argument("invalid " + std::string(var_name) + ": " + std::string(token));
...
const char* get_value_token(const char** args, unsigned size, unsigned& i, const char* var_name) {
  if (i + 1 >= size)
    throw std::invalid_argument(std::string(var_name) + " is required after '" +
                                std::string(args[i]) + "'");
...
void Options::arg_draco_quantization(const char** args, unsigned size, unsigned& i) {
  if (!_is_draco_compressed)
    throw std::invalid_argument(
        "Quantization settings can only be specified if Draco compression is enabled");
  ...
  if (quant_type == "-qp")
    _draco_quantization[0] = value;
  else if (quant_type == "-qn")
    _draco_quantization[1] = value;
  else if (quant_type == "-qt")
    _draco_quantization[2] = value;
  else if (quant_type == "-qc")
    _draco_quantization[3] = value;
  else
    throw std::invalid_argument("Invalid quantization type specified: " + quant_type);
}
...
void print_help() { std::cout << usage_str << std::endl << help_str << std::endl; }
...
Options::Options(unsigned argc, const char* argv[]) {
  ...
  for (unsigned i = 0; i < args_size; ++i) {
    ...
    if (callback_iter == arg_callbacks.end())
      throw std::invalid_argument("unexpected token '" + std::string(arg) + "'\n" +
