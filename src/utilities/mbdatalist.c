...
int main(int argc, char **argv) {
	...
	while ((c = getopt_long(argc, argv, "VvHhCcDdF:f:I:i:L:l:NnOoPpQqR:r:SsUuYyZz", options, &option_index)) != -1)
		switch (c) {
		/* long options */
		case 0:
			/* verbose */
	...
	/* else parse datalist */
		...
		while ((status = mb_datalist_read3(verbose, datalist, &pstatus, file, pfile, dfile, &format, &file_weight, &has_bounds, file_bounds, &error)) == MB_SUCCESS) {
			nfile++;
