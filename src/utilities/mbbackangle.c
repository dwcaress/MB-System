...
[-Akind -Bmode[/beamwidth/depression] -Fformat -Ggridmode/angle/min/max/n_columns/n_rows  -Ksmooth \
-Nnangles/angle_max -Ppings -Q -Rrefangle -Ttopogridfile -Xacross/along -Zaltitude -V -H]";
	...
	while (read_data == MB_YES) {
        ...
        mb_pr_get_abscorr(verbose, swathfile, &mbp_sap_mode, &mbp_sap_src, &mbp_sap_use, mbp_sap_profile, &mbp_sa_old, &mbp_sa_new, &error);
		...
		while (error <= MB_ERROR_NO_ERROR) {
			...
			if (error == MB_ERROR_NO_ERROR || error == MB_ERROR_TIME_GAP) {
				...
				if (mbp_sap_mode == MBP_SAP_ON && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA) {
		...
		if (amplitude_on == MB_YES)
			status = mb_pr_update_ampcorr(verbose, swathfile, MB_YES, amptablefile, amp_corr_type, corr_symmetry, ref_angle,
										  amp_corr_slope, amp_corr_area, grid.file, &error);
		...
		if (sidescan_on == MB_YES)
			status = mb_pr_update_sscorr(verbose, swathfile, MB_YES, sstablefile, ss_corr_type, corr_symmetry, ref_angle,
										 ss_corr_slope, ss_corr_area, grid.file, &error);
