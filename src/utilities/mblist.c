...
int mb_get_raw(int verbose, void *mbio_ptr, int *mode, int *ipulse_length, int *png_count, double *ssv, 
               int *sample_rate, double *depression, int *range, double *bs, double *absorption,
               int *max_range, int *r_zero, int *r_zero_corr, int *tvg_start, int *tvg_stop, double *bsn, double *bso, int *tx,
               int *tvg_crossover, int *nbeams_ss, int *npixels, int *sort, int *beam_samples, int *start_sample, int *center_sample, double *ss_pixels, int *error);
int mb_get_raw_simrad2(int verbose, void *mbio_ptr, int *mode, int *ipulse_length, int *png_count, double *ssv, 
               int *sample_rate, double *depression, int *range, double *bs, double *absorption,
               int *max_range, int *r_zero, int *r_zero_corr, int *tvg_start, int *tvg_stop, double *bsn, double *bso, int *tx,
               int *tvg_crossover, int *nbeams_ss, int *npixels, int *sort, int *beam_samples, int *start_sample, int *center_sample, double *ss_pixels, int *error);
int mb_get_raw_simrad3(int verbose, void *mbio_ptr, int *mode, int *ipulse_length, int *png_count, 
               int *sample_rate, double *depression, int *range, double *bs, double *absorption,
               int *max_range, int *r_zero, int *r_zero_corr, int *tvg_start, int *tvg_stop, double *bsn, double *bso, int *tx,
               int *tvg_crossover, int *nbeams_ss, int *npixels, int *sort, int *beam_samples, int *start_sample, int *center_sample, double *ss_pixels, int *error);
...
int main(int argc, char **argv) {
	...
	while (read_data == MB_YES) {
		...
		if (error == MB_ERROR_NO_ERROR)
			status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(int), (void **)&center_sample, &error);
		if (error == MB_ERROR_NO_ERROR)
			status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(int), (void **)&sort, &error);
		if (error == MB_ERROR_NO_ERROR)
		...
		if (error == MB_ERROR_NO_ERROR)
			status = mb_register_array(verbose, mbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&depthacrosstrack, &error);
		if (error == MB_ERROR_NO_ERROR)
		...
		while (error <= MB_ERROR_NO_ERROR) {
			...
			if (error == MB_ERROR_NO_ERROR && use_raw == MB_YES) {
				status = mb_get_raw(verbose, mbio_ptr, &mode, &ipulse_length, &png_count, &ssv, &sample_rate, depression, range, bs, &absorption, &max_range,
				                    &r_zero, &r_zero_corr, &tvg_start, &tvg_stop, &bsn, &bso, &tx, &tvg_crossover, &nbeams_ss,
				                    &npixels, sort, beam_samples, start_sample, center_sample, ss_pixels, &error);
			}
						...
						for (i = 0; i < n_list; i++) {
							...
							else /* raw_next_value */
							{
								...
								case 'b': /* BSO - Oblique backscatter */
									...
									break;
								case 'C': /* Sound speed */
									printsimplevalue(verbose, output[i], ssv, 5, 1, ascii, &invert_next_value, 
													 &signflip_next_value, &error);
									raw_next_value = MB_NO;
									break;
								case 'c': /* Mean backscatter */
								...
								case 'X': /* TVG crossover */
									printsimplevalue(verbose, output[i], tvg_crossover, 5, 1, ascii, &invert_next_value, 
													 &signflip_next_value, &error);
									raw_next_value = MB_NO;
									break;
								case 'x': /* Tx beamwidth */
									printsimplevalue(verbose, output[i], tx, 5, 1, ascii, &invert_next_value, 
													 &signflip_next_value, &error);
									raw_next_value = MB_NO;
									break;
...
int mb_get_raw(int verbose, void *mbio_ptr, int *mode, int *ipulse_length, int *png_count, double *ssv, 
	...
	*npixels = 0;
	...
	case MBF_EM300MBA:
	case MBF_EM300RAW:
		mb_get_raw_simrad2(verbose, mbio_ptr, mode, ipulse_length, png_count, ssv, sample_rate, depression, range, bs, absorption, max_range, r_zero,
						   r_zero_corr, tvg_start, tvg_stop, bsn, bso, tx, tvg_crossover, nbeams_ss, npixels, sort, beam_samples,
		                   start_sample, center_sample, ss_pixels, error);

		break;
	case MBF_EM710MBA:
	case MBF_EM710RAW:
		mb_get_raw_simrad3(verbose, mbio_ptr, mode, ipulse_length, png_count, sample_rate, depression, range, bs, absorption, max_range, r_zero,
						   r_zero_corr, tvg_start, tvg_stop, bsn, bso, tx, tvg_crossover, nbeams_ss, npixels, sort, beam_samples,
		                   start_sample, center_sample, ss_pixels, error);
