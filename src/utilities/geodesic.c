...
static real epsilon, realmin, pi, degree, NaN, tiny, tol0, tol1, tol2, tolb, xthresh;
...
static real hypotx(real x, real y) { return sqrt(x * x + y * y); }
...
static real minx(real x, real y) { return x < y ? x : y; }

static real maxx(real x, real y) { return x > y ? x : y; }
...
static real AngNormalize(real x) { return x >= 180 ? x - 360 : (x < -180 ? x + 360 : x); }
static real AngNormalize2(real x) { return AngNormalize(fmod(x, (real)(360))); }
...
static void Lengths(const struct geod_geodesic *g, real eps, real sig12, real ssig1, real csig1, real dn1, real ssig2, real csig2,
                    real dn2, real cbet1, real cbet2, real *ps12b, real *pm12b, real *pm0, bool scalep, real *pM12, real *pM21,
                    /* Scratch areas of the right size */
                    real C1a[], real C2a[]);
static real Astroid(real x, real y);
static real InverseStart(const struct geod_geodesic *g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2,
                         real lam12, real *psalp1, real *pcalp1,
                         ...
                         /* Scratch areas of the right size */
                         real C1a[], real C2a[]);
static real Lambda12(const struct geod_geodesic *g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2,
                     real salp1, real calp1, real *psalp2, real *pcalp2, real *psig12, real *pssig1, real *pcsig1, real *pssig2,
                     real *pcsig2, real *peps, real *pdomg12, bool diffp, real *pdlam12,
                     /* Scratch areas of the right size */
...
void geod_init(struct geod_geodesic *g, real a, real f) {
	...
	g->b = g->a * g->f1;
	g->c2 =
	    (sq(g->a) + sq(g->b) * (g->e2 == 0 ? 1 : (g->e2 > 0 ? atanhx(sqrt(g->e2)) : atan(sqrt(-g->e2))) / sqrt(fabs(g->e2)))) /
	...
	g->etol2 = 0.1 * tol2 / sqrt(maxx((real)(0.001), fabs(g->f)) * minx((real)(1), 1 - g->f / 2) / 2);
...
void geod_lineinit(struct geod_geodesicline *l, const struct geod_geodesic *g, real lat1, real lon1, real azi1, unsigned caps) {
	...
	l->caps =
	    (caps ? caps : GEOD_DISTANCE_IN | GEOD_LONGITUDE) | GEOD_LATITUDE | GEOD_AZIMUTH; /* Always allow latitude and azimuth */
...
real geod_genposition(const struct geod_geodesicline *l, bool arcmode, real s12_a12, real *plat2, real *plon2, real *pazi2,
                      real *ps12, real *pm12, real *pM12, real *pM21, real *pS12) {
	real lat2 = 0, lon2 = 0, azi2 = 0, s12 = 0, m12 = 0, M12 = 0, M21 = 0, S12 = 0;
	/* Avoid warning about uninitialized B12. */
	real sig12, ssig12, csig12, B12 = 0, AB1 = 0;
	real omg12, lam12, lon12;
	real ssig2, csig2, sbet2, cbet2, somg2, comg2, salp2, calp2, dn2;
	unsigned outmask = (plat2 ? GEOD_LATITUDE : 0U) | (plon2 ? GEOD_LONGITUDE : 0U) | (pazi2 ? GEOD_AZIMUTH : 0U) |
	                   (ps12 ? GEOD_DISTANCE : 0U) | (pm12 ? GEOD_REDUCEDLENGTH : 0U) | (pM12 || pM21 ? GEOD_GEODESICSCALE : 0U) |
	                   (pS12 ? GEOD_AREA : 0U);

	outmask &= l->caps & OUT_ALL;
	if (!(true /*Init()*/ && (arcmode || (l->caps & GEOD_DISTANCE_IN & OUT_ALL))))
	...
	if (arcmode) {
		...
		real tau12 = s12_a12 / (l->b * (1 + l->A1m1)), s = sin(tau12), c = cos(tau12);
		/* tau2 = tau1 + tau12 */
		B12 = -SinCosSeries(true, l->stau1 * c + l->ctau1 * s, l->ctau1 * c - l->stau1 * s, l->C1pa, nC1p);
		...
		if (fabs(l->f) > 0.01) {
			...
			real ssig2 = l->ssig1 * csig12 + l->csig1 * ssig12, csig2 = l->csig1 * csig12 - l->ssig1 * ssig12, serr;
	...
	if (outmask & GEOD_LONGITUDE) {
		lam12 = omg12 + l->A3c * (sig12 + (SinCosSeries(true, ssig2, csig2, l->C3a, nC3 - 1) - l->B31));
		lon12 = lam12 / degree;
	...
	if (outmask & (GEOD_REDUCEDLENGTH | GEOD_GEODESICSCALE)) {
		real B22 = SinCosSeries(true, ssig2, csig2, l->C2a, nC2), AB2 = (1 + l->A2m1) * (B22 - l->B21),
	...
	if (outmask & GEOD_AREA) {
		real B42 = SinCosSeries(false, ssig2, csig2, l->C4a, nC4);
		real salp12, calp12;
...
void geod_position(const struct geod_geodesicline *l, real s12, real *plat2, real *plon2, real *pazi2) {
	geod_genposition(l, false, s12, plat2, plon2, pazi2, 0, 0, 0, 0, 0);
}

real geod_gendirect(const struct geod_geodesic *g, real lat1, real lon1, real azi1, bool arcmode, real s12_a12, real *plat2,
                    real *plon2, real *pazi2, real *ps12, real *pm12, real *pM12, real *pM21, real *pS12) {
	...
	return geod_genposition(&l, arcmode, s12_a12, plat2, plon2, pazi2, ps12, pm12, pM12, pM21, pS12);
}

void geod_direct(const struct geod_geodesic *g, real lat1, real lon1, real azi1, real s12, real *plat2, real *plon2,
                 real *pazi2) {
	geod_gendirect(g, lat1, lon1, azi1, false, s12, plat2, plon2, pazi2, 0, 0, 0, 0, 0);
}

real geod_geninverse(const struct geod_geodesic *g, real lat1, real lon1, real lat2, real lon2, real *ps12, real *pazi1,
                     real *pazi2, real *pm12, real *pM12, real *pM21, real *pS12) {
	...
	unsigned outmask = (ps12 ? GEOD_DISTANCE : 0U) | (pazi1 || pazi2 ? GEOD_AZIMUTH : 0U) | (pm12 ? GEOD_REDUCEDLENGTH : 0U) |
	                   (pM12 || pM21 ? GEOD_GEODESICSCALE : 0U) | (pS12 ? GEOD_AREA : 0U);
	...
	if (meridian) {
		...
		{
			real dummy;
			Lengths(g, g->n, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, &s12x, &m12x, &dummy,
	...
	if (!meridian && sbet1 == 0 && /* and sbet2 == 0 */
		...
		real dnm = 0;
		sig12 = InverseStart(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, &salp1, &calp1, &salp2, &calp2, &dnm, C1a, C2a);
		...
		if (sig12 >= 0) {
			...
			for (tripn = false, tripb = false; numit < maxit2; ++numit) {
				...
				real dv, v = (Lambda12(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1, &salp2, &calp2, &sig12, &ssig1,
				                       &csig1, &ssig2, &csig2, &eps, &omg12, numit < maxit1, &dv, C1a, C2a, C3a) -
				              lam12);
				/* 2 * tol0 is approximately 1 ulp for a number in [0, pi]. */
				/* Reversed test to allow escape with NaNs */
				if (tripb || !(fabs(v) >= (tripn ? 8 : 2) * tol0))
					break;
				...
				if (numit < maxit1 && dv > 0) {
					real dalp1 = -v / dv;
					real sdalp1 = sin(dalp1), cdalp1 = cos(dalp1), nsalp1 = salp1 * cdalp1 + calp1 * sdalp1;
				...
				SinCosNorm(&salp1, &calp1);
				tripn = false;
				tripb = (fabs(salp1a - salp1) + (calp1a - calp1) < tolb || fabs(salp1 - salp1b) + (calp1 - calp1b) < tolb);
			}
			{
				real dummy;
				Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, &s12x, &m12x, &dummy,
				        (outmask & GEOD_GEODESICSCALE) != 0U, &M12, &M21, C1a, C2a);
			}
			m12x *= g->b;
			s12x *= g->b;
	...
	if (outmask & GEOD_AREA) {
		...
		if (calp0 != 0 && salp0 != 0) {
			real
			    ...
			    ssig1 = sbet1,
			    csig1 = calp1 * cbet1, ssig2 = sbet2, csig2 = calp2 * cbet2, k2 = sq(calp0) * g->ep2,
		...
		if (!meridian && omg12 < (real)(0.75) * pi && /* Long difference too big */
		    sbet2 - sbet1 < (real)(1.75)) {           /* Lat difference too big */
			/* Use tan(Gamma/2) = tan(omg12/2)
			 * * (tan(bet1/2)+tan(bet2/2))/(1+tan(bet1/2)*tan(bet2/2))
			 * with tan(x/2) = sin(x)/(1+cos(x)) */
			real somg12 = sin(omg12), domg12 = 1 + cos(omg12), dbet1 = 1 + cbet1, dbet2 = 1 + cbet2;
			alp12 = 2 * atan2(somg12 * (sbet1 * dbet2 + sbet2 * dbet1), domg12 * (sbet1 * sbet2 + dbet1 * dbet2));
		}
		else {
			/* alp12 = alp2 - alp1, used in atan2 so no need to normalize */
			real salp12 = salp2 * calp1 - calp2 * salp1, calp12 = calp2 * calp1 + salp2 * salp1;
			/* The right thing appears to happen if alp1 = +/-180 and alp2 = 0, viz
...
void geod_inverse(const struct geod_geodesic *g, real lat1, real lon1, real lat2, real lon2, real *ps12, real *pazi1,
                  real *pazi2) {
	geod_geninverse(g, lat1, lon1, lat2, lon2, ps12, pazi1, pazi2, 0, 0, 0, 0);
}

real SinCosSeries(bool sinp, real sinx, real cosx, const real c[], int n) {
	/* Evaluate
...
void Lengths(const struct geod_geodesic *g, real eps, real sig12, real ssig1, real csig1, real dn1, real ssig2, real csig2,
             real dn2, real cbet1, real cbet2, real *ps12b, real *pm12b, real *pm0, bool scalep, real *pM12, real *pM21,
             /* Scratch areas of the right size */
             real C1a[], real C2a[]) {
...
real InverseStart(const struct geod_geodesic *g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2, real lam12,
...
#endif
	bool shortline = cbet12 >= 0 && sbet12 < (real)(0.5) && cbet2 * lam12 < (real)(0.5);
	...
	salp1 = cbet2 * somg12;
	calp1 =
	    comg12 >= 0 ? sbet12 + cbet2 * sbet1 * sq(somg12) / (1 + comg12) : sbet12a - cbet2 * sbet1 * sq(somg12) / (1 - comg12);
	...
	if (shortline && ssig12 < g->etol2) {
		...
		sig12 = atan2(ssig12, csig12);
	         ...
	         csig12 >= 0 || ssig12 >= 6 * fabs(g->n) * pi * sq(cbet1)) {
		...
		if (g->f >= 0) { /* In fact f == 0 does not get here */
			/* x = dlong, y = dlat */
			{
				real k2 = sq(sbet1) * g->ep2, eps = k2 / (2 * (1 + sqrt(1 + k2)) + k2);
			...
			/* x = dlat, y = dlong */
			real cbet12a = cbet2 * cbet1 - sbet2 * sbet1, bet12a = atan2(sbet12a, cbet12a);
			real m12b, m0, dummy;
			/* In the case of lon12 = 180, this repeats a calculation made in
			 * Inverse. */
			Lengths(g, g->n, pi + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2, dn2, cbet1, cbet2, &dummy, &m12b, &m0, false, &dummy,
			        &dummy, C1a, C2a);
		...
		if (y > -tol1 && x > -1 - xthresh) {
			...
			real k = Astroid(x, y);
			real omg12a = lamscale * (g->f >= 0 ? -x * k / (1 + k) : -y * (1 + k) / k);
			somg12 = sin(omg12a);
			comg12 = -cos(omg12a);
...
real Lambda12(const struct geod_geodesic *g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2, real salp1,
              real calp1, real *psalp2, real *pcalp2, real *psig12, real *pssig1, real *pcsig1, real *pssig2, real *pcsig2,
              real *peps, real *pdomg12, bool diffp, real *pdlam12,
              /* Scratch areas of the right size */
              real C1a[], real C2a[], real C3a[]) {
	real salp2 = 0, calp2 = 0, sig12 = 0, ssig1 = 0, csig1 = 0, ssig2 = 0, csig2 = 0, eps = 0, domg12 = 0, dlam12 = 0;
	real salp0, calp0;
	...
	calp2 =
	    cbet2 != cbet1 || fabs(sbet2) != -sbet1
	        ? sqrt(sq(calp1 * cbet1) + (cbet1 < -sbet1 ? (cbet2 - cbet1) * (cbet1 + cbet2) : (sbet1 - sbet2) * (sbet1 + sbet2))) /
	              cbet2
	        : fabs(calp1);
	...
	omg12 = atan2(maxx(comg1 * somg2 - somg1 * comg2, (real)(0)), comg1 * comg2 + somg1 * somg2);
	k2 = sq(calp0) * g->ep2;
	eps = k2 / (2 * (1 + sqrt(1 + k2)) + k2);
	C3f(g, eps, C3a);
	...
	if (diffp) {
		if (calp2 == 0)
			...
			real dummy;
			Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, &dummy, &dlam12, &dummy, false, &dummy,
			        &dummy, C1a, C2a);
...
real A1m1f(real eps) {
	real eps2 = sq(eps), t = eps2 * (eps2 * (eps2 + 4) + 64) / 256;
	return (t + eps) / (1 - eps);
}

/* The coefficients C1[l] in the Fourier expansion of B1 */
void C1f(real eps, real c[]) {
	real eps2 = sq(eps), d = eps;
...
real A2m1f(real eps) {
	real eps2 = sq(eps), t = eps2 * (eps2 * (25 * eps2 + 36) + 64) / 256;
	return t * (1 - eps) - eps;
}
...
void C4coeff(struct geod_geodesic *g) {
	g->C4x[0] = (g->n * (g->n * (g->n * (g->n * (100 * g->n + 208) + 572) + 3432) - 12012) + 30030) / 45045;
...
void geod_polygon_addpoint(const struct geod_geodesic *g, struct geod_polygon *p, real lat, real lon) {
	...
	if (p->num == 0) {
		...
		real s12, S12;
		geod_geninverse(g, p->lat, p->lon, lat, lon, &s12, 0, 0, 0, 0, 0, p->polyline ? 0 : &S12);
		accadd(p->P, s12);
		if (!p->polyline) {
			accadd(p->A, S12);
...
void geod_polygon_addedge(const struct geod_geodesic *g, struct geod_polygon *p, real azi, real s) {
	if (p->num) { /* Do nothing is num is zero */
		real lat, lon, S12;
		geod_gendirect(g, p->lat, p->lon, azi, false, s, &lat, &lon, 0, 0, 0, 0, 0, p->polyline ? 0 : &S12);
		accadd(p->P, s);
		if (!p->polyline) {
			accadd(p->A, S12);
			p->crossings += transit(p->lon, lon);
...
unsigned geod_polygon_compute(const struct geod_geodesic *g, const struct geod_polygon *p, bool reverse, bool sign, real *pA,
                              real *pP) {
	...
	geod_geninverse(g, p->lat, p->lon, p->lat0, p->lon0, &s12, 0, 0, 0, 0, 0, &S12);
	if (pP)
		*pP = accsum(p->P, s12);
	acccopy(p->A, t);
	accadd(t, S12);
...
unsigned geod_polygon_testpoint(const struct geod_geodesic *g, const struct geod_polygon *p, real lat, real lon, bool reverse,
                                bool sign, real *pA, real *pP) {
	...
	for (int i = 0; i < (p->polyline ? 1 : 2); ++i) {
		...
		if (!p->polyline) {
			tempsum += S12;
			crossings += transit(i == 0 ? p->lon : lon, i != 0 ? p->lon0 : lon);
		}
	}
...
unsigned geod_polygon_testedge(const struct geod_geodesic *g, const struct geod_polygon *p, real azi, real s, bool reverse,
                               bool sign, real *pA, real *pP) {
	real perimeter, tempsum, area0;
	int crossings;
	unsigned num = p->num + 1;
	...
	crossings = p->crossings;
	{
		...
		geod_gendirect(g, p->lat, p->lon, azi, false, s, &lat, &lon, 0, 0, 0, 0, 0, &S12);
		tempsum += S12;
		crossings += transit(p->lon, lon);
		geod_geninverse(g, lat, lon, p->lat0, p->lon0, &s12, 0, 0, 0, 0, 0, &S12);
		perimeter += s12;
		tempsum += S12;
