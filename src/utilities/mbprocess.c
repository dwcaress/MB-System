 ...
 *    $Id: mbprocess.c,v 5.49 2007-10-08 16:48:07 caress Exp $
 *
...
{
	/* id variables */
	static char rcs_id[] = "$Id: mbprocess.c,v 5.49 2007-10-08 16:48:07 caress Exp $";
...
and mbedit edit save files.\n";
	...
	if (strip_comments == MB_NO)
		...
		if (process.mbp_ampcorr_slope == MBP_AMPCORR_IGNORESLOPE
			...
			status = mb_put_comment(verbose,ombio_ptr,comment,&error);
			strncpy(comment,"\0",MBP_FILENAMESIZE);
			sprintf(comment,"  Topography grid file:      %s m\n", process.mbp_ampsscorr_topofile);
			status = mb_put_comment(verbose,ombio_ptr,comment,&error);
			}
		if (process.mbp_ampcorr_slope == MBP_AMPCORR_IGNORESLOPE)
	    		{
