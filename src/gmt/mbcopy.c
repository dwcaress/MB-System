...
/*--------------------------------------------------------------------*/
static int mbcopy_simrad_to_simrad2(struct GMTAPI_CTRL *API, int verbose,
	...
	if (istore != NULL && ostore != NULL && (void *)istore != (void *)ostore) {
		...
		if (istore->kind == MB_DATA_DATA && istore->ping != NULL && ostore->ping != NULL) {
			...
			if (*error == MB_ERROR_NO_ERROR && nbeams > 0 && angles_simrad != NULL) {
...
int GMT_mbcopy(void *V_API, int mode, void *args) {
	...
	if (stripmode == MBCOPY_STRIPMODE_NONE) {
		kind = MB_DATA_COMMENT;
		strncpy(comment, "", 256);
		snprintf(comment, sizeof(comment), "These data copied by program %s", THIS_MODULE_NAME);
		status = mb_put_comment(verbose, ombio_ptr, comment, &error);
		if (error == MB_ERROR_NO_ERROR)
			ocomment++;
		strncpy(comment, "", 256);
	...
	while (error <= MB_ERROR_NO_ERROR) {
		...
		if (copymode == MBCOPY_FULL && error == MB_ERROR_NO_ERROR) {
			...
			ostore_ptr = omb_io_ptr->store_data;
			status = mbcopy_simrad_to_simrad2(API, verbose, (struct mbsys_simrad_struct *)istore_ptr,
