...
case EQUIT:
	...
	break;
case OBLIQ:
	if (P->sinph0 * (sinphi = sin(lp.phi)) + P->cosph0 * cosphi * coslam < -EPS10)
		F_ERROR;
...
case N_POLE:
	coslam = -coslam;
case S_POLE:
	if (fabs(lp.phi - P->phi0) - EPS10 > HALFPI)
		F_ERROR;
	xy.y = cosphi * coslam;
	break;
}
...
if (fabs(rh) <= EPS10) {
	...
	lp.lam = (xy.y == 0. && (P->mode == OBLIQ || P->mode == EQUIT)) ? (xy.x == 0. ? 0. : xy.x < 0. ? -HALFPI : HALFPI)
	                                                                : atan2(xy.x, xy.y);
}
return (lp);
}
