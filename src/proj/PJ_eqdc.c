...
P->rho = P->c - (P->ellips ? pj_mlfn(lp.phi, sin(lp.phi), cos(lp.phi), P->en) : lp.phi);
...
SPECIAL(fac) {
	...
	fac->code |= IS_ANAL_HK;
	fac->h = 1.;
	fac->k = P->n * (P->c - (P->ellips ? pj_mlfn(lp.phi, sinphi, cosphi, P->en) : lp.phi)) / pj_msfn(sinphi, cosphi, P->es);
}
FREEUP;
if (P) {
	if (P->en)
...
if ((P->ellips = (P->es > 0.))) {
	...
	if (secant) { /* secant cone */
		sinphi = sin(P->phi2);
		cosphi = cos(P->phi2);
		P->n = (m1 - pj_msfn(sinphi, cosphi, P->es)) / (pj_mlfn(P->phi2, sinphi, cosphi, P->en) - ml1);
	}
	P->c = ml1 + m1 / P->n;
	P->rho0 = P->c - pj_mlfn(P->phi0, sin(P->phi0), cos(P->phi0), P->en);
}
else {
	if (secant)
