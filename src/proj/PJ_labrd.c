...
#define EPS 1.e-10
FORWARD(e_forward);
double V1, V2, ps, sinps, cosps, sinps2, cosps2, I1, I2, I3, I4, I5, I6, x2, y2, t;
...
I6 *= P->A * P->A * (5. * cosps2 * cosps2 + sinps2 * (sinps2 - 18. * cosps2)) / 120.;
t = lp.lam * lp.lam;
...
double x2, y2, V1, V2, V3, V4, t, t2, ps, pe, tpe, s, I7, I8, I9, I10, I11, d, Re;
int i;
...
P->A = sinp / sin(P->p0s);
t = P->e * sinp;
P->C = .5 * P->e * P->A * log((1. + t) / (1. - t)) + -P->A * log(tan(FORTPI + .5 * P->phi0)) + log(tan(FORTPI + .5 * P->p0s));
t = Az + Az;
