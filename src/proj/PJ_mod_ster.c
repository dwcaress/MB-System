...
chi = 2. * atan(tan((HALFPI + lp.phi) * .5) * pow((1. - esphi) / (1. + esphi), P->e * .5)) - HALFPI;
schi = sin(chi);
cchi = cos(chi);
...
if (nn) {
	...
	for (nn = 20; nn; --nn) {
		esphi = P->e * sin(phi);
		dphi = 2. * atan(tan((HALFPI + chi) * .5) * pow((1. + esphi) / (1. - esphi), P->e * .5)) - HALFPI - phi;
		phi += dphi;
		if (fabs(dphi) <= EPSLN)
			break;
	}
...
if (nn) {
	lp.phi = phi;
	lp.lam = atan2(p.r * sinz, rh * P->cchio * cosz - p.i * P->schio * sinz);
}
else
...
static PJ *setup(PJ *P) { /* general initialization */
	...
	if (P->es) {
		...
		chio = 2. * atan(tan((HALFPI + P->phi0) * .5) * pow((1. - esphi) / (1. + esphi), P->e * .5)) - HALFPI;
	}
	else
		chio = P->phi0;
	P->schio = sin(chio);
...
static COMPLEX /* Lee Oblated Stereographic */
    AB[] = {{0.721316, 0.}, {0., 0.}, {-0.0088162, -0.00617325}};
...
static COMPLEX /* 48 United States */
    AB[] = {{0.98879, 0.}, {0., 0.}, {-0.050909, 0.}, {0., 0.}, {0.075528, 0.}};
...
static COMPLEX ABe[] =
    {/* Alaska ellipsoid */
     ...
     {-.0151089, -.1932526}, {.0642675, -.1381226}, {.3582802, -.2884586}},
               ABs[] = {/* Alaska sphere */
                        {.9972523, 0.},         {.0052513, -.0041175}, {.0074606, .0048125},
                        {-.0153783, -.1968253}, {.0636871, -.1408027}, {.3660976, -.2937382}};
...
static COMPLEX ABe[] =
    {/* GS50 ellipsoid */
     {.9827497, 0.},       {.0210669, .0053804},   {-.1031415, -.0571664}, {-.0323337, -.0322847}, {.0502303, .1211983},
     {.0251805, .0895678}, {-.0012315, -.1416121}, {.0072202, -.1317091},  {-.0194029, .0759677},  {-.0210072, .0834037}},
               ABs[] = {/* GS50 sphere */
                        {.9842990, 0.},        {.0211642, .0037608}, {-.1036018, -.0575102}, {-.0329095, -.0320119},
                        {.0499471, .1223335},  {.0260460, .0899805}, {.0007388, -.1435792},  {.0075848, -.1334108},
                        {-.0216473, .0776645}, {-.0225161, .0853673}};
