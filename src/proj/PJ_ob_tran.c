...
sinphi = sin(lp.phi);
cosphi = cos(lp.phi);
lp.lam = adjlon(aatan2(cosphi * sin(lp.lam), P->sphip *cosphi *coslam + P->cphip * sinphi) + P->lamp);
...
if (lp.lam != HUGE_VAL) {
	...
	cosphi = cos(lp.phi);
	lp.phi = aasin(P->ctx, P->sphip * sinphi + P->cphip * cosphi * coslam);
	lp.lam = aatan2(cosphi * sin(lp.lam), P->sphip * cosphi * coslam - P->cphip * sinphi);
}
return (lp);
}
...
if (pj_param(P->ctx, P->params, "to_alpha").i) {
	...
	*/
	...
	if (fabs(phi1 - phi2) <= TOL || (con = fabs(phi1)) <= TOL || fabs(con - HALFPI) <= TOL || fabs(fabs(phi2) - HALFPI) <= TOL)
		E_ERROR(-33);
	P->lamp = atan2(cos(phi1) * sin(phi2) * cos(lam1) - sin(phi1) * cos(phi2) * cos(lam2),
