...
switch (P->mode) {
case OBLIQ:
	xy.y = P->ymf * (b = sqrt(2. / b)) * (P->cosb1 * sinb - P->sinb1 * cosb * coslam);
	goto eqcon;
	break;
case EQUIT:
...
case EQUIT:
	...
	break;
case OBLIQ:
	lp.phi = fabs(rh) <= EPS10 ? P->phi0 : asin(cosz * sinph0 + xy.y * sinz * cosph0 / rh);
	xy.x *= sinz * cosph0;
...
lp.lam = (xy.y == 0. && (P->mode == EQUIT || P->mode == OBLIQ)) ? 0. : atan2(xy.x, xy.y);
return (lp);
}
FREEUP;
if (P) {
...
if (P->es) {
	...
	case OBLIQ:
		...
		P->dd = cos(P->phi0) / (sqrt(1. - P->es * sinphi * sinphi) * P->rq * P->cosb1);
