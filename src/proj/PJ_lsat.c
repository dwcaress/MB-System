...
static void seraz0(double lam, double mult, PJ *P) {
	...
	sd = sin(lam);
	sdsq = sd * sd;
	s = P->p22 * P->sa * cos(lam) * sqrt((1. + P->t * sdsq) / ((1. + P->w * sdsq) * (1. + P->q * sdsq)));
	d__1 = 1. + P->q * sdsq;
	h = sqrt((1. + P->q * sdsq) / (1. + P->w * sdsq)) * ((1. + P->w * sdsq) / (d__1 * d__1) - P->p22 * P->ca);
...
int l, nn;
double lamt, xlam, sdsq, c, d, s, lamdp, phidp, lampp, tanph, lamtp, cl, sd, sp, fac, sav, tanphi;
...
if (l) {
	sp = sin(lp.phi);
	phidp = aasin(P->ctx, (P->one_es * P->ca * sp - P->sa * cos(lp.phi) * sin(lamt)) / sqrt(1. - P->es * sp * sp));
	tanph = log(tan(FORTPI + .5 * phidp));
	sd = sin(lamdp);
	sdsq = sd * sd;
	s = P->p22 * P->sa * cos(lamdp) * sqrt((1. + P->t * sdsq) / ((1. + P->w * sdsq) * (1. + P->q * sdsq)));
	d = sqrt(P->xj * P->xj + s * s);
	xy.x = P->b * lamdp + P->a2 * sin(2. * lamdp) + P->a4 * sin(lamdp * 4.) - tanph * s / d;
...
do {
	...
	sdsq = sd * sd;
	s = P->p22 * P->sa * cos(lamdp) * sqrt((1. + P->t * sdsq) / ((1. + P->w * sdsq) * (1. + P->q * sdsq)));
	lamdp = xy.x + xy.y * s / P->xj - P->a2 * sin(2. * lamdp) - P->a4 * sin(lamdp * 4.) -
	        s / P->xj * (P->c1 * sin(lamdp) + P->c3 * sin(lamdp * 3.));
	lamdp /= P->b;
} while (fabs(lamdp - sav) >= TOL && --nn);
sl = sin(lamdp);
fac = exp(sqrt(1. + s * s / P->xj / P->xj) * (xy.y - P->c1 * sl - P->c3 * sin(lamdp * 3.)));
...
spp = sin(phidp);
sppsq = spp * spp;
lamt = atan(((1. - sppsq * P->rone_es) * tan(lamdp) * P->ca -
             spp * P->sa * sqrt((1. + P->q * dd) * (1. - sppsq) - sppsq * P->u) / cos(lamdp)) /
            (1. - sppsq * (1. + P->u)));
sl = lamt >= 0. ? 1. : -1.;
scl = cos(lamdp) >= 0. ? 1. : -1;
...
if (fabs(P->sa) < TOL)
	...
	lp.phi = atan((tan(lamdp) * cos(lamt) - P->ca * sin(lamt)) / (P->one_es * P->sa));
return lp;
}
FREEUP;
if (P)
