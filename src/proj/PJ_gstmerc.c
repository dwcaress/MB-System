...
/*fprintf(stderr,"fwd:\nL      =%16.13f\nLs     =%16.13f\nLs1
 * =%16.13f\nLP(%16.13f,%16.13f)=XY(%16.4f,%16.4f)\n",L,Ls,Ls1,lp.lam+P->lam0,lp.phi,(xy.x*P->a + P->x0)*P->to_meter,(xy.y*P->a +
 * P->y0)*P->to_meter);*/
return (xy);
}
...
P->c = log(pj_tsfn(-1.0 * P->phic, 0.0, 0.0)) - P->n1 * log(pj_tsfn(-1.0 * P->phi0, -1.0 * sin(P->phi0), P->e));
 ...
 * (m)=%16.4f\n\nlC(rad)=%16.13f\npC(rad)=%16.13f\nc      =%16.13f\nn1     =%16.13f\nn2 (m) =%16.4f\nXS (m) =%16.4f\nYS (m)
 * =%16.4f\n", P->a, P->e, P->lam0, P->phi0, P->k0, P->x0, P->y0, P->lamc, P->phic, P->c, P->n1, P->n2, P->XS +P->x0, P->YS +
 * P->y0);*/
ENDENTRY(P)
