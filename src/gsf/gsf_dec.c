...
static int DecodeFromByteToUnsignedShortArray(unsigned short **array, unsigned char *sptr, int num_beams, gsfScaleFactors *sf,
                                              int id, int handle);
 ...
 *
 ********************************************************************/
int gsfDecodeHeader(gsfHeader *header, unsigned char *sptr) {
 ...
 *
 ********************************************************************/
int gsfDecodeSwathBathySummary(gsfSwathBathySummary *sum, unsigned char *sptr) {
	gsfuLong ltemp;
 ...
 *
 ********************************************************************/
int gsfDecodeSinglebeam(gsfSingleBeamPing *ping, unsigned char *sptr, GSF_FILE_TABLE *ft, int handle, int record_size) {
	gsfuLong ltemp;
	int subrecord_size;
	int subrecord_id;
	...
	while ((record_size - bytes) > 4) {
		...
		default:
			...
			if ((((p - sptr) + subrecord_size) == record_size) || ((record_size - ((p - sptr) + subrecord_size)) > 0))
				p += subrecord_size;
			else
				return (-1);
			break;
 ...
 *
 ********************************************************************/
int gsfDecodeSwathBathymetryPing(gsfSwathBathyPing *ping, unsigned char *sptr, GSF_FILE_TABLE *ft, int handle, int record_size) {
	gsfuLong ltemp;
	int subrecord_size;
	int subrecord_id;
	gsfsShort signed_short;
	...
	while ((record_size - bytes) > 4) {
		...
		while (((record_size - bytes - sr_size) > 4) && (count <= 3)) {
			...
			if ((next_id > 0) && (next_id <= GSF_MAX_PING_ARRAY_SUBRECORDS) &&
			    ((next_size == ping->number_beams) || (next_size == 2 * ping->number_beams) ||
			     (next_size == 4 * ping->number_beams))) {
		...
		/* Clear the high order 4 bits of the compression flag field, then set these bits to specify the field size we have just
		 * decoded. The field size is determined above just for the beam-array subcords. (I.E. not for the scale factor or sensor
		 * specific subrecords.)
		 */
		if ((subrecord_id > 0) && (subrecord_id <= GSF_MAX_PING_ARRAY_SUBRECORDS)) {
			...
			ping->scaleFactors.scaleTable[subrecord_id - 1].compressionFlag =
			    ft->rec.mb_ping.scaleFactors.scaleTable[subrecord_id - 1].compressionFlag;
		}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_DEPTH_ARRAY):
			...
			case GSF_FIELD_SIZE_TWO:
				ret = DecodeTwoByteArray(&ft->rec.mb_ping.depth, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                         GSF_SWATH_BATHY_SUBRECORD_DEPTH_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_FOUR:
				ret = DecodeFourByteArray(&ft->rec.mb_ping.depth, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                          GSF_SWATH_BATHY_SUBRECORD_DEPTH_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_NOMINAL_DEPTH_ARRAY):
			...
			case GSF_FIELD_SIZE_DEFAULT:
			case GSF_FIELD_SIZE_TWO:
				ret = DecodeTwoByteArray(&ft->rec.mb_ping.nominal_depth, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                         GSF_SWATH_BATHY_SUBRECORD_NOMINAL_DEPTH_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_FOUR:
				ret = DecodeFourByteArray(&ft->rec.mb_ping.nominal_depth, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                          GSF_SWATH_BATHY_SUBRECORD_NOMINAL_DEPTH_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_ACROSS_TRACK_ARRAY):
			...
			case GSF_FIELD_SIZE_TWO:
				ret =
				    DecodeSignedTwoByteArray(&ft->rec.mb_ping.across_track, (char *)p, ping->number_beams,
				                             &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_ACROSS_TRACK_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_FOUR:
				ret = DecodeSignedFourByteArray(&ft->rec.mb_ping.across_track, (char *)p, ping->number_beams,
				                                &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_ACROSS_TRACK_ARRAY,
				                                handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_ALONG_TRACK_ARRAY):
			...
			case GSF_FIELD_SIZE_TWO:
				ret =
				    DecodeSignedTwoByteArray(&ft->rec.mb_ping.along_track, (char *)p, ping->number_beams,
				                             &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_ALONG_TRACK_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_FOUR:
				ret =
				    DecodeSignedFourByteArray(&ft->rec.mb_ping.along_track, (char *)p, ping->number_beams,
				                              &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_ALONG_TRACK_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_TRAVEL_TIME_ARRAY):
			...
			case GSF_FIELD_SIZE_DEFAULT:
			case GSF_FIELD_SIZE_TWO:
				ret = DecodeTwoByteArray(&ft->rec.mb_ping.travel_time, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                         GSF_SWATH_BATHY_SUBRECORD_TRAVEL_TIME_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_FOUR:
				ret = DecodeFourByteArray(&ft->rec.mb_ping.travel_time, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                          GSF_SWATH_BATHY_SUBRECORD_TRAVEL_TIME_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_MEAN_CAL_AMPLITUDE_ARRAY):
			...
			case GSF_FIELD_SIZE_ONE:
				ret = DecodeSignedByteArray(&ft->rec.mb_ping.mc_amplitude, (char *)p, ping->number_beams,
				                            &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_MEAN_CAL_AMPLITUDE_ARRAY,
				                            handle);
				break;

			case GSF_FIELD_SIZE_TWO:
				ret = DecodeSignedTwoByteArray(&ft->rec.mb_ping.mc_amplitude, (char *)p, ping->number_beams,
				                               &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_MEAN_CAL_AMPLITUDE_ARRAY,
				                               handle);
				break;
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_MEAN_REL_AMPLITUDE_ARRAY):
			...
			case GSF_FIELD_SIZE_DEFAULT:
			case GSF_FIELD_SIZE_ONE:
				ret = DecodeByteArray(&ft->rec.mb_ping.mr_amplitude, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                      GSF_SWATH_BATHY_SUBRECORD_MEAN_REL_AMPLITUDE_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_TWO:
				ret = DecodeTwoByteArray(&ft->rec.mb_ping.mr_amplitude, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                         GSF_SWATH_BATHY_SUBRECORD_MEAN_REL_AMPLITUDE_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_ECHO_WIDTH_ARRAY):
			...
			case GSF_FIELD_SIZE_DEFAULT:
			case GSF_FIELD_SIZE_ONE:
				ret = DecodeByteArray(&ft->rec.mb_ping.echo_width, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                      GSF_SWATH_BATHY_SUBRECORD_ECHO_WIDTH_ARRAY, handle);
				break;

			case GSF_FIELD_SIZE_TWO:
				ret = DecodeTwoByteArray(&ft->rec.mb_ping.echo_width, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
				                         GSF_SWATH_BATHY_SUBRECORD_ECHO_WIDTH_ARRAY, handle);
				break;
			}
			if (ret < 0) {
		...
		case (GSF_SWATH_BATHY_SUBRECORD_QUALITY_FACTOR_ARRAY):
			ret = DecodeByteArray(&ft->rec.mb_ping.quality_factor, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                      GSF_SWATH_BATHY_SUBRECORD_QUALITY_FACTOR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_DEPTH_ERROR_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.depth_error, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_DEPTH_ERROR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_ACROSS_TRACK_ERROR_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.across_track_error, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_ACROSS_TRACK_ERROR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_ALONG_TRACK_ERROR_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.along_track_error, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_ALONG_TRACK_ERROR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_BEAM_ANGLE_FORWARD_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.beam_angle_forward, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_BEAM_ANGLE_FORWARD_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_VERTICAL_ERROR_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.vertical_error, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_VERTICAL_ERROR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_HORIZONTAL_ERROR_ARRAY):
			ret = DecodeTwoByteArray(&ft->rec.mb_ping.horizontal_error, p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                         GSF_SWATH_BATHY_SUBRECORD_HORIZONTAL_ERROR_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_SECTOR_NUMBER_ARRAY):
			...
			ret = DecodeFromByteToUnsignedShortArray(&ft->rec.mb_ping.sector_number, p, ping->number_beams,
			                                         &ft->rec.mb_ping.scaleFactors, GSF_SWATH_BATHY_SUBRECORD_SECTOR_NUMBER_ARRAY,
			                                         handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_DETECTION_INFO_ARRAY):
			ret = DecodeFromByteToUnsignedShortArray(&ft->rec.mb_ping.detection_info, p, ping->number_beams,
			                                         &ft->rec.mb_ping.scaleFactors,
			                                         GSF_SWATH_BATHY_SUBRECORD_DETECTION_INFO_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_SYSTEM_CLEANING_ARRAY):
			ret = DecodeFromByteToUnsignedShortArray(&ft->rec.mb_ping.system_cleaning, p, ping->number_beams,
			                                         &ft->rec.mb_ping.scaleFactors,
			                                         GSF_SWATH_BATHY_SUBRECORD_SYSTEM_CLEANING_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_DOPPLER_CORRECTION_ARRAY):
			ret =
			    DecodeSignedByteArray(&ft->rec.mb_ping.doppler_corr, (char *)p, ping->number_beams, &ft->rec.mb_ping.scaleFactors,
			                          GSF_SWATH_BATHY_SUBRECORD_DOPPLER_CORRECTION_ARRAY, handle);
			if (ret < 0) {
				return (-1);
			}
		...
		case (GSF_SWATH_BATHY_SUBRECORD_TYPEIII_SEABEAM_SPECIFIC):
			...
			ping->sensor_id = GSF_SWATH_BATHY_SUBRECORD_TYPEIII_SEABEAM_SPECIFIC;
			break;
#endif
...
static int DecodeScaleFactors(gsfScaleFactors *sf, unsigned char *sptr) {
	unsigned char *p = sptr;
...
static int DecodeTwoByteArray(double **array, unsigned char *sptr, int num_beams, gsfScaleFactors *sf, int id, int handle) {
	double *dptr;
	...
	for (i = 0; i < num_beams; i++) {
		...
		*dptr = (((double)temp) / sf->scaleTable[id - 1].multiplier) - sf->scaleTable[id - 1].offset;
		ptr += 2;
		dptr++;
	}
...
static int DecodeSignedTwoByteArray(double **array, char *sptr, int num_beams, gsfScaleFactors *sf, int id, int handle) {
	double *dptr;
	char *ptr = sptr;
	gsfuShort stemp;
	...
	for (i = 0; i < num_beams; i++) {
		...
		*dptr = (((double)signed_temp) / sf->scaleTable[id - 1].multiplier) - sf->scaleTable[id - 1].offset;
		ptr += 2;
		dptr++;
	}
	return (ptr - sptr);
...
static int DecodeByteArray(double **array, unsigned char *sptr, int num_beams, gsfScaleFactors *sf, int id, int handle) {
	double *dptr;
	unsigned char *ptr = sptr;
	unsigned char ctemp;
	int i;
	...
	for (i = 0; i < num_beams; i++) {
		ctemp = *ptr;
		*dptr = (((double)ctemp) / sf->scaleTable[id - 1].multiplier) - sf->scaleTable[id - 1].offset;
		ptr++;
		dptr++;
	}
	return (ptr - sptr);
...
static int DecodeFromByteToUnsignedShortArray(unsigned short **array, unsigned char *sptr, int num_beams, gsfScaleFactors *sf,
	...
	for (i = 0; i < num_beams; i++) {
		ctemp = *ptr;
		*dptr = (((unsigned short)ctemp) / sf->scaleTable[id - 1].multiplier) - sf->scaleTable[id - 1].offset;
		ptr++;
		dptr++;
	}
	return (ptr - sptr);
 ...
 *
 ********************************************************************/
static int DecodeSignedByteArray(double **array, char *sptr, int num_beams, gsfScaleFactors *sf, int id, int handle) {
	double *dptr;
...
static int DecodeBeamFlagsArray(unsigned char **array, unsigned char *sptr, int num_beams, int handle) {
	unsigned char *ptr = sptr;
	unsigned char *aptr;
	int i;
 ...
 *
 ********************************************************************/
static int DecodeQualityFlagsArray(unsigned char **array, unsigned char *sptr, int num_beams, int sr_size, int handle) {
	unsigned char *ptr = sptr;
	unsigned char *aptr;
	int i;
	int j;
...
static int DecodeEM3Specific(gsfSensorSpecific *sdata, unsigned char *sptr, GSF_FILE_TABLE *ft) {
	...
	if (run_time_id & 0x00000001) {
		...
		if (sdata->gsfEM3Specific.run_time[0].stbd_swath_width) {
			sdata->gsfEM3Specific.run_time[0].swath_width =
			    sdata->gsfEM3Specific.run_time[0].port_swath_width + sdata->gsfEM3Specific.run_time[0].stbd_swath_width;
		}
		else {
		...
		if (sdata->gsfEM3Specific.run_time[0].stbd_coverage_sector) {
			sdata->gsfEM3Specific.run_time[0].coverage_sector =
			    sdata->gsfEM3Specific.run_time[0].port_coverage_sector + sdata->gsfEM3Specific.run_time[0].stbd_coverage_sector;
		}
		else {
		...
		if (run_time_id & 0x00000002) {
			...
			if (sdata->gsfEM3Specific.run_time[1].stbd_swath_width) {
				sdata->gsfEM3Specific.run_time[1].swath_width =
				    sdata->gsfEM3Specific.run_time[1].port_swath_width + sdata->gsfEM3Specific.run_time[1].stbd_swath_width;
			}
			else {
			 ...
			 *  to save these to the file table so the'll be available to the caller for each ping.
			 *
			 * memcpy (&ft->rec.mb_ping.sensor_data.gsfEM3Specific.run_time[1], &sdata->gsfEM3Specific.run_time[1],
			 * sizeof(gsfEM3RunTime));
			 */
		}
	}
	 ...
	 * else
	 * {
	 *     memcpy (&sdata->gsfEM3Specific.run_time[0], &ft->rec.mb_ping.sensor_data.gsfEM3Specific.run_time[0],
	 * sizeof(gsfEM3RunTime)); memcpy (&sdata->gsfEM3Specific.run_time[1],
...
static int DecodeEM3RawSpecific(gsfSensorSpecific *sdata, unsigned char *sptr, GSF_FILE_TABLE *ft) {
	 ...
	 *  spare field is data structure is smaller than 16 bytes during time frame while durotong speed and hi_lo_absorption_ratio
	 * remain in the structure. This is done simply to allow the structure size to stay fixed.  When durotong speed and
	 * hi_lo_absorption_ratio are removed, size of spare array will be increased from 9 bytes to 16 bytes.
	 */
...
static int DecodeKlein5410BssImagerySpecific(gsfSensorImagery *sdata, unsigned char *sptr) {
	int i;
	unsigned char *p = sptr;
	gsfuShort stemp;
...
static int DecodeReson8100ImagerySpecific(gsfSensorImagery *sdata, unsigned char *sptr) {
	unsigned char *p = sptr;
...
static int DecodeBRBIntensity(gsfBRBIntensity **idata, unsigned char *sptr, int num_beams, int sensor_id, int handle) {
	unsigned char *ptr = sptr;
	gsfuShort stemp;
	gsfuLong ltemp;
	int i, j;
	...
	if (num_beams > arraySize[handle - 1][id - 1]) {
		(*idata)->time_series =
		    (gsfTimeSeriesIntensity *)realloc((void *)(*idata)->time_series, num_beams * sizeof(gsfTimeSeriesIntensity));
	...
	for (i = 0; i < num_beams; i++) {
		...
		if ((*idata)->time_series[i].samples == (unsigned int *)NULL) {
			(*idata)->time_series[i].samples =
			    (unsigned int *)calloc((*idata)->time_series[i].sample_count, sizeof(unsigned int));
		...
		if ((*idata)->time_series[i].sample_count > samplesArraySize[handle - 1][i]) {
			(*idata)->time_series[i].samples = (unsigned int *)realloc(
			    (void *)(*idata)->time_series[i].samples, (*idata)->time_series[i].sample_count * sizeof(unsigned int));
 ...
 *
 ********************************************************************/
int gsfDecodeSoundVelocityProfile(gsfSVP *svp, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
	gsfsLong signed_int;
	int i;
 ...
 *
 ********************************************************************/
int gsfDecodeProcessingParameters(gsfProcessingParameters *param, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
	gsfuShort stemp;
	int i;
	...
	for (i = 0; (i < param->number_parameters) && (i < GSF_MAX_PROCESSING_PARAMETERS); i++) {
		...
		if (ft->rec.process_parameters.param[i] == (char *)NULL) {
			...
			ft->rec.process_parameters.param[i] =
			    (char *)realloc((void *)ft->rec.process_parameters.param[i], param->param_size[i] + 1);
			if (ft->rec.process_parameters.param[i] == (char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
 ...
 *
 ********************************************************************/
int gsfDecodeSensorParameters(gsfSensorParameters *param, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
	gsfuShort stemp;
	int i;
	...
	for (i = 0; (i < param->number_parameters) && (i < GSF_MAX_SENSOR_PARAMETERS); i++) {
		...
		if (ft->rec.sensor_parameters.param[i] == (char *)NULL) {
			...
			ft->rec.sensor_parameters.param[i] =
			    (char *)realloc((void *)ft->rec.sensor_parameters.param[i], param->param_size[i] + 1);
			if (ft->rec.sensor_parameters.param[i] == (char *)NULL) {
				gsfError = GSF_MEMORY_ALLOCATION_FAILED;
				return (-1);
			}
 ...
 *
 ********************************************************************/
int gsfDecodeComment(gsfComment *comment, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
...
int gsfDecodeHistory(gsfHistory *history, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	int len;
	gsfuLong ltemp;
	gsfuShort stemp;
...
int gsfDecodeNavigationError(gsfNavigationError *nav_error, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
	gsfsLong signed_int;
...
int gsfDecodeHVNavigationError(gsfHVNavigationError *hv_nav_error, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	int length;
	unsigned char *p = sptr;
	gsfsShort stemp;
	gsfuShort utemp;
...
static void LocalAddTimes(struct timespec *base_time, double delta_time, struct timespec *sum_time) {
	...
	tmp = (((double)base_time->tv_nsec) / 1.0e9) + fraction;
 ...
 *
 ********************************************************************/
int gsfDecodeAttitude(gsfAttitude *attitude, GSF_FILE_TABLE *ft, unsigned char *sptr) {
	unsigned char *p = sptr;
	gsfuLong ltemp;
	gsfuShort stemp;
	gsfsShort signed_short;
	 ...
	 */
		...
		ft->rec.attitude.attitude_time =
		    (struct timespec *)realloc(ft->rec.attitude.attitude_time, attitude->num_measurements * sizeof(struct timespec));
