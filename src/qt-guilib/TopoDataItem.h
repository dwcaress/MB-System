  ...
  public:
    ...
    struct Pipeline : vtkObject {
      Pipeline() {
        ...
        //
        //   contourFilter_  →  stripper  →  pointDataToCellData
        //                   →  cellCenters  (one point per polyline)
        //                   →  transformFilter (matches surfaceActor Z scale)
        //                   →  labeledDataMapper  →  labelActor (2D overlay)
        //
      ...
      // Label sub-pipeline (stencil-free; vtkLabeledContourMapper needs a
      // stencil buffer that QQuickVTKItem's FBO does not provide).
      vtkNew<vtkStripper>                contourStripper_;       // join segs
      vtkNew<vtkPointDataToCellData>     contourPointToCell_;    // scalar→cell
      vtkNew<vtkCellCenters>             contourLabelPoints_;    // 1 pt / line
      vtkNew<vtkTransform>               contourLabelTransform_; // verticalExagg
    ...
    /// Toggle numeric labels on each contour line (only effective when
    /// contours are also enabled).
