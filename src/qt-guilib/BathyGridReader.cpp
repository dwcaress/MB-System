...
BathyGridReader::BathyGridReader() :
  fileName_(nullptr),
  gridType_(BathyGridType::Unknown),
  xUnits_(nullptr), yUnits_(nullptr), zUnits_(nullptr)
{
  ...
  cbc->SetCallback(&BathyGridReader::SelectionModifiedCallback); // what's this?
...
int BathyGridReader::RequestData(vtkInformation* request,
			       vtkInformationVector** inputVector,
			       vtkInformationVector* outputVector) {

  std::cerr << "BathyGridReader::RequestData()" << std::endl;
  ...
  // Read grid file
  std::cerr << "BathyGridReader::RequestData() - readGridfile()" << std::endl;
  ...
  double x, y, z;
  std::cerr << "BathyGridReader::RequestData() - load points" << std::endl;    
  for (row = 0; row < nRows; row++) {
    for (col = 0; col < nColumns; col++) {
      ...
      if (!convertToUTM) {
        ...
        PJ_COORD latLon = proj_coord(x, y,
                                     0, 0);
  ...
  std::cerr << "BathyGridReader::RequestData() - done" << std::endl;

  return 1;
}


void BathyGridReader::SetFileName(const char *fileName) {
  fprintf(stderr, "In BathyGridReader::SetFileName()\n");
  if (fileName_) {
    fprintf(stderr, "BathyGridReader::SetFileName: free fileName_\n");
    free((void *)fileName_);
  }
  fprintf(stderr, "BathyGridReader::SetFileName: call strdup()\n");
  fileName_ = strdup((char *)fileName);
}


void BathyGridReader::SelectionModifiedCallback(vtkObject*, unsigned long,
                                              void* clientdata, void*)
{
  static_cast<BathyGridReader*>(clientdata)->Modified();
}


void BathyGridReader::gridBounds(double *xMin, double *xMax,
...
vtkIdType BathyGridReader::gridOffset(unsigned nRows, unsigned nCols,
...
float BathyGridReader::zScaleLatLon(float latRange, float lonRange,
                                  float zRange) {
...
bool BathyGridReader::fileInUTM() {
...
BathyGridData *BathyGridReader::readGridfile(char *filename) {

  BathyGridData *grid = nullptr;
  
  switch (gridType_) {
  case BathyGridType::GMTGrid:
    grid = new GmtGridData();
    break;

    /* ***
  case BathyGridType::SwathGrid:
