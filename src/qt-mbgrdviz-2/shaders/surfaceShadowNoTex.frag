...
void main() {
    ...
    // highp float shininess = 10;  // original value
    highp float shininess = 50.;
    // highp float specIntensFactor = 1.0;   // original value
    highp float specIntensFactor = 0.2;
    gl_FragColor.rgb =
        (materialAmbientColor +
        materialDiffuseColor * lightStrength * cosTheta +
        materialSpecularColor * lightStrength * specIntensFactor * pow(cosAlpha, shininess));
    gl_FragColor.a = 1.0;
