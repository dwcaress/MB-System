...
int mbnavadjust_new_project(int verbose, char *projectpath, double section_length, int section_soundings, double cont_int,
  ...
  /* project->path/home/datadir/logfile are all built from projectpath (or its
      derived name) via unchecked strcpy/strcat below, with the longest suffix
      appended being "/log.txt" onto a ".dir" datadir (12 characters beyond
      path+name combined). Reject an oversized projectpath here rather than
      silently overflowing the fixed mb_path (MB_PATH_MAXLINE) fields. */
  if (strlen(projectpath) >= MB_PATH_MAXLINE - 16) {
    fprintf(stderr, "Unable to create new project!\nProject path too long (%zu characters, maximum %d): %s\n",
            strlen(projectpath), MB_PATH_MAXLINE - 16, projectpath);
    ...
    if (status == MB_SUCCESS) {
      ...
      if (hfp != NULL) {
        ...
        if (version_id >= 312) {
          ...
          /* clamp num_refgrids read from file to the valid range - a corrupted or
              malicious project file must not be trusted to drive loop bounds and
              indices into the fixed-size refgrid_names/refgrid_bounds arrays */
          if (project->num_refgrids < 0 || project->num_refgrids > MBNA_REFGRID_NUM_MAX) {
            fprintf(stderr, "Warning: NUMREFERENCEGRIDS value %d out of range [0,%d] read from project file %s - clamping\n",
                    project->num_refgrids, MBNA_REFGRID_NUM_MAX, project->home);
            project->num_refgrids = project->num_refgrids < 0 ? 0 : MBNA_REFGRID_NUM_MAX;
          }
...
__FILE__, __LINE__, __FUNCTION__, ifile, isection, buffer, result, nscan);
            ...
            /* a real global tie cannot exist without a valid snav index - if the file
                claims a tie (status != MBNA_TIE_NONE) but snav is the MBNA_SELECT_NONE (-1)
                sentinel, the file is inconsistent; treat this as "no tie" rather than trusting
                a -1 index into the fixed snav_time_d[]/snav_lon_offset[]/etc arrays below.
                Fixing this once here (immediately after the file is read) means every
                downstream use of section->globaltie.snav can assume it is valid whenever
                section->globaltie.status != MBNA_TIE_NONE. */
            if (section->globaltie.status != MBNA_TIE_NONE && section->globaltie.snav == MBNA_SELECT_NONE) {
              fprintf(stderr, "Warning: section globaltie.status %d claims a tie but globaltie.snav is unset (%d) - "
                      "resetting globaltie.status to MBNA_TIE_NONE\n",
                      section->globaltie.status, section->globaltie.snav);
              section->globaltie.status = MBNA_TIE_NONE;
            }
          ...
          if (status == MB_SUCCESS) {
            for (int itie = 0; itie < crossing->num_ties; itie++) {
              ...
              if (status == MB_SUCCESS) {
                ...
                if (tie->snav_1 < 0 || tie->snav_1 >= section->num_snav) {
                  fprintf(stderr, "Crossing %d (%4.4d:%4.4d %4.4d:%4.4d)  Reset tie snav_1 on read from %d to ",
                    ...
                    tie->snav_1);
                  if (tie->snav_1 < 0 || section->num_pings <= 0) {
                    tie->snav_1 = 0;
                  }
                  else {
                  ...
                  if (tie->snav_1 < 0)
                    tie->snav_1 = 0;
                  else if (tie->snav_1 >= section->num_snav)
                    tie->snav_1 = section->num_snav > 0 ? section->num_snav - 1 : 0;
                ...
                if (tie->snav_2 < 0 || tie->snav_2 >= section->num_snav) {
                  ...
                  if (tie->snav_2 < 0)
                    tie->snav_2 = 0;
                  else if (tie->snav_2 >= section->num_snav)
                    tie->snav_2 = section->num_snav > 0 ? section->num_snav - 1 : 0;
    ...
    if ((project->logfp = fopen(project->logfile, "a")) != NULL) {
      ...
      *error = MB_ERROR_INIT_FAIL;
      status = MB_FAILURE;
      /* the project was not fully opened - make sure mbnavadjust_close_project()
          is not later fooled into treating this as a live open project with a
          NULL logfp */
      project->open = false;
    }
  }
...
int mbnavadjust_close_project(int verbose, struct mbna_project *project, int *error) {
  ...
  /* add info text - guard against a NULL logfp (e.g. if the log file failed
      to open during mbnavadjust_read_project()) so closing a not-fully-open
      project cannot crash here */
...
//__FILE__, __LINE__, __FUNCTION__, file->block, ifile, isection, section->distance, section->num_beams);
        ...
        /* sectionprior->num_snav should always be > 0 here (a section always has at
            least one snav point), but guard against the degenerate case anyway so a
            corrupted project can't drive a negative index into snav_num_ties[] */
        if (sectionprior->num_snav > 0) {
        ...
        mb_make_info(verbose, true, priorsectionfile, MBF_MBLDEOIH, error);
      ...
      if (import_tie_file_1 >= 0 && import_tie_section_1 >= 0 && import_tie_snav_1 >= 0
        ...
        if (!found) {
          ...
          if (project->num_crossings_alloc <= project->num_crossings) {
            ...
            if (tptr != NULL) {
              project->crossings = (struct mbna_crossing *)tptr;
...
int mbnavadjust_section_load(int verbose, struct mbna_project *project,
  ...
  if (project->open && project->num_crossings > 0) {
    ...
    mb_pathplus tpath;
    snprintf(tpath, sizeof(mb_pathplus), "%s/nvs_%4.4d_%4.4d.mb71.tri", project->datadir, file_id, section_id);
    const int iformat = MBF_MBLDEOIH;
    struct mbna_file *file = &(project->files[file_id]);
    ...
    if (status == MB_SUCCESS) {
      ...
      while (!done) {
        ...
        if (status == MB_SUCCESS && kind == MB_DATA_DATA) {
          ...
          if (status == MB_SUCCESS && ping->beams_bath_alloc < beams_bath) {
...
int mbnavadjust_import_file(int verbose, struct mbna_project *project,
  ...
  bool section_alloc_failed = false;
  project->inversion_status = MBNA_INVERSION_NONE;
  ...
  if (status == MB_SUCCESS) {
    ...
    while (*error <= MB_ERROR_NO_ERROR) {
      ...
      /* extract beam widths */
      if (*error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA && !beamwidths_set) {
        status = mb_beamwidths(verbose, imbio_ptr, &beamwidth_xtrack, &beamwidth_ltrack, error);
        beamwidths_set = true;
      }
      ...
      if (kind == MB_DATA_DATA && *error == MB_ERROR_NO_ERROR && new_section) {
        ...
        if (file->num_sections_alloc <= file->num_sections) {
          tptr = realloc(file->sections, sizeof(struct mbna_section) *
                      (file->num_sections_alloc + ALLOC_NUM));
          if (tptr != NULL) {
            file->sections = (struct mbna_section *)tptr;
            file->num_sections_alloc += ALLOC_NUM;
          }
          else {
            section_alloc_failed = true;
            status = MB_FAILURE;
            *error = MB_ERROR_MEMORY_FAIL;
            mb_error(verbose, *error, &error_message);
            fprintf(stderr, "\nError in function <%s>:\n%s\n", __func__, error_message);
            fprintf(stderr, "\nUnable to allocate additional section array space importing <%s>\n", ipath);
          }
        }

        /* only proceed to initialize and use the new section if the
            section array is known to be valid - otherwise fall through
            and let the ping-reading loop terminate on the memory error */
        if (status == MB_SUCCESS) {
          ...
          if ((status = mb_write_init(verbose, opath, MBF_MBLDEOIH, &ombio_ptr, &obeams_bath, &obeams_amp, &opixels_ss,
                    error)) != MB_SUCCESS) {
            mb_error(verbose, *error, &error_message);
        ...
        } /* end if (status == MB_SUCCESS) section-array-valid guard */
      }
  ...
  /* a section array allocation failure aborted reading of this file part way
      through - make sure that failure is reflected in the final status even
      though the cleanup calls above reuse the status and error variables */
  if (section_alloc_failed) {
    status = MB_FAILURE;
  }
...
int mbnavadjust_update_file(int verbose, struct mbna_project *project,
  ...
  int sbeams_amp = 0;
  int spixels_ss = 0;
  char scomment[MB_COMMENT_MAXLINE];
  char *sbeamflag = NULL;
  double *sbath = NULL;
  double *sbathacrosstrack = NULL;
  double *sbathalongtrack = NULL;
  double *samp = NULL;
  double *sss = NULL;
  double *sssacrosstrack = NULL;
  double *sssalongtrack = NULL;
  ...
  if (ifile >=  0 && ifile < project->num_files) {
    ...
    if (fstat == 0 && (file_status.st_mode & S_IFMT) != S_IFDIR) {
      ...
      strncat(fbtpath, ".fbt", sizeof(mb_path));
      fstat = stat(fbtpath, &file_status);
      if (fstat == 0 && (file_status.st_mode & S_IFMT) != S_IFDIR) {
      	strncpy(ipath, fbtpath, sizeof(mb_path));
      	iformat = MBF_MBLDEOIH;
	  }
	}
  }
  ...
  if (update) {
	...
	*error = MB_ERROR_NO_ERROR;
	...
	/* register data arrays for memory allocation */
	if (status == MB_SUCCESS) {
	  if (*error == MB_ERROR_NO_ERROR)
	   status = mb_register_array(verbose, imbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(char), (void **)&beamflag, error);
	  if (*error == MB_ERROR_NO_ERROR)
	...
	while (!done) {
      ...
      /* read a ping of input processed data */
      status = mb_get_all(verbose, imbio_ptr, &istore_ptr, &kind, time_i, &time_d, &navlon, &navlat, &speed, &heading,
              &distance, &altitude, &sensordepth, &beams_bath, &beams_amp, &pixels_ss, beamflag, bath, amp,
              bathacrosstrack, bathalongtrack, ss, ssacrosstrack, ssalongtrack, comment, error);
    
      /* if at end of file set done */
      if (*error > 0) {
        done = true;
      }
      ...
      if (*error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA) {
        ...
        /* figure out which section this ping is in */
        struct mbna_file *file = &project->files[ifile];
        int new_section = -1;
        for (int isection = current_section; isection < file->num_sections && new_section < 0; isection++) {
          struct mbna_section *section = &file->sections[isection];
          if ((time_d >= section->btime_d - MB_ESF_MAXTIMEDIFF) && (time_d <= section->etime_d + MB_ESF_MAXTIMEDIFF)) {
          	new_section = isection;
          }
        }

        /* close old section if finished */
        if (section_initialized && new_section != current_section) {
          status = mb_close(verbose, &smbio_ptr, error);
          status = mb_close(verbose, &ombio_ptr, error);
		
		  /* delete old section file and related ancilliary files */
		  snprintf(spath, sizeof(mb_path), "%s/nvs_%4.4d_%4.4d.mb71", project->datadir, ifile, current_section);
		  remove(spath);
		  snprintf(opath, sizeof(mb_path), "%s.fnv", spath);
		  remove(opath);
		  snprintf(opath, sizeof(mb_path), "%s.inf", spath);
		  remove(opath);
		  snprintf(opath, sizeof(mb_path), "%s.tri", spath);
		  remove(opath);
		  
		  /* rename the new section file and make ancilliary files */
        ...
        if (!section_initialized && new_section >= 0) {
		  ...
		  snprintf(spath, sizeof(mb_path), "%s/nvs_%4.4d_%4.4d.mb71", project->datadir, ifile, new_section);
		  if ((status = mb_read_init(verbose, spath, MBF_MBLDEOIH, pings, lonflip, bounds, btime_i, etime_i, speedmin, timegap,
				&smbio_ptr, &btime_d, &etime_d, &sbeams_bath, &sbeams_amp, &spixels_ss, error)) != MB_SUCCESS) {
			mb_error(verbose, *error, &error_message);
			fprintf(stderr, "\nMBIO Error returned from function <mb_write_init>:\n%s\n", error_message);
			fprintf(stderr, "\nSwath sonar File <%s> not initialized for writing\n", spath);
		  }
		  snprintf(opath, sizeof(mb_path), "%s/tmp_nvs_%4.4d_%4.4d.mb71", project->datadir, ifile, new_section);
		  if ((status = mb_write_init(verbose, opath, MBF_MBLDEOIH, &ombio_ptr, &obeams_bath, &obeams_amp, &opixels_ss,
					error)) != MB_SUCCESS) {
			mb_error(verbose, *error, &error_message);
			fprintf(stderr, "\nMBIO Error returned from function <mb_write_init>:\n%s\n", error_message);
		  ...
		  if (status == MB_SUCCESS) {
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(char), (void **)&sbeamflag, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&sbath, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_AMPLITUDE, sizeof(double), (void **)&samp, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&sbathacrosstrack, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_BATHYMETRY, sizeof(double), (void **)&sbathalongtrack, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_SIDESCAN, sizeof(double), (void **)&sss, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_SIDESCAN, sizeof(double), (void **)&sssacrosstrack, error);
			if (*error == MB_ERROR_NO_ERROR)
			 status = mb_register_array(verbose, smbio_ptr, MB_MEM_TYPE_SIDESCAN, sizeof(double), (void **)&sssalongtrack, error);
			if (*error == MB_ERROR_NO_ERROR)
...
int mbnavadjust_coverage_mask(int verbose, struct mbna_project *project,
  ...
  /* initialize reading the swath file */
  if ((status = mb_read_init(verbose, sectionpath, MBF_MBLDEOIH, 1, lonflip, bounds, btime_i, etime_i, speedmin, timegap,
...
int mbnavadjust_addcrossing(int verbose, struct mbna_project *project, int ifile1, int isection1, int ifile2, int isection2, int *error) {
  ...
  if (!disqualify) {
    /* allocate mbna_crossing array if needed */
    if (project->num_crossings_alloc <= project->num_crossings) {
      void *tptr = realloc(project->crossings, sizeof(struct mbna_crossing) *
                       (project->num_crossings_alloc + ALLOC_NUM));
      if (tptr != NULL) {
        project->crossings = (struct mbna_crossing *)tptr;
        project->num_crossings_alloc += ALLOC_NUM;
        ...
        *error = MB_ERROR_MEMORY_FAIL;
        mb_error(verbose, *error, &error_message);
        fprintf(stderr, "\nError in function <%s>:\n%s\n", __func__, error_message);
        fprintf(stderr, "\nUnable to allocate additional crossing array space\n");
      }
    }
