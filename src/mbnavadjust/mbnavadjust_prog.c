...
mbnavadjust_invertnav()
{
	...
	/* invert if there is a project and enough crossings have been analyzed */
		...
		/* count the number of global ties */
		nglobal = 0;
		for (i=0;i<project.num_files;i++)
		    {
		    file = &project.files[i];
...
icrossing,j,tie->offset_z_m,nc2,x[3*nc2+2],nc1,x[3*nc1+2],offsetz,nc2,xx[3*nc2+2],nc1,xx[3*nc1+2]);*/
			...
			for (isection=0;isection<file->num_sections;isection++)
				...
				if (section->global_tie_status != MBNA_TIE_Z)
				    {
				    offsetx = section->global_tie_offset_x_m
						- xa[3*nc1]
						- (x[3*nc1] + xx[3*nc1]);
				    offsety = section->global_tie_offset_y_m
						- xa[3*nc1+1]
						- (x[3*nc1+1] + xx[3*nc1+1]);
				    }
				if (section->global_tie_status != MBNA_TIE_XY)
				    {
				    offsetz = section->global_tie_offset_z_m
						- xa[3*nc1+2]
						- (x[3*nc1+2] + xx[3*nc1+2]);
				    }
				offsetr = sqrt(offsetx * offsetx + offsety * offsety + offsetz * offsetz);
				...
				if (section->global_tie_status != MBNA_TIE_Z)
				    ...
				    offsetsigma = offsetx;
				    if (fabs(offsetsigma) > 0.0)
					weight = MAX(mbna_offsetweight / section->global_tie_xsigma, 1.0);
				    else
					weight = 1.0;
				    xx[3*nc1]   += weight * offsetsigma;
					
				    /* deal with y-axis */
				    offsetsigma = offsety;
				    if (fabs(offsetsigma) > 0.0)
					weight = MAX(mbna_offsetweight / section->global_tie_ysigma, 1.0);
				    else
					weight = 1.0;
				    xx[3*nc1+1] += weight * offsetsigma;
				    }
				...
				if (section->global_tie_status != MBNA_TIE_XY)
				    ...
				    offsetsigma = offsetz;
				    if (fabs(offsetsigma) > 0.0)
					weight = MAX(mbna_offsetweight / section->global_tie_zsigma, 1.0);
				    else
					weight = 1.0;
				    xx[3*nc1+2] += weight * offsetsigma;
				    }
...
fprintf(stderr," xx: ");
if (section->global_tie_status != MBNA_TIE_Z) fprintf(stderr,"x:%f y:%f ",xx[3*nc1],xx[3*nc1+1]);
if (section->global_tie_status != MBNA_TIE_XY) fprintf(stderr,"z:%f ",xx[3*nc1+2]);
...
fprintf(stderr, "BAD GLOBAL TIE snav ID: %d %d\n", nc1, nsnav);
				...
				/* get x axis misfit */
				misfit = mbna_offsetweight / section->global_tie_xsigma * offsetx;
				misfit_ties += misfit * misfit;
    
				/* get y axis misfit */
				misfit = mbna_offsetweight / section->global_tie_ysigma * offsety;
				misfit_ties += misfit * misfit;
    
				/* get z axis misfit */
				misfit = mbna_offsetweight / section->global_tie_zsigma * offsetz;
				misfit_ties += misfit * misfit;
				}
			    }
			}
