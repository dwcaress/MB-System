...
// Function pointers to update graphics, passed through mbedit_init()
// For older apps such as mbedit, these functions may be X11-based.
// Newer apps may pass Qt-based functions.
static void (*drawLine)(void *gPtr, int x1, int y1, int x2, int y2,
			unsigned int color, int style);

static void (*drawRect)(void *gPtr, int x, int y, int width, int height,
			unsigned int color, int style);

static void (*fillRect)(void *gPtr, int x, int y, int width, int height,
			unsigned int color, int style);
...
int (*resetScaleX)(int pwidth, int maxx, int xntrvl, int yntrvl);

// Pointer, may point to graphics context
void *graphPtr;
...
int mbedit_init(int argc, char **argv, int *startup_file,
		void *gPtr,
		void (*drawLineArg)(void *gPtr, int x1, int y1, int x2, int y2,
				    unsigned int color, int style),
		...
		void (*enableNextButtonArg)(void),
		void (*disableNextButtonArg)(void),
		int (*resetScaleXArg)(int pwidth, int maxx,
				      int xntrvl, int yntrvl)) {
  graphPtr = gPtr;
  drawLine = drawLineArg;
  drawRect = drawRectArg;
  ...
  enableNextButton = enableNextButtonArg;
...
int mbedit_action_open(char *file, int form, int fileid, int numfiles, int savemode, int outmode, int plwd, int exgr, int xntrvl,
  ...
  if (status == MB_SUCCESS) {
    ...
    if (*nloaded == 0)
      (*showError)("No data were loaded from the input", "file. You may have specified an",
...
int mbedit_action_filter_all(int plwd, int exgr, int xntrvl, int yntrvl, int plt_size, int sh_mode, int sh_flggdsdg, int sh_flggdprf, int sh_time,
  ...
  /* do nothing unless file has been opened */
  if (file_open) {
    (*showMessage)("MBedit is applying bathymetry filters...");
...
int mbedit_open_file(char *file, int form, bool savemode) {
  ...
  if (status == MB_SUCCESS) {
	...
	(*showError)("Unable to open new edit save file.", "You may not have write", "permission in this directory!");
      }
      else if (status == MB_FAILURE && error == MB_ERROR_MEMORY_FAIL) {
	esffile_open = false;
	fprintf(stderr, "\nUnable to allocate memory for edits in esf file %s\n", esf.esffile);
	(*showError)("Unable to allocate memory for.", "edits in existing edit", "save file!");
      }
    }
  }
...
int mbedit_close_file() {
  ...
  if (output_mode == MBEDIT_OUTPUT_EDIT) {
    ...
    if (run_mbprocess) {
      /* turn message on */
      (*showMessage)("Bathymetry edits being applied using mbprocess...");
...
int mbedit_dump_data(int hold_size, int *ndumped, int *nbuffer) {
  ...
  if (nbuff > 0) {
    /* turn message on */
    (*showMessage)("MBedit is clearing data...");
...
int mbedit_load_data(int buffer_size, int *nloaded, int *nbuffer, int *ngood, int *icurrent) {
  ...
  *icurrent = current_id;
  ...
  if (esf.nedit > 0) {
    /* reset message */
    (*showMessage)("MBedit is applying saved edits...");
...
int mbedit_plot_all(int plwd, int exgr, int xntrvl, int yntrvl, int plt_size,
  ...
  *nplt = nplot;
  ...
  if (autoscale && xtrack_max > 0.0) {
    ...
    yntrvl = xntrvl;
    (*resetScaleX)(plot_width, maxx, xntrvl, yntrvl);
  }
...
int mbedit_action_goto(int ttime_i[7], int hold_size, int buffer_size, int plwd, int exgr, int xntrvl, int yntrvl, int plt_size,
  ...
  if (!file_open) {
    ...
    *nplt = 0;
    ...
    (*showError)("No data file has", "been opened...", "  ");
  }
  ...
  /* check if the target time is in the present buffer */
	...
	fprintf(stderr, "\n>> Beginning of present buffer is later than target time...\n");
      (*showError)("Beginning of loaded data", "is later than the", "specified target time...");
    }
	...
	fprintf(stderr, "\n>> Target time is beyond end of file...\n");
      (*showError)("Target time is", "beyond the end", "of the data file...");
    }
  }
  ...
  while (!found && status == MB_SUCCESS) {
	...
	*nplt = 0;
	...
	(*showError)("Target time is beyond the", "end of the data file!", "The file has been closed...");
      }
    }
