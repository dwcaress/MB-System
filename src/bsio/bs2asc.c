...
char *sdstr[ACP_NSIDES] = {"Port", "Starboard"};
...
int main(int argc, char **argv) {
	...
	for (i = 0; (i < count) || ((count == BS_UNDEFINED) && (ignorecnt == 1)); i++) {
		...
		if ((output != OUTPUT_HDRSONLY) && (showivisping || mbbs_pngvisible(pngp->png_flags))) {
			if (showsns) {
				(void)fprintf(fp, "\nCompass Data:\n");
...
void usage() {
	(void)fprintf(stderr,
	              "usage: %s [ -f filenm] [ -h | -d ] [ -nip ] [ -nsns ] [ -nb ] [ -nbd ] [ -nbf ] [ -nabi ] [ -ns ] [ -nsd ] [ "
	              "-nsf ] [ -i ] [ -pco ] [ -jt | -ct ] [ -s skip ] [ -c count ] [ -H ] -sm shmcid | < bsfile > ASCIIfile\n",
	              progname);
	exit(BS_BADARG);
}

void pr_bsfhdr(BSFile *bsf) {
...
void pr_pnghdr(Ping *png) {
	...
	case TM_CALENDAR:
		(void)fprintf(fp, "Time: %1d/%1d/%1d %02d:%02d:%02d.%03ld\n", (int)(tmp->tm_year + 1900), (int)(tmp->tm_mon + 1),
		              tmp->tm_mday, tmp->tm_hour, tmp->tm_min, tmp->tm_sec, msec);
		break;
	}
